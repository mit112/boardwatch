...
2. `remoteType` is a **real structured field** on the listed-posting object, but it is
   tenant-dependent: some tenants populate it, some omit the key entirely. It must not be
   assumed present.
3. Requisition tokens embedded in `externalPath` sometimes carry a `-N` instance suffix
   (e.g. `..._JR1000001-2`), not just the bare requisition id.
...
| `detail_normal.json` | A detail payload for the Senior Platform Engineer posting: full `jobPostingInfo` including `jobReqId` and `jobPostingId`. |
...
4. `remoteType` is present on rows 1 and 2 and **absent** (key missing entirely) on row 3
   — pins that the field must be read defensively.
5. Row 1's requisition token in `externalPath` carries the `-N` instance suffix
   (`..._JR1000001-2`) — pins that the suffix must not be assumed away.
