...
# --- ruling 3: company_title_location suppresses, but only on a near-identical body ------


_JD = (
    "Affirm is reinventing credit. As a Manager of Software Engineering on the Resilience "
    "Engineering team you will lead a group of engineers building the platform that keeps "
    "Affirm available. You will set technical direction, grow the team, and partner with "
    "product. We are looking for 8+ years of industry experience and 2+ years managing."
)


def _ctl_pair(body_a: str, body_b: str) -> tuple[IdentityInputs, ...]:
    """Two postings sharing company, title and locations, differing only in body.

    Distinct `content_hash` on purpose: that is what makes `exact_quad` decline, so anything
    suppressed here was suppressed by `company_title_location` and nothing else. No forged
    identity is needed — the two rows group on that key naturally, exactly as the live pair
    did. The titles must MATCH, because the resolver re-compares the strings behind the hash
    rather than trusting it.
    """
    return (
        _p(1, title="Manager, Software Engineering", body_text=body_a,
           content_hash=content_hash(body_a)),
        _p(2, title="Manager, Software Engineering", body_text=body_b,
           content_hash=content_hash(body_b)),
    )


def test_company_title_location_suppresses_a_reposted_job():
    """The measured known positive: Affirm 1930/1931.

    Same role, same city, two requisitions, bodies 0.967 similar — and each built its own
    resume because their content hashes differ, so `exact_quad` never saw them.
    """
    a, b = _ctl_pair(_JD, _JD.replace("8+ years", "7+ years") + " Equal opportunity employer.")
    result = _resolve(a, b)
    assert [(s.posting_id, s.kind) for s in result] == [(2, "company_title_location")]


def test_company_title_location_spares_two_openings_sharing_a_title():
    """The measured known NEGATIVE, and the reason the body clause exists at all.

    Stripe ran two "Backend Engineer, Payments and Risk" openings in one city on the same
    day — Bank Connections and Optimized Checkout, different requisitions, bodies 0.740
    similar. Eight Adobe "Software Development Engineer" openings in San Jose are the same
    shape. Suppressing either is hiding a real job.
    """
    other = (
        "Stripe is looking for a backend engineer on the Optimized Checkout team. You will "
        "build the checkout experiences merchants embed, own services in Ruby and Go, and "
        "work across the payments stack. This is a different charter from balance management."
    )
    assert _resolve(*_ctl_pair(_JD, other)) == ()


def test_a_missing_body_never_suppresses():
    """Fail-safe direction: no evidence is not agreement. An absent body scores 0."""
    assert _resolve(*_ctl_pair(_JD, "")) == ()


def test_no_suppression_survives_pointing_at_a_hidden_posting():
    """Two suppressing kinds make CHAINS reachable, and both consumers break on one.

    An `exact_quad` survivor is not skipped when the weaker kind groups, so it can itself be
    suppressed afterwards. `top --include-duplicates` would then name a survivor the operator
    cannot see, and `identities regroup` would anchor a permanent grouping row on it.
    """
    shared = _JD + " We work in Python."
    edited = _JD + " We work in Python and Go."
    rows = (
        # 1 and 2 are byte-identical: an exact_quad pair.
        _p(1, title="Manager, Software Engineering", body_text=shared,
           content_hash=content_hash(shared), first_seen_at=datetime(2026, 8, 2, 12, 0, 0)),
        _p(2, title="Manager, Software Engineering", body_text=shared,
           content_hash=content_hash(shared), first_seen_at=datetime(2026, 8, 3, 12, 0, 0)),
        # 3 is the earliest, so it wins the election, and its body is a near-identical edit.
        _p(3, title="Manager, Software Engineering", body_text=edited,
           content_hash=content_hash(edited), first_seen_at=datetime(2026, 8, 1, 12, 0, 0)),
    )
    result = resolve_duplicates(rows, {r.posting_id: compute_identities(r) for r in rows})
    hidden = {s.posting_id for s in result}
    assert hidden, "expected the group to suppress something"
    for suppression in result:
        assert suppression.survivor_posting_id not in hidden, (
