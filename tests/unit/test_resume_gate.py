...
from boardwatch.tailor.render.typst import TypstRenderer, escape
...
def test_validate_layout_rejects_escaping_mismatch_on_unescaped_bullet() -> None:
    text = 'Shipped the "critical" path fix for the checkout flow quickly'
    r = _layout_resume(
        entries=[Entry(entry_id="e1", heading="H", bullets=[_clean_bullet(text=text)])]
    )
    good_source = TypstRenderer().emit(r)
...
def test_validate_layout_accepts_a_correctly_escaped_quote() -> None:
    text = 'Shipped the "critical" path fix for the checkout flow quickly'
    r = _layout_resume(
        entries=[Entry(entry_id="e1", heading="H", bullets=[_clean_bullet(text=text)])]
    )
