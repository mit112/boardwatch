...
const AUTHORED: QueueRow[] = [
  {
    ...
    pdf_available: true,
    score: 87.4,
    why: 'target company (+18.0), title "software engineer" (+22.4), coverage 0.72 (+14.4), posted 3d ago (+6.0)',
    coverage: 0.72,
  ...
  },
  {
    ...
    pdf_available: true,
    score: 84.1,
    why: 'target company (+18.0), title "backend engineer" (+20.1), coverage 0.64 (+12.8), new-grad match (+8.0)',
    coverage: 0.64,
    off_target_reason: null,
    review_reason: null,
  },
  ...
  },
  {
    ...
    locations: ["San Francisco, CA", "Remote, United States"],
    remote_policy: null,
    posted_days: 6,
...
function generate(count: number, seed: number, idBase: number): QueueRow[] {
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    // Roughly a quarter of the corpus lists a second location, which is what makes the `+N` on
    // the row and the comma-separated line in the pane demonstrable without a live store.
    const where = pick(random, LOCATIONS);
    const alsoWhere = random() < 0.25 ? pick(random, LOCATIONS) : null;
    const locations =
      alsoWhere === null || alsoWhere === where ? [where] : [where, alsoWhere];
    rows.push({
      ...
      // The generated corpus leaves four rows in five unexplained, which is the mix the live
      // store shows: the score cell has to read correctly with and without a reason.
      why:
        random() < 0.2
          ? `title "${spec.title.toLowerCase()}" matched, coverage ${thin ? "not measured" : "measured"}`
          : null,
