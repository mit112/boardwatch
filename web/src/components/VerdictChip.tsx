...
export function VerdictChip({ verdict }: { verdict: Verdict | null }) {
  if (verdict === null) {
    return (
      <span
        className="inline-flex min-h-5 items-center gap-1.5 rounded border border-dashed border-control px-2 py-0.5 text-xs text-fg-3"
  ...
  return (
    <span
      className={`inline-flex min-h-5 items-center gap-1.5 rounded px-2 py-0.5 text-xs ${style.className}`}
      title={style.hint}
    >
