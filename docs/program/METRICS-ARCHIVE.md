# Metrics — archive (P0 … P5, sessions through Gate P2)

The closed per-session measurement records, moved here verbatim from `METRICS.md` by **D-108** so the live
file stays cheap to open. Nothing was reworded, recomputed or dropped.

**This file is closed.** New measurements go in `METRICS.md`, never here. The live run log and the
acceptance-run table stayed there, because both are still appended to. The two files share one index, which
lives in `METRICS.md` — start there.

Same conventions as the live file: `—` = not emitted, which is not `0`.
