...
| Windows evidence, and what it is bound to | Dispatch **`32047384310`** on **`0ab16e9`**; its `generalization`, `gitleaks` and `perf` jobs (CI-only, never local) went green first. The review fixes are a later commit, so the Windows verdict is re-taken on the final tip rather than inherited |
| Phase gates | Unmoved. **Still 0 applications sent** |
