# Decision log — archive (D-001 … D-076)

Entries D-001 through D-076, moved here verbatim from `DECISIONS.md` by **D-108** so the live log stays
cheap to open. Nothing was reworded, summarised or dropped; the entries are byte-for-byte what they were.

**This file is closed.** New entries are appended to `DECISIONS.md`, never here. The two files share one
index, which lives in `DECISIONS.md` — start there.

Cross-references are by number (`D-028`), so they resolve across the split without naming a file.
