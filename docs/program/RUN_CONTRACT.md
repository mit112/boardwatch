...
| 2 | **No profile configured** (`NoProfileError`, raised in `src/boardwatch/cli/top_cmd.py:42`) | caught at `src/boardwatch/pipeline/runner.py:186-193` | A fresh install has nothing to rank against; nothing downstream (tailoring) can run. |
| 3 | **Typst binary unavailable** (`TypstUnavailableError`, `src/boardwatch/reports/resume_gate.py:18`) | caught at `src/boardwatch/pipeline/runner.py:216-224` | An environment fault, not a per-lead one: the binary is either on `PATH` or it isn't, so every remaining lead in the shortlist would fail identically — the stage aborts rather than burning through the whole shortlist re-discovering that. |
| 4 | **Every lead failed to tailor** from a non-empty shortlist | `src/boardwatch/pipeline/runner.py:258-262` | Zero output was not provably right: it was produced from postings the ranker actually shortlisted, which means the résumé path itself is broken (missing `resume.yaml`, typst gone), not an honest empty day. |
