...
| **A `SIGKILL`ed run leaves a dangling `runs` row, and the drain has never once fired** | `try/finally` covers exceptions and Ctrl-C, not SIGKILL. **Measured 2026-08-14: 8 rows in `status='running'`** — ids 1, 2, 3, 4, 8, 15, 16, 17, the oldest from **2026-08-04**. This corrects two claims: it was "three rows (ids 15–17)", and "largely drained by the age-based reaper" — **nothing has been drained.** `reap_stale_runs` is wired on both sides (`pipeline/runner.py:374` and `cli/doctor_cmd.py:128`) and `reap_stale_after_hours` defaults to 24, so every row qualifies; but both call sites are user-invoked, `run` has not been run and `doctor` takes minutes, so **the drain exists and is unscheduled, which is the leak.** A one-shot `boardwatch doctor` would clear it. `top` is what opens them | P3 |
