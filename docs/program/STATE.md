...
| **Gate A T1–T10's review is STARTED, not done** | Stopped early on usage grounds; only the closed-catalog checks completed. **Verified: 9 catalogs and all 41 predicate rows × 14 columns match the design exactly**, negative-controlled. **Still owed: the code-running T10 pass, the rest of the T1–T9 foundations, and a docs-only pass.** Gate B stays prohibited. Being on `origin/main` is not sign-off, and neither is being published | review |
...
| **One MAJOR and five MINORs are open from the partial review** | The MAJOR: §10.4's "every legal verification basis must be backed by its corresponding evidence class" is not validated at catalog level (`models/policy.py:124-171`), so a tenant-authored predicate row can declare an unreachable evidence route. Latent — shipped data conforms. Full table in `METRICS.md`'s T10 record. **Not fixed: batched behind one `make check` rather than paying ~7.5 min per finding** | Gate A |
