...
| Item | Detail | Owner |
|---|---|---|
| **CI is RED on `test (3.12, ubuntu-latest)`** | **Four consecutive failures. D-153's fix did NOT work** — do not trust any record saying otherwise. `test_abstain_names_rules_that_have_never_been_detected` fails because the rich table renders too narrow and a 34-char rule id is not a contiguous substring. **Any root cause must explain why 3.11 and 3.13 pass on the identical runner** — an env explanation cannot. Investigation dispatched to worktree branch `fix/ubuntu-312-abstain` | open |
