...
| Item | Detail | Owner |
|---|---|---|
| **The ubuntu/3.12 CI failure — FIXED and CI-CONFIRMED (D-159)** | rich 15's `Console.__init__` bakes width from `COLUMNS` **eagerly**, and `cli/eligibility_cmd.py:66` builds its console at import — so `monkeypatch.setenv("COLUMNS", …)` was a **no-op** and three width-controlling tests had never controlled anything. Fixed by popping `COLUMNS`/`LINES` at conftest import, plus a premise test. **Green on run `31698148914` at `950578f`, all 9 push-path jobs, `test (3.12, ubuntu-latest)` included** — the job that had failed six consecutive times and survived two confident wrong fixes. Reproduce the old failure in 1.5 s: `COLUMNS=80 uv run pytest tests/unit/test_eligibility_cmd.py -k abstain --no-cov -n 0`. **D-153 is partial, not wrong** — it closed the `is_terminal` arm, not this one | **CLOSED** |
| **Residual of D-159: nothing explains what supplied `COLUMNS≈80` to that one job** | Red began at exactly `e629ea1`, the commit that added `-n auto` (D-150), and the fix does not depend on the answer — it makes the tests control their own width either way. Carried deliberately rather than closed by assertion. A push run is **9 jobs, not 12**: D-151 took Windows off the per-push path (`ci.yml:21-33`), so it runs nightly and on `workflow_dispatch` only | unscheduled |
