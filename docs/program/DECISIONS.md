...
| D-077 | DECISIONS.md | 323 | P6 Slice 1: the design is settled and the plan is written; no code exists yet |
| D-078 | DECISIONS.md | 413 | P6 Slice 1: the plan's test fixtures are now real; eleven defects, all found by running code |
| D-079 | DECISIONS.md | 502 | P6 Slice 1 annotates only; `postings.job_id` is not mutated |
| D-080 | DECISIONS.md | 516 | `content_hash` alone may never suppress |
| D-081 | DECISIONS.md | 530 | `exact_quad` is the sole suppressing kind, and its yield is stated honestly |
| D-082 | DECISIONS.md | 548 | `cross_host` ships annotate-only, reversing an earlier draft |
| D-083 | DECISIONS.md | 569 | No location evidence ⇒ no location-bearing identity, never a `"[]"` sentinel |
| D-084 | DECISIONS.md | 584 | Three host classes, not two; matching is exact-or-dot-suffix |
| D-085 | DECISIONS.md | 598 | Allowlist URL normalization, not a denylist |
| D-086 | DECISIONS.md | 612 | Survivor election never consults score; `posting_id` is a load-bearing tiebreak |
| D-087 | DECISIONS.md | 627 | Instrumentation is completeness-gated, not existence-gated |
| D-088 | DECISIONS.md | 641 | `assisted` stays `None` in this slice |
| D-089 | DECISIONS.md | 656 | Identities are upserted on every observation; a kind that stops being produced is deleted |
| D-090 | DECISIONS.md | 673 | The ranker is completeness-gated for reproducibility, not safety |
| D-091 | DECISIONS.md | 691 | The recount recomputes in Python, and claims staleness only |
| D-092 | DECISIONS.md | 706 | Identities are backfilled by an explicit command, not by the migration |
| D-093 | DECISIONS.md | 720 | Slice 1 does NOT meet Gate P6, and makes only one of its four clauses measurable |
| D-094 | DECISIONS.md | 734 | P6 Slice 1 BUILT (unattended run): five more plan defects, three of them tests that could not fail |
| D-095 | DECISIONS.md | 862 | P6 Slice 1 reviewed by three independent reviewers; fourteen findings fixed, two rejected |
| D-096 | DECISIONS.md | 927 | The C++/C# fix folds punctuation into words; it does NOT add a raw-title comparison |
| D-097 | DECISIONS.md | 971 | `_verify_quad` rejected nothing on the live corpus; "string-verified" is not precision evidence |
| D-098 | DECISIONS.md | 1000 | Suppression reports when it is OFF; wiring backfill into the pipeline is Slice 2 |
| D-099 | DECISIONS.md | 1036 | Gate batching stays allowed; the per-task fast-check set must include the schema guards |
| D-100 | DECISIONS.md | 1062 | P6 Slice 1 merged to `main`; Gate P6 clause 3 is MET, not merely measurable |
| D-101 | DECISIONS.md | 1094 | Gate P6 clause 4 is MET: 20/20 sampled suppressions are genuine duplicates |
| D-102 | DECISIONS.md | 1124 | D-072 (model-tier benchmark) is deferred indefinitely |
| D-103 | DECISIONS.md | 1146 | P6 Slice 2: the ledger is a current-state row per job, `seen` suppresses on a TTL, and the policy stamp never auto-reopens |
| D-104 | DECISIONS.md | 1218 | Job regrouping: the survivor's job wins, and a tracked group is refused whole |
| D-105 | DECISIONS.md | 1261 | Identity writes move into the scan path, closing D-098 — and D-098's cost argument did not apply |
| D-106 | DECISIONS.md | 1292 | Two consequences the build forced: what earns a permanent `skipped`, and the zero-output guard |
| D-107 | DECISIONS.md | 1318 | P6 Slice 2 BUILT and verified on real data; `cross_host` dereference deferred by measured absence |
| D-108 | DECISIONS.md | 1367 | The decision log and the metrics log are archive-split; the reading protocol moves into the index |
| D-109 | DECISIONS.md | 1426 | Index drift fails the gate, and the fixer lives in `tools/` |
| D-110 | DECISIONS.md | 1504 | The Slice 2 review: only a caller that delivers a lead may consume the queue |
| D-111 | DECISIONS.md | 1621 | P6 Slice 3: applied-state suppression, and liveness sized to what the corpus actually is |
| D-112 | DECISIONS.md | 1790 | 0.3.0 is cut, the changelog gets ONE triple, and the tag is the owner's to push |
| D-113 | DECISIONS.md | 1894 | The Slice 3 external review: a followed redirect can forge a gone-status |
| D-114 | DECISIONS.md | 1982 | CI installs tectonic and pdfinfo on all three OSes; skipping the gate was refused |
| D-115 | DECISIONS.md | 2050 | Gate A of the career-profile bundle: 9 of 19 slices, and a rule for checks that cannot fire |
| D-116 | DECISIONS.md | 2132 | A docs-only commit owes the two fast gates, not the full suite; the tectonic pin gets a detector |
| D-117 | DECISIONS.md | 2179 | 0.3.0's tag moves rather than 0.3.1 being cut; gitleaks fixed by cleaning bytes, not allowlisting |
| D-118 | DECISIONS.md | 2229 | Gate A slice T10: effectiveness derived in one place, and two more §20.4 rows with no check |
| D-119 | DECISIONS.md | 2326 | 0.3.0 is PUBLISHED: the tag moved onto a CI-green commit, and ships two known BLOCKERs deliberately |
| D-120 | DECISIONS.md | 2394 | Gate A slice T12: the résumé emission order is fixed, and three more checks that cannot fire |
| D-121 | DECISIONS.md | 2477 | The T12 review: a green gate and a perfect mutation score hid five BLOCKING defects |
| D-122 | DECISIONS.md | 2538 | The T12 re-review: one defect the fix created, two contracts never enforced, and a decline that was wrong |
| D-123 | DECISIONS.md | 2661 | A recurring trigger holding a one-shot prompt re-fires a task that already shipped |
| D-124 | DECISIONS.md | 2706 | The third T12 review: the locator grammar keeps failing because it restates the emitter instead of deriving from it |
| D-125 | DECISIONS.md | 2771 | The T12 round-three fix, and two more reviews of it: a forbidden segment is escaped, never refused |
| D-126 | DECISIONS.md | 2903 | T12's review loop is CLOSED, with a stated exit criterion |
| D-127 | DECISIONS.md | 2959 | Gate A slices T13 and T14: an approval bound to nothing, and the first code that WRITES a bundle |
| D-128 | DECISIONS.md | 3080 | Gate A T14 round 2, T15 and T17: what three green suites could not see |
| D-129 | DECISIONS.md | 3269 | The two Gate A design departures are RULED: the design text was wrong, not the code |
| D-130 | DECISIONS.md | 3334 | Correcting D-128 and D-129: what the fix rounds actually established, and what the rebase actually deletes |
| D-131 | DECISIONS.md | 3409 | The T14/T15 fix-round review's findings are fixed: a merge short-cut that skipped the append-only rule, and five residues |
| D-132 | DECISIONS.md | 3544 | Gate A slice T16 reviewed by three lenses: the highest-risk slice, and the one defect two of them found separately |
| D-133 | DECISIONS.md | 3621 | Correcting D-130 and D-131: what is actually pushed, and five statements a docs review caught in this session's own records |
| D-134 | DECISIONS.md | 3693 | A finding's tier is a property of the operation, not of the code alone |
| D-135 | DECISIONS.md | 3750 | The Gate A integration gate is green on all nineteen slices, and the 03:10 job misfired a second time |
| D-136 | DECISIONS.md | 3823 | Gate A slices T18 and T19: two lenses, a ten-commit fix round, and an integration merge where two green branches wrote one rule twice |
| D-137 | DECISIONS.md | 3939 | Gate A's review loop CLOSES at round five, and a two-document write is named rather than made atomic |
| D-138 | DECISIONS.md | 4024 | A missing bundle root is its own fact, and `inventory` reporting it as clean was the defect |
| D-139 | DECISIONS.md | 4079 | `STATE.md` splits its standing facts out, because a read-first file at twice its stated length is read past |
| D-140 | DECISIONS.md | 4127 | D-116's conclusion survives, its premise does not: two tests do read the real `docs/` tree |
| D-141 | DECISIONS.md | 4175 | The third site of the blocking-`open()` class is closed, at the layout boundary |
| D-142 | DECISIONS.md | 4218 | D-138 delivered eight of twelve commands, and said twelve. The review that caught it, and what the surviving mutation cost |
| D-143 | DECISIONS.md | 4287 | `add-evidence` writes the back-citation, closing Gate A's last open question |
| D-144 | DECISIONS.md | 4370 | Grounding reads `supports` alone; citing a source is not being backed by it |
| D-145 | DECISIONS.md | 4428 | The Gate A subsystem never ran on Windows, and one `write_text` hid it |
| D-146 | DECISIONS.md | 4498 | LLM lane-death is one typed error, classified at the raise site, latched per invocation — scoped to the two lanes that call out |
| D-147 | DECISIONS.md | 4671 | Slice 5 merges as-is: four known residuals, recorded rather than fixed |
| D-148 | DECISIONS.md | 4756 | D-147's R1 closed: one flag drives the tailor lane's ledger row and its exit code |
| D-149 | DECISIONS.md | 4842 | The `STATE.md` trim is BLOCKED: three Gate A records disagree with the code or with each other |
| D-150 | DECISIONS.md | 4929 | The suite runs across worker processes; `-n auto` lives at the call sites, not in `addopts` |
| D-151 | DECISIONS.md | 5038 | Windows leaves the per-push path for a nightly schedule; it is not dropped |
| D-152 | DECISIONS.md | 5103 | Retraction: the archived CGPA claim is inverted; job-apps was never the stale copy |
| D-153 | DECISIONS.md | 5142 | A rich table's width can ignore `COLUMNS`, so terminal env is pinned for the whole suite |
| D-154 | DECISIONS.md | 5229 | `eligibility_inputs` gains an identity index; `top`'s pending anti-join cost 141 s per run |
| D-155 | DECISIONS.md | 5293 | The program reorients onto the bundle-to-résumé path; `resume.yaml` becomes an import source, not an artifact to hand-fix |
| D-156 | DECISIONS.md | 5381 | v1 projection is not authoritative for header, education or summary, because the renderer never reads them |
| D-157 | DECISIONS.md | 5476 | Corrections that unblock D-149: the manifest write order, and Windows closed by CI |
| D-158 | DECISIONS.md | 5560 | The projection scorer is chosen by measurement, because two design rounds picked two scorers and a probe falsified both |
| D-159 | DECISIONS.md | 5648 | `COLUMNS` is baked into a `Console` at import, so three width-controlling tests never controlled anything |
| D-160 | DECISIONS.md | 5753 | Preflighting a thrice-reviewed spec still found four false claims, and the plan argues from the preflight |
| D-161 | DECISIONS.md | 5857 | A third import wall guards the bundle serializer, and projection digests through the YAML writer instead |
| D-162 | DECISIONS.md | 5931 | A fourth import wall guards the CLI command module against the store, found only by tripping it |
| D-163 | DECISIONS.md | 5969 | The plan's four candidate scorers are two behavioural families, and none survives both probes |
| D-164 | DECISIONS.md | 6014 | Where the closed `ProjectionIssue` catalog is extended, and where a foreign error may escape |
| D-165 | DECISIONS.md | 6060 | A consent control gets one definition, because the rationale for copying it was false |
| D-166 | DECISIONS.md | 6101 | Projection maps its issues onto the bundle's catalog at the boundary, rather than inverting the dependency |
| D-167 | DECISIONS.md | 6136 | A projection approval binds the bundle it was made against, and the check is unconditional |
| D-168 | DECISIONS.md | 6187 | Stage 2's scorer is a required parameter with no default, because the plan is forbidden to pick one |
| D-169 | DECISIONS.md | 6221 | A plan can ship an artifact no task consumes, and only a whole-branch lens sees it |
| D-170 | DECISIONS.md | 6265 | `profile-bundle import` writes the ledger and nothing else, and derives no disposition |
| D-171 | DECISIONS.md | 6339 | A CI-only failure was a lazy-import race in typer, not an OS difference and not a regression |
| D-172 | DECISIONS.md | 6423 | Gate B is met at a promoted revision, and the extraction mapping lives inside the bundle |
| D-173 | DECISIONS.md | 6501 | Gate B gets a mechanical predicate, the drain gets a digest-bound carrier, and the mapping's carrier is questioned |
| D-174 | DECISIONS.md | 6598 | The extraction mapping's carrier is `policy/extraction-mappings.yaml`, not a `SourceSpec` field |
| D-175 | DECISIONS.md | 6634 | Review round 3 outcome: 7 findings, all accepted; the schema bump needs a real migrator, not a raw-v1 loader |
| D-176 | DECISIONS.md | 6680 | Review round 4 outcome: 4 blocking findings accepted; the kind→subject→predicate relation gets modelled once |
| D-177 | DECISIONS.md | 6725 | Review round 5: the rule interface is under-designed; revision 7 redesigns it completely, not by patch |
| D-178 | DECISIONS.md | 6779 | Stop the spec-review loop as the gate to building; de-risk the rule interface with a thin TDD slice |
| D-179 | DECISIONS.md | 6809 | The Task-1 predicate audit: seed the audited starter catalog, and roster three dead verification bases |
| D-180 | DECISIONS.md | 6843 | The skill-id derivation scheme, and the two easy extraction buckets proven in code |
| D-181 | DECISIONS.md | 6867 | Gate B extraction ships end to end: interpreter, schema v2, `extract`, and 78/81 records reach `imported` |
| D-182 | DECISIONS.md | 6918 | The §6.8 promotion slice: candidates become entities, facts, and grounded skills — deterministic, owner-mediated, one-shot |
| D-183 | DECISIONS.md | 6964 | Two owed Gate B gates ship: §5.2 invariant 4 reachability, and the drain reconciliation wired at the completeness tier, not validity |
| D-184 | DECISIONS.md | 7015 | The Gate B merge review: the catalog check was never wired, and is now the gate D-181 said it was |
| D-185 | DECISIONS.md | 7089 | boardwatch's first promoted revision: the bundle becomes a real résumé source, and Gate B's remaining nine are evidence, not code |
| D-186 | DECISIONS.md | 7146 | Revision 2: the skills surface, D-185's "not reachable" claim is retracted, and the bootstrap draft is a one-time dead end |
| D-187 | DECISIONS.md | 7186 | Projection `skill_groups` are optional and synthesized from the bundle catalog when omitted |
| D-188 | DECISIONS.md | 7225 | An entry's bullets can come from facts, not only claims: `bullet_predicates` |
| D-189 | DECISIONS.md | 7265 | The master is a RESERVOIR sourced from the wiki, and `project.contribution` is widened to owner_attested in Mit's bundle |
| D-190 | DECISIONS.md | 7307 | Content edits are incremental: `edit-fact` files a correction as an edge, and no rebuild is needed |
| D-191 | DECISIONS.md | 7440 | Repository evidence grounds the project bullets, and the verification basis deliberately does not change |
| D-192 | DECISIONS.md | 7511 | `exclude-record` ships, and both documents re-derived from one ledger are guarded |
| D-193 | DECISIONS.md | 7567 | Task 20's matrix is recorded unlabeled, and Stage 2 is blocked by a pinning decision underneath it |
| D-194 | DECISIONS.md | 7645 | `approve_source_scope` binds the spelling already on disk, and the helper is the side that moves |
| D-195 | DECISIONS.md | 7714 | The pinned set is the three fixed jobs, and the one-page ceiling is 16 bullets |
| D-196 | DECISIONS.md | 7770 | Gate B's three undispositioned import records are excluded as `owner_excluded`, 7 blockers → 4 |
| D-197 | DECISIONS.md | 7804 | Task 20's matrix is owner-labeled, unblocking scorer selection (Task 23) |
| D-198 | DECISIONS.md | 7842 | Task 23: `mean_per_bullet` is adopted as the CLI scorer default, threshold stays `Decimal(0)` |
| D-199 | DECISIONS.md | 7904 | `resume project`'s manifest maps bullets by their own id, not by re-parsing the declaration's `claims` |
| D-200 | DECISIONS.md | 7964 | Résumé heading formatting is declaration-driven; clickable project links are an optional code feature |
| D-201 | DECISIONS.md | 8013 | `employment.organization` is owner-attestable; the four org facts are resolved by a scoped owner attestation — Gate B 4 → 0 |
| D-202 | DECISIONS.md | 8060 | The skill-id slug collision (D-184 finding 3) is fixed: promotion refuses a grounded id built from more than one item, rather than silently merging |
| D-203 | DECISIONS.md | 8109 | The other two promotion slug-collision sites (entity_id, category_id) are closed the same way; a fourth (fact_id) is found open, not closed |
| D-204 | DECISIONS.md | 8184 | A missing `pdfinfo` is a run-level fatal, not a laundered `COMPILE_FAILED`; the tool identity travels as typed data |
| D-205 | DECISIONS.md | 8252 | The fourth promotion slug-collision site (`fact_id`) is refused; the guard sits on the derived id, not on each builder |
| D-206 | DECISIONS.md | 8311 | CSV export to stdout is written UTF-8 through a locally-wrapped stream |
| D-207 | DECISIONS.md | 8338 | The `STATE.md` trim executes D-149, and the fact-check that gated it corrects six stale figures |
| D-208 | DECISIONS.md | 8414 | Dates render at month precision, and a projection may declare a two-fact range so an open-ended project is renderable at all |
| D-209 | DECISIONS.md | 8495 | A fact that is simply wrong is retired by flipping its verification state to `rejected`; there is no delete, and `year_month` has no null form |
| D-210 | DECISIONS.md | 8532 | A skill listed under two skill groups is refused, because a skill has exactly one category and arrival order must not pick it |
| D-211 | DECISIONS.md | 8593 | Correction: Windows runs only on the scheduled CI build, and that build has been red since 2026-08-14 |
| D-212 | DECISIONS.md | 8652 | Windows is a best-effort platform, the nightly gets a consumer, and D-211's "not a flake" is corrected |
| D-213 | DECISIONS.md | 8733 | Résumé bullets state what was built with metrics, never a story; and a bullet is parked by surface, not by an extra fact |
| D-214 | DECISIONS.md | 8811 | Hookrail's bullets: a merged perf-plus-chaos claim, a keyword measured back in after a length trim, and a correct-but-unwanted bullet parked |
| D-215 | DECISIONS.md | 8882 | StreakSync ships two bullets; a control test is only evidence about the corpus it ran against, so a historical absence needs the pickaxe; authorship is verified per entity |
| D-216 | DECISIONS.md | 8940 | SAKEC's bullets are ruled and worded but NOT promoted; a private repo makes a disk sweep's negative worthless; and keywords are chosen by diffing the résumé's own Skills section |
| D-217 | DECISIONS.md | 9048 | Crop-RF's numbers all verify against the paper, but its award count, its host and its authorship do not; and `grep` here silently honours `.gitignore` |
| D-218 | DECISIONS.md | 9202 | Nakshatra's bullets are rewritten; both percentages stay, as client-supplied estimates |
| D-219 | DECISIONS.md | 9282 | The one-page budget is a character budget, not a bullet count; D-195's two-candidate ceiling is retired |
| D-220 | DECISIONS.md | 9336 | NIO's bullets drop the SwiftUI and SensorKit-authorship claims and add the VPN lifecycle work; the owner attests SensorKit shipped |
| D-221 | DECISIONS.md | 9427 | Saayam keeps its entry with one role-scoped bullet, because "role + org + dates only" is unrepresentable today |
| D-222 | DECISIONS.md | 9509 | Correction: D-212 marked two of the three tests exercising the Windows stale-lock race, and the third turned the nightly red |
| D-223 | DECISIONS.md | 9571 | Correction: D-222's own census was short one, and instance 4 is marked by mechanism rather than observation |
| D-224 | DECISIONS.md | 9649 | The Windows stale-lock race is fixed by re-asking the OS for a bounded window; the four `xfail` markers come off together |
| D-225 | DECISIONS.md | 9839 | The daily pipeline gets projection behind an opt-in `--project`, fail-closed before any lead earns a disposition |
| D-226 | DECISIONS.md | 10005 | A bullet-less entry is legal only when it is DECLARED; a bullet source that resolves to nothing stays fatal |
| D-227 | DECISIONS.md | 10138 | The scan lock gets the same reclaim window as the bundle lock, and the shared constant moves to `core/lock_reclaim.py` |
| D-228 | DECISIONS.md | 10244 | Fixture drift is three separate gates, and the staleness one enforces a review deadline rather than freshness |
| D-229 | DECISIONS.md | 10358 | P5b's criteria, named by the owner: three clean projected runs over ≥ 30 distinct postings |
| D-230 | DECISIONS.md | 10415 | The backfilled `runs` rows close as `ok`, matched by predicate, and `doctor` asserts the invariant |
| D-231 | DECISIONS.md | 10467 | D-225's residuals: four closed, one declined, one not reproducible |
| D-232 | DECISIONS.md | 10516 | An independent repo audit found a false number on the rendered résumés; P5b's fabrication clause fails, then is repaired |
| D-233 | DECISIONS.md | 10604 | ATS-parsable résumé PDFs: XeTeX common ligatures disabled via `fontspec`, small caps preserved |
| D-234 | DECISIONS.md | 10632 | Opt-in `fill_to_page` second growth phase for projection Stage 2, bypassing `ADMISSION_FLOOR` |
| D-235 | DECISIONS.md | 10657 | Opt-in first-bullet link placement and reverse-chronological project sort |
| D-236 | DECISIONS.md | 10682 | STATE's "next session is the résumé" was stale; the résumé track is done and the linear roadmap is frozen/owner-gated |
| D-237 | DECISIONS.md | 10714 | `_merge_categories` refuses a label that slug-collides with a seeded catalog category; the catalog owns the display_name |
| D-238 | DECISIONS.md | 10755 | A partial extraction sets the record aside whole; the drain fires even when the record produced candidates (D-184 finding 2) |
| D-239 | DECISIONS.md | 10794 | Education Slice C ruled: exclude `header/1` (the professional name) like `header/2`; the name is owner-supplied at onboarding |
| D-240 | DECISIONS.md | 10825 | Mit lifts D-155's freeze on P3/P6/the 14-day clock; the roadmap resumes and its remaining gates are operational, not build |
| D-241 | DECISIONS.md | 10868 | P3 item 8's two-writer test/guard: same-OS test + a runtime refusal of WAL-unsafe filesystems, since the cross-OS config can't run in CI |
| D-242 | DECISIONS.md | 10910 | The unattended daily entrypoint is `boardwatch run`, scheduled by the operator; the daily driver runs live, not against a copy |
| D-243 | DECISIONS.md | 10950 | Verifying the daily-run handoff: D-230 already applied, `doctor` exit is not a valid check, and BOTH résumé sources block a clean run |
| D-244 | DECISIONS.md | 10984 | The daily driver goes live: projection approved, launchd agent installed with an explicit homebrew PATH (D-204), and run 61 is the first clean unattended run |
| D-245 | DECISIONS.md | 11029 | Run 61 ruled day 1 of Gate P3; "run 61" is an invocation counter not 61 days; nothing to capitalize from prior runs; root cause of two off-target leads (role-gate `uncertain` passthrough + no numeric-seniority veto) |
| D-246 | DECISIONS.md | 11089 | Rank-time seniority gate + versioned company-free leveling catalog + explicit `target_seniority_band` (default `any`, gate inert) + guarded bare-`coordinator` deny; the registry company-map option was not buildable, replaced by `{config_dir}/leveling-bindings.yaml` |
| D-247 | DECISIONS.md | 11186 | The four open review findings on the seniority gate closed: `stats` now counts in the ranker's gate ORDER; the inert-gate probe is per-branch case-sensitive AND masks non-seniority phrases (90 live disagreements with the armed gate → 0); `_why_cell` names every drain, not the first; `LevelingCatalog.version` deleted |
| D-248 | DECISIONS.md | 11250 | The launchd calendar trigger has fired ZERO times: the plist was created 39 min after the 08:00 window and run 61 began 70 s later, so it was a kickstart; `runs = 1`. Gate P3 is 1 of 7 by D-245's ruling, 0 of 7 unattended. Kickstart and scheduled paths ARE identical (same plist/argv/env/domain); the gap is an unproven trigger plus no external staleness alarm |
| D-249 | DECISIONS.md | 11289 | Eligibility produced 0 ineligible in 120,330 evaluations because `work_authorization.needs_sponsorship` was unset, so `resolve.py:188` abstained for every `ead_or_similar` holder; setting it turned 1,739 uncertain into 1,650 ineligible + 89, `eligible` unmoved, all 1,692 unmet rows span-backed and every span contains "sponsor". Net pool effect 401, not 1,650. Keystone worked; the dial went unread. STILL OPEN: zero-row `eligible` (41.3%) must abstain |
| D-250 | DECISIONS.md | 11342 | A zero-evidence `eligible` (11,158 of 26,997 = 41.3%; 98% truly thin at median 5,922 chars) now abstains to `uncertain`: `eligible` requires a requirement row OR an ignore/skip-excluded family that would have detected one, so multi-tenant opt-outs stay `eligible` (corpus `p0002` preserved). Label-only — the ranker hides only `ineligible`, so leads are unchanged. The zero-output guard now counts `eligible`+`uncertain` candidate work. Mit chose flip-and-rebaseline; 320 frozen corpus cases flip. Rejected: length guard (98% are long), synthetic abstain rows (corrupts never_fired vs fully_abstaining), keep-and-monitor |
| D-251 | DECISIONS.md | 11392 | Hard location gate becomes US-only (Mit's visa requirement). New `rank/location_gate.classify_location` — a positive US allowlist (states/cities/markers/US-remote), word-bounded, **US-state-signal-before-non-US so "Vienna, VA"/"Athens, GA" are KEPT not dropped** (review caught the reverse order deleting real US postings; fixed pre-merge, 0 US-drops-with-signal); "Bangalore, IN" leaks to us fail-open; tokens versioned in `location_data.py`. `passes_hard_filters` hard mode drops `non_us`, keeps `us`+`unknown` (fail-open, Mit's ruling); `remote_only` veto preserved; `location_fit` stays soft scorer; drops reuse `hidden_hard_filter` (no new bucket). Measured 26,997: US 61% keep, non-US 34% drop, unknown 4.7% keep. Old `location_fit` substring dropped real US ("Boston, MA") and kept non-US remote. Deferred: separate drop count + per-lead unverified flag. Activate: `config set location_filter_mode hard` AFTER merge |
| D-252 | DECISIONS.md | 11441 | Role gate: closed pre-sales/support/BD consistency gaps. Soft-lane denies for "(solutions\|enterprise\|customer\|pre-sales\|sales) architect", "(business\|partner\|account\|…) development (rep\|manager\|…)", "(customer\|technical\|it\|desktop\|helpdesk) support engineer"; "SW Engineer" added to signal. Soft lane never reaches a rescued/signalled software title. Measured 26,997: swe 5,438→5,453 (0 software lost), ~491 pre-sales/support/BD uncertain→not_swe; bare "Support Engineer" kept. Deferred to owner: P1 (hard-exclude all mgrs/directors, ~4,200), Data Scientist/Analyst scope |
| D-253 | DECISIONS.md | 11465 | Abstain report separates STRUCTURALLY undecidable rules (schema has no datum for any profile) from fixable 100%-abstains. Versioned allowlist `experience_years:scoped_years_minimum` + `clearance:clearable_required`; adds `structurally_undecidable` flag + `AbstainReport.structurally_undecidable`/`.fully_abstaining_fixable`; `eligibility abstain` summary now "N fire but never decide · M structurally undecidable". Additive — `fully_abstaining` + all consumers unchanged. Detections kept (prevent eligible-by-silence). Degree in-field rules excluded (profile-dependent, not structural) |
| D-254 | DECISIONS.md | 11485 | The 08:00 launchd trigger FIRED for the first time (2026-08-20 08:00:22, run 63 clean: exit 0, ~50 min, funnel RECONCILES, 8 leads/8 PDFs, all US-located); resolves D-248 — never broken, no prior opportunity. Gate P3 now 1 of 7 UNATTENDED; run also did the owed post-#102 store re-eval. Still open: no external missed-window alarm |
| D-255 | DECISIONS.md | 11501 | Role gate: `_NOENG`-guarded SOFT denies `\b(manager\|director)\b` and `\bdata\s+(scientist\|analyst)\b` (#106). not_swe lane; spares eng titles + Data Engineer/Analytics Engineer. Manager/director half is REDUNDANT for Mit (his exclude_titles vetoes them upstream) but is the correct generic form; the data half helps Mit. TDD, make check 6888 |
| D-256 | DECISIONS.md | 11517 | resolve.py: `any_degree_required` (unleveled `bar is None` had no MET path → degree-holder was UNKNOWN) and `sponsorship_available` (ignored the needs_sponsorship bit) now resolve MET (#107). Keystone preserved (absent field still abstains). Oracle row m0105 corrected + corpus content pin re-recorded (987 rows unchanged). TDD, make check 6878 |
| D-257 | DECISIONS.md | 11530 | Clearance armed as a BLOCKER (Mit: remove clearance-required jobs, not demote; resolves open Q2). `security_clearance={state:none,level:none}` (both required — coherence check) + `policy clearance=blocker`, verified in store. 11 required-clearance patterns → UNMET → ineligible → dropped (~138 rows in run 63). Severity user-owned; other users keep `preference` |
| D-258 | DECISIONS.md | 11542 | Seniority band=`entry` (open Q4) + internships excluded via `exclude_titles` (Intern/Internship/Co-op), NOT the body-only engine. job-apps confirmed intern must be detected BY TITLE; boardwatch's engine reads `posting_versions.body_text` (no title), so `internship_role_declared` is 100%-precision/~27%-recall and already suppresses the "internships count" trap. Both enter profile_row_hash → first-run policy re-key (~11 rows, `ledger reopen --stale`). Arming is Mit's TTY act (`profile edit`), pending |
| D-259 | DECISIONS.md | 11559 | Role gate: SOFT denies for run-63 ranked-pool leaks — strategy & ops, business operations/partner (engineer-guarded), stock plan, pricing (#108). Top 40 was ~28% non-software. Rescued/signalled software never reaches SOFT. TDD, make check 6906. Deferred to owner: Team Leader, Data Center Engineer, bare Administrator; NOT excluding "User Researcher" (overlaps ML/Research Engineer) |
| D-260 | DECISIONS.md | 11577 | Dead-man's-switch heartbeat (#110): a successful `run` GETs env-only `BOARDWATCH_HEARTBEAT_URL`, so an external cron-monitor alerts when a scheduled run never happens (Mac off/asleep all day — the one failure a local check can't see). Fires ONLY on success (gated `summary.fatal is None`), presence-gated/off by default, follows redirects, swallows all errors (D-076). Standalone `send_heartbeat()` in `notify/heartbeat.py`, not a Channel. Rejected: on-machine watchdog (shares the machine's fate), config field (it is a secret). TDD |
| D-261 | DECISIONS.md | 11597 | Whole-branch review of the precision merges found 3 false-drops. HIGH #1 (#111): location gate dropped US jobs whose one string mixed bare `US` + a foreign place (`"US, Canada"`→non_us); fixed by resolving the US signal before foreign tokens within a segment. HIGH #2 (#112 + config): seniority gate matched manager/lead/director with no domain guard (`"…, Password Manager"`→above_band); now an ambiguous mgmt word raises band only when a role token shares its comma-clause, and `exclude_titles` refined (bare Lead/Manager/Director → specific phrases). MEDIUM #3 deferred: zero-output guard can false-alarm on a real zero-lead day (safe-direction). TDD |
| D-262 | DECISIONS.md | 11627 | A manual verification run (run 65, NOT a P3 tick) exposed a "Lead" role-gate hole: D-261's exclude_titles refine removed bare `Lead`, but unlike Manager/Director (compensated by the role gate's `_NOENG` deny, D-255) `Lead` was left ungated, so 5/8 run-65 leads were non-SWE business/ops "Lead" roles crowding real SWE out under the cap. Fix (#114): `_NOENG + \b(manager\|director\|lead)\b` — mirror of manager/director; `_NOENG` spares eng titles (Lead Engineer). TDD, make check 6943, merged after one tectonic-flake 3.11 rerun (3.12/3.13 green on same commit). Rejected: re-adding bare `Lead` to exclude_titles (reintroduces the #112 product-noun over-veto). Deferred to owner: GE HealthCare Buc/France leak (US-only gate fail-opens on an unrecognized non-US city) |
| D-263 | DECISIONS.md | 11657 | `_alternation` built `(?<![a-z])a|b|c(?![a-z])` — ungrouped, so `|` binding looser than concatenation left the lookbehind on the FIRST token and the lookahead on the LAST, and every token between matched as a bare substring. `uk` fired inside `Waukesha`/`West Milwaukee`: 41 real GE HealthCare Wisconsin postings (incl. `Software Engineer`) dropped by the US-only gate. INTERMITTENT — `frozenset` order under hash randomisation, so 43 postings' drop decision differed between PYTHONHASHSEED 0 and 4. Fixed with `(?:...)`; `milano`/`deutschland`/`russian federation` added because the substring accident was the only thing dropping them. Seed-independent invariant test across all 7 catalogs, RED-confirmed on a src COPY (14 failures) |
| D-264 | DECISIONS.md | 11692 | Buc/France leak closed (D-262's deferral) by THREE measured non-US signals, each firing on 0 US-classified postings over 28,287 open: (A) 57 curated foreign city tokens + Saxony/Thuringia, `LOCATION_DATA_VERSION` 1→2; (B) structural ISO alpha-3 code (`BGR-Varna`, `Remote (IND)`) — alpha-3 ONLY, since 2-letter collides with 51 US state abbrevs and `IT -`/`SE -`; (C) new `rank/foreign_ad_gate` reading DACH `(m/w/d)` / French `(H/F)` / `Ingénieur` off the TITLE, the only signal reaching 3 postings whose locations_json is exactly `["Remote"]`. Net 299 drops / 162 ranked survivors / 36 US false drops recovered / 280 of 444 `unknown` still passing. Dublin + 10 US-namesake names left leaking BY RULING; rejected list in the docstring. No new drop bucket (folds into hidden_hard_filter) |
| D-265 | DECISIONS.md | 11741 | The funnel's `hidden_hard_filter` note claimed hard mode "has never been measured firing" while printing 17,189 drops from it in run 66 (mode set since D-251, firing confirmed in run 63). Corrected and extended to name the second axis (a non-US ad convention on the title). "(not the default)" kept — soft IS the default for other users. NOT done: naming the mode actually active on the run, which would need it threaded into `Drop` |
| D-266 | DECISIONS.md | 11757 | `digest_of_sources` hashed `ast.dump`, whose output is interpreter-dependent (3.13 omits default-valued fields; `type_params` absent before 3.12), so byte-identical source digested `6f9feb84bfee`/`a1d0be72a338`/`7e88ed2b193d` on 3.11/3.12/3.13. Since `engine_version` keys the ledger, a Python upgrade silently re-evaluated ~29,000 postings and each CI interpreter computed a different version. New `canonical_dump` walks `_fields` directly: empty lists skipped (absorbs a future grammar field), `None` KEPT so `Constant(None)` stays distinguishable. Verified `63c6f8fd5a3e` on all three. Rejected: raw bytes (re-keys on comment edits, the thing D-P2-22 avoided), `ast.unparse` (own version drift). ONE-TIME re-key: first run after merge re-evaluates the corpus |
| D-267 | DECISIONS.md | 11792 | RETRACTS the run-66 claim "`Buc` still appeared 4x in the ranked pool". `grep -ic buc` returns 4 on runs 61/63/65/66 and 2 on older artifact_versions REGARDLESS of Buc — it counts "bucket" in the funnel's own stage prose; run 65 reads 4 while its shortlist genuinely carried the Buc/France lead, and run 67 reads 6 with no Buc at all. `\bBuc\b` is 0 in every funnel. Structural: the funnel enumerates NO ranked pool (only `leads` + aggregate counters), and a `leads` row carries NO location (`posting_id,title,company,provider,board_slug,company_source,out_dir,pdf_built`). Answered instead by the production predicate `passes_hard_filters` against the store: 0 of 62 Buc/Ingenieur open postings survive, incl. posting 31365 (**that denominator was itself unpinned — corrected by D-268**). Rejected: grepping `Ing` (same mistake one layer along). OPEN FOR OWNER: the hard location gate leaves no trace in its own artifact, so every "all leads US-located" claim is a by-hand store read; closing it needs `locations` on `Lead` + an artifact_version bump. Gate tests mutation-checked and NOT vacuous — dropping only `buc` fails exactly 1 named test; cities/ISO3/ad-gate mutants fail 37/11/12 |
| D-268 | DECISIONS.md | 11843 | Run 68's 16 reopened decisions pre-verified through the production gate chain (`passes_hard_filters`->`role_verdict`->`seniority_verdict`, real profile/catalog/`location_filter_mode=hard`) BEFORE the tick: all SIX known leaks blocked - 5 in the role gate (Affirm TAM x2, Airbnb Programs Ops, Instacart Insights x2, D-262's `_NOENG+lead`) and posting 31365 in the hard filter (`non_us`, D-264) - and the 10 survivors are exactly the SWE set Mit accepted. CORRECTS D-267's "0 of 62": the 0 is robust (27 word-boundary open / 27 all-status / 69 open with ad-gate signals / 70 all -> 0 every time) but 62 is unreproducible because the match rule and corpus snapshot were never recorded (D-264 measured 28,287 open; now 30,243). Bare substring gives 103 matched / 39 SURVIVING (US Starbucks postings), so the match rule is load-bearing - the same error family D-267 retracted was inside its own replacement's denominator. Rule adopted: a ratio records its match rule AND corpus size. Reader route: `sqlite3` CLI `?mode=ro` fails CANTOPEN(14) on a cleanly-checkpointed store (no `-shm`, not the sandbox); `?immutable=1` works but is STALE against a live writer; Python `sqlite3` `?mode=ro` works in both states and is the only valid mid-run probe. Raw-SQL 22 vs tool 16 explained: `reopened_at` is set not deleted (16 on 08-21 + 6 on 08-20); `ledger show --stale` now clean. Rejected: reading the run log to decide if the schedule fired (only `launchctl runs` is authoritative); building `locations` onto `Lead` unilaterally (shipped-schema, Mit's); deleting a backup or `spike/p0-5-resume` |
| D-269 | DECISIONS.md | 11901 | The nightly had failed 7 of its last 8 SCHEDULED runs; ubuntu always passes, so every cause lived in the schedule-only jobs and `make check` stayed green locally. THREE causes, from the job logs not the carried "two flakes" description. (1) PRODUCTION DEFECT: `get_engine` is lazy and `ensure_schema` runs alembic through an engine ALEMBIC builds from the URL, so the pragma listener never fires and a store is CREATED in `delete` mode; the deferred switch to WAL is a CONVERSION, which no other connection's lock permits - measured: raises after the full busy timeout vs a reader, INSTANTLY vs a writer (busy handler never invoked). Two processes opening a fresh store race and the loser cannot open it. Fix: one instrumented connection before alembic. Mit's live store already reads `wal`, so nothing needs migrating. FALSIFIED first hypothesis (recorded because it was the obvious one): that `busy_timeout` ran after `journal_mode=WAL` - the test written to prove it PASSED unfixed, because pysqlite arms timeout=5.0 at connect. (2) Five Windows `fs_safety` failures, deterministic on all 3 Windows jobs: `os.path.realpath` rewrites "/data" to "\\data" (proved via `ntpath`), so all cases collapse onto the root mount - the None-expecting ones PASS VACUOUSLY. Production unaffected (no /proc on Windows). Fix: skip the 8 POSIX-addressed cases on win32, keep the 3 platform-independent ones; verified 3 passed / 8 skipped. (3) tectonic: warmup compiled a minimal `article`, and actions/cache only SAVES ON A MISS, so the partial bundle was frozen forever and every run fetched the template's real packages over the network - one hiccup cost ~52 render tests. Fix: derive the warmup from the template's placeholder-free preamble, assert fontspec survived, bump the cache key. make check 7061 passed. Windows/macOS evidence comes ONLY from workflow_dispatch + the nightly, never a PR. Rejected: re-running any of them as a flake (7/8 is not intermittency); admin bypass |
| D-270 | DECISIONS.md | 11972 | RETRACTS the caveat that boardwatch's 2,300-4,300 postings-first-seen-per-day is inflated by re-keyed duplicates. Measured with match rule AND corpus size (D-268's rule) over 34,367 postings / 30,243 open: of the 2,323 rows first seen 2026-08-21, only 176 (7.6%) share a `company_title_location` identity with a strictly earlier posting, 182 (7.8%) a body hash, 37 (1.6%) the strict `exact_quad` - ~92% are new to the store. ROOT CAUSE: `detail_fetch_budget` (default 50) caps UNSEEN postings materialised per board per run, so the daily count measures our throughput, not the market. Conclusive by arithmetic: 19 Workday boards hold exactly 600 rows and each gained exactly 100 on a 2-run day (66,67) and 150 on a 3-run day (63,64,65) = 50 x runs, identical across unrelated companies; run 67 carve reconciles to postings_seen 7,762 (44 complete=6,762 + 20 partial=1,000 + 59 unchanged + 12 failed). FINDING: **15,535 listed postings never materialised** across those 20 boards (Citi 1,614 ... Fidelity 104), recorded ONLY as English in `board_scans.error` and absent from the funnel - the D-267 shape again. Also 12 boards fail outright (Workday 401/403/422) and 17 companies have never produced a posting. `jobs`==`postings` is POLICY: only `exact_quad` merges (96 events, p6.2); `posting_identities` already holds 1,118 body-hash / 1,062 company-title-location / 285 quad keys whose postings sit in DIFFERENT jobs, so Gate P6's leakage clause is a query, not new code. Verified NOT a defect: budget-skipped postings are not falsely closed (`listed_ids` computed before the detail phase; `_process_missing` runs only on `complete`; 0 closures on the 20 partial boards). Rejected: raising the budget mid-session (adds input, breadth-is-last, 42m41s run); merging the looser keys (changes which posting is canonical); repairing the 12 boards in a docs session; rewriting the funnel dedup note (the STAGE really does count nothing). Owner-gated: the budget value, a first-class backlog counter, the 12 dead boards |
| D-271 | DECISIONS.md | 12036 | **Measured the job-apps replacement question and REOPENS D-008** (comparative parity is listed under *Deliberately NOT doing*). Match rule + corpus size stated (D-268): 530 job-apps eligible records 08-12..08-21 with a company, normalised company name vs 135 watched boards (159 keys) - **41 (7.7%) at a company boardwatch watches; 352 distinct companies, 24 watched**. Top missing: Amazon 25, TikTok 20, AWS 8, Apple 7, ByteDance 7, SpaceX 6 - **none uses the 6 supported ATS**, so a slug cannot reach them. Lane value measured by LOSS-IF-REMOVED, not frequency: commercial aggregators 421 of 446, GitHub new-grad lists **73 of 103 (19.1% of yield for ~5 public-repo GETs)**, direct ATS 5 of 14; cross-lane overlap only 5.8%, so lanes are non-redundant. Counterweight: ~1/5 of job-apps' yield is staffing firms (Infosys/TCS/BeaconFire/Capgemini/UST) and artifacts (`Jobright.ai`, `Stealth Startup`, `↳`). WHERE BOARDWATCH IS BETTER: gh/lever/ashby both un-paginated but job-apps title-filters INSIDE the fetch loop so we hold more; its Workday lane is 3 hardcoded queries x 2 pages x 12 details = 77 roles over 39 boards, 16 returning zero. **NEW DEFECT: Workday `total` is censored at 2000; facet `values[].count` is NOT** - Citi real board **4,589** (we hold 600 = 13.1%), NVIDIA 2,656 (22.6%); known-positive control PASSED (Adobe 740/740, Intel 645/645, Regeneron 592/592, Fidelity 565/565 agree exactly). Our pager also wraps at ~2,000, so post-drain Citi stays ~2,214 of 4,589 - a permanent invisible hole that is NOT the budget. Mirror defect: Regeneron 101.4% / Fidelity 106.2% coverage - permanently `partial` boards never run `_process_missing`, so dead rows accumulate. CONFIRMS D-270 against a contrary claim: every board's backlog falls monotonically 26-49/scan (15 drained); the 'newest-first prefix, tails never read' hypothesis was FALSIFIED by `posted_at` (Databricks 2019-11, Cisco 2025-12, Adobe 2026-03 reach back; Citi is an outlier). Backlog ETA **48 runs (~7 weeks at 1/day)**. Also: 5 boards report GREEN and return ZERO ever (Snyk, Vercel, HubSpot, Plaid, Qualcomm) - the dangerous class, since a loud failure gets fixed; 7 of the 12 dead boards are HTTP 422 = malformed request = probably wrong slugs, i.e. recoverable; `unchanged` (59 of 135) has NO test that a payload hash can misreport; `hidden_hard_filter` (17,891 = 59% of corpus) is the only bucket with NO `--include-` drain. THE INSTRUMENT IS A PERSISTENCE PROBLEM: every provider already enumerates fully and parses a total (`total`/`totalFound`/`meta.total` - the last confirmed live on the CHEAP `_health_url` `probe_health` already fetches: stripe 576, databricks 818); none reaches the DB. Design: `docs/superpowers/specs/2026-08-22-coverage-assurance-design.md`. Rejected: implementing anything this session (owner AFK, Track 5 reverses a shipped decision); LinkedIn/Indeed (the real ToS treadmill the v2 decision was written about). Owner-gated: reopening D-008, Track 5 scope, `DEFAULT_TOP_N`, the funnel `artifact_version` bump, and whether breadth-is-last still binds |
| D-272 | DECISIONS.md | 12153 | **Mit's ruling on D-271. Reverses two shipped decisions.** IN: three discovery lanes in order — GitHub new-grad lists (Simplify/jobright/vanshb03/speedyapply), hiring.cafe, then LinkedIn+Indeed via JobSpy. OUT: bespoke first-party adapters (Amazon/Apple/TikTok). `DEFAULT_TOP_N` **8 → 40**. Evidence against the v2 §4.1 "ToS + maintenance trap" exclusion: measured over job-apps' whole ledger, linkedin 22,016 obs / 22 of 23 days / **14 of last 14**, indeed 15,189 / 22 / 14, hiringcafe 4,318 / 22 / 14, jobright 2,865 / 22 / 14 — no gaps, no bans. **PREMISE CORRECTED: the history is 23 days, not months** (ledger starts 2026-07-30), which bounds the confidence. The treadmill IS real but killed the OPPOSITE class — dead are `hn`/`smartrecruiters_api`/`amazon_api`/`jobicy`/`himalayas`/`oracle_hcm_api`/`workable_api`, plus Handshake (HTTP 403, zero rows ever) and HigherEdJobs (Incapsula); **every casualty is a bespoke adapter or niche API, not one is an aggregator** — hence rejecting first-party adapters (`amazon_api` ran 2 days for 19 obs while Amazon still reaches eligible 33x via LinkedIn/jobright). Lane quality by job-apps' own `materialization_state` reproduces the ordering: simplify 75.8% / speedyapply 63.2% / hiringcafe 57.6% / vanshb03 57.1% / jobright 36.6% / linkedin 28.3% / indeed 19.8%. **BLOCKER attached: the engine is BODY-ONLY** (`preflight.py` passes `posting_versions.body_text` alone), a stub is a whitespace-only body, and D-250 abstains zero-evidence to `uncertain` — so a lane shipped without JD-body acquisition adds corpus and **zero leads**. This MEETS the deferral condition `PROGRAM.md` §4 set for the 2,200-line JD chain ("P7 where a non-API source might first appear"); 3 rows struck there. boardwatch needs far less: P7's already-required dereferencing step IS the body fix, since an aggregator link mostly resolves to an ATS board whose parser exists. Track 1 (coverage instrument) ships FIRST — a banned lane currently dies silently, and P7 judges a source by leads over ≥3 runs. Rejected: bespoke adapters; a lane before the instrument; a lane without dereferencing. Owner-gated still: the `artifact_version` bump, `detail_fetch_budget`, the 17 silent boards |
| D-273 | DECISIONS.md | 12230 | **The coverage instrument SHIPPED (#124 + #125), and `main` is DELIBERATELY ahead of the checkout that runs.** `make check` 7,153 passed, full CI green. Seven-way partition (`measured`/`enumerated_only`/`censored`/`dark`/`stale`/**`unscanned`**/**`unreadable`**) — the last two added because an INNER JOIN was dropping watched boards with no scan row (routinely reachable: `scan --company X` mints a run containing only X) and because one unclassifiable row took down the whole report. Four nullable `board_scans` columns, **zero extra HTTP**. Measured on a store copy after a real 135-board scan: **82.7%, 26,183 of 31,643**, buckets summing to exactly 135; **Target's real board is 12,097 against 649 held (5.4%)** — Workday censors `total` at 2,000. **THE OPERATIONAL FACT: merging armed nothing.** The launchd venv is an EDITABLE install resolving to the primary tree's `src/`, so 08:00 runs what is CHECKED OUT, not what is merged; that tree is pinned at `151e7c4` and `git pull` arms it (untracked `ARMING-NOTE.md` says so). Ruled: pin it — never exercised end to end at 40 leads, and a scheduled tick must not be a change's first live exercise with Gate P3 at 2 of 7. Also: `rank_open_postings` keeps its own `limit=10`, so the cap moves `run --project` only, not interactive `top`. Whole-branch review found 7 Important the task reviews structurally could not: **`board_enumerated` meant THREE things across six providers** (a PERSISTED column — fixed before any scheduled write; all six now call one shared `count_listed_ids`), a known-positive control that **returned before the code it controlled**, `_uncapped_total` raising on six malformed shapes, one bad total killing the whole report, a censor value persisted as a recovered total, `censored` withholding shortfall (now published; **ratio stays None** — owner's call), and a schema guard catching ABSENT but not STALE. Every fix mutation-verified. **LIMITATION: nothing reads the columns unattended** — a scheduled run persists coverage and reports nothing until the funnel gains a `board_coverage` section (`artifact_version` bump, Mit's). Rejected: pulling the checkout; a CheckConstraint on `board_total_censored` (cannot protect direct-SQL writes, so the typed `UnknownCensorFlag` + degrade-one-board is the load-bearing half); renaming the `coverage` verb. Owner-gated: the bump, arming, the censored ratio, `detail_fetch_budget`, the 17 silent boards, and sub-projects 3-4 (JD acquisition, then the three approved lanes) — neither started |
| D-274 | DECISIONS.md | 12361 | **The coverage instrument now REPORTS itself — D-273's mute-instrument gap closed.** One load in `runner.py`'s `finally`, the same `CoverageReport` object handed to BOTH emitters: funnel gains a `board_coverage` key after `scan` plus a `## Board coverage` section (**`ARTIFACT_VERSION` 5 -> 6**), morning gains a `## Discovery reach` block (**1 -> 2**), and `run --project` prints one `board coverage ->` line into `~/Library/Logs/boardwatch-run.log`. Single load is LOAD-BEARING: `held` comes from `postings` with NO run dimension, so two loads seconds apart can differ and one run's two artifacts must not disagree; both carry a `note` saying stated totals are this run's while `held` is as of write time, so the section is NOT a frozen record. Scoped to `run_id=summary.run_id` explicitly (the query defaults to the newest scanned run), so runs 67-and-earlier report honestly instead of borrowing 68's numbers - verified, run 67's four columns are 135 NULL rows. `boardwatch coverage --json` refactored onto the SAME serializer, deleting the drift class instead of asserting against it. **A coverage failure costs the SECTION, never the artifact**: both emitters swallow-and-write-nothing, so `_load_board_coverage` guards the whole-SELECT fault (a pre-columns store raises `OperationalError`) that D-273's degrade-one-ROW path cannot reach; the seam test caught a real defect while being written - `console` is not module-level in `runner.py`, so the guard's own warning raised `NameError` inside its handler. Measured through the shipped read path on a scratch store seeded from live values (1.2 GB copy REJECTED at 97% disk): run 68 = **82.4%, 26,075 of 31,629**, buckets 90/11/4/12/18/0/0 = 135 - which proves the RENDER path agrees with the CLI path, NOT that the number is independently corroborated (the scratch store was seeded from the very columns it rolls up, and D-273's 82.7% is a different number from a different moment); **4 censored boards are short 18,927 postings**; 1 measured board states total 0 and is excluded, not folded. DELIBERATELY NOT DONE: a ratio for `censored` - `BoardCoverage.__post_init__` ENFORCES `ratio is None` there and that guard IS the unfailable-ratio mechanism, so editing it would pre-empt the owner's open question; the absolute shortfall is published instead. CORRECTS D-273: **`notify` was never a candidate surface** - standalone CLI command, `runner.py` imports nothing from it, launchd runs only `run --project`. Rejected: a separate `coverage-N` artifact (third file to open); funnel-only (leaves the opened file mute); morning-only (nowhere for the 135-row table); `ruff format` (repo-wide unformatted, ungated). Named `board_coverage`/`Discovery reach`, never `coverage` - that key is resume KEYWORD coverage and every lead already prints a `coverage:` line. Owner-gated: the censored ratio, **merging AND arming** (editable venv -> `git pull` in the primary tree arms it, merging arms nothing), `detail_fetch_budget`, the 17 silent boards, sub-projects 3-4 |
| D-275 | DECISIONS.md | 12450 | **CORRECTS D-272's JD-acquisition plan: "dereference, then reuse the provider parser" is the WRONG SHAPE for 4 of 6 providers.** Read-only scoping, nothing built. CONFIRMED half: the parsers ARE separable - module-level pure functions the tests already call directly (`greenhouse.parse_job`, `lever.parse_posting`, `ashby.parse_job`, `workable.parse_job`, `smartrecruiters.parse_posting`, `workday.parse_posting`); `ashby.parse_payload` is the one bytes->BoardSnapshot seam and the pattern to copy. FALSE half: **there is nothing to fetch.** The `Provider` protocol declares only `board_url` / `fetch_board` / `healthcheck` - NO `fetch_posting` - and `_detail_url` exists for **SmartRecruiters and Workday ONLY**, because the other four board endpoints already inline every body (verified by reading the URLs: greenhouse `?content=true`, lever `?mode=json`, ashby `?includeCompensation=true`, workable `?details=true`). So for those four the body problem is **COMPANY DISCOVERY, not dereferencing** - `parse_board_target` -> `upsert_watch` -> `run_scan` already buys the whole board for ZERO new code, while dereferencing means a new URL builder against an API never called here, each owing a drift-fingerprinted fixture. **BIGGEST OBSTACLE, exactly where the plan cannot route around it: nothing extracts a posting id from a URL.** `parse_board_target` returns `(provider, slug)` and discards the rest; SmartRecruiters needs a `posting_id`, **Workday needs an `externalPath` path-string, not an id**, and the public->CXS mapping is verified NOWHERE. Secondary: `FetchResult` exposes no final URL and no redirect history despite `follow_redirects=True`, so the fetcher cannot say where a link landed; `count_stub_postings` is CORPUS-WIDE with no per-source split and `SourceOutcome` has no stubs field, so a lane's stub rate moves the global number (matters - P7 judges a source by leads over >=3 runs); per-host delay is 1.0s. SHARP EDGE: `apply_board` with `status='complete'` runs `_process_missing` and CLOSES every open posting of that company absent from the snapshot, so a lane must emit `partial` or own its own company rows; `postings.job_id` is nullable but a TRIGGER aborts on NULL; skipping `_write_posting_identity` disables duplicate suppression corpus-wide. ARGUMENT FOR dereferencing where possible: resolving to the real `(provider, slug, provider_posting_id)` makes `UNIQUE(company_id, provider_posting_id)` CONVERGE the aggregator posting with the later board scan instead of duplicating it. Prior art not to rebuild: `parse_board_target`, `classify_host`, `normalize_url`, and `pipeline/liveness.py:build_prober` (the only existing GET-one-posting path). No decision taken; sub-project 3 still needs its own brainstorm |
| D-276 | DECISIONS.md | 12522 | **Gate P3 RESET to 0 of 7 — it does NOT "stay at 2".** Counting correction to D-273 and every post-2026-08-22 STATE line. `PROGRAM.md`'s gate reads "7 **consecutive** unattended runs"; run 68's 08:00 tick exited 1 (`CommandError: Can't locate revision identified by 'p_board_coverage'`), and a failed unattended run breaks consecutiveness, so the streak went to ZERO. Evidence from the only authority: `runs = 4`, `last exit code = 1`, plus the traceback in `~/Library/Logs/boardwatch-run.log`; invocations 2 and 3 were the clean ticks (runs 63, 66), invocation 4 died. **CONSEQUENCE: the next clean tick is 1 of 7, not 3 of 7 — seven more clean days, not five**, and Gate P4 (the owner's blind craft review) is barred until P3 is met so its date moves too. Rejected: weakening "consecutive" to a tally so the earlier number stays true - consecutive is what makes the gate a claim about OPERATIONAL STABILITY rather than an accumulated count of good days, and a crashing run is exactly the instability it detects. Unchanged and still correct: a manual `run --project` contributes nothing (runs 65, 67, 68-rerun, 69 all move the counter not at all) |
| D-277 | DECISIONS.md | 12555 | **The `hidden_hard_filter` bucket gets a drain (`top --include-hard-filter`), closing the last bucket with no `--include-` flag — 17,891 drops on run 67, 59% of the corpus.** The blocker was not the flag: `passes_hard_filters` returned a bare `bool`, discarding which clause fired and which token matched, so no consumer COULD report it. Adds `hard_filter_verdict` returning a frozen `HardFilterVeto` (closed-catalog clause + deciding text) with the bool reduced to a wrapper; rejected changing the bool's own signature (28 predicate assertions, zero benefit; `notify`/`stats`/`count_filter_matches` untouched). **FALSIFIES the 16-day-old 'all exclude_titles' claim: the bucket is FOUR clauses** — `exclude_titles` always, plus remote-only, the hard US location gate (D-251) and the foreign-ad title marker (D-264) in `hard` mode; the 100%/0% split was measured when the mode was `soft` and two clauses did not exist. D-265 fixed the identical stale sentence in the funnel note and left it in `top_cmd.py`'s field comment; now corrected. Owner rulings: the drain is **UNBOUNDED** by the rank cutoff (a bounded drain cannot audit the 59% that never nears it), and `_shortlist_line` now names the bucket so `hidden_hard_filter` LEAVES `SUMMARY_LINE_EXEMPT`. **No new bucket** — the clause rides on the ROW via `RankedPosting` twin fields, so none of the >=27 mirror sites move (D-251/D-264 reasoning). Drained row `continue`s BEFORE `surfaced_job_ids` (D-110); `show <id>` gains a `Hard filter:` line; `run --project` unchanged. Method note: the identity test asserted EIGHT terms and the real identity has TEN — an eight-term sum passes vacuously whenever handled/applied are 0, which is what `test_top_accounting::_accounted` does today |
| D-278 | DECISIONS.md | 12607 | **JD-body acquisition: owner rulings on the D-272/D-275 blocker. Design only, nothing built** (`docs/superpowers/specs/2026-08-22-jd-acquisition-design.md`). RULING 1: the lanes exist to reach companies **no existing route can reach**. RULING 2: **the lane order REVERSES D-272** — Indeed (JobSpy) first because its body arrives FREE inside the search response (`description { html }` is a field of the search query, zero extra requests), then hiring.cafe (one unauthenticated GET, verified 3,463 chars), then the GitHub lists, which carry **no body at all** and whose active entries are **53.3% duplicates** of the six boards already scanned against 34.7% serving Ruling 1. RULING 3: adding a whole board **IS breadth** — permitted only under a per-run new-company cap, reported (unbounded, one Simplify pull adds 5,695 companies). RULING 4: honest UA stays on the six providers, browser UA only on new aggregator fetches — re-ruled after the first framing omitted that boardwatch ALREADY ships an honest UA and is a published package. PREMISE CHECK (37,442 rows / 9 lists, netloc rule + sample size stated): 4 of 5 named companies present (TikTok 804, ByteDance 312, Amazon 227, Apple 171) but **SpaceX is on Greenhouse** so D-271's five is a four; **half the non-six tail is three more ATS families** (Oracle Cloud HCM 31.3%, iCIMS 13.6%, eightfold), and the largest non-six company is Sainsbury's at 1,639 postings, a UK grocer the US gate discards. PRIOR ART: job-apps' browser tier is **13% historically and 0% currently** — dead since 2026-08-11, 11 consecutive runs at zero, invisible because `except Exception: return ""` makes a missing dependency, a timeout and an empty page one empty string (corroborated by unchanged candidate mix 185->146 OK vs 53->0, and wall clock 13m58s->1m40s->28s); LinkedIn plain HTTP carries **60.6% at a 97.7% hit rate**. **It has NO generic careers-page extraction** — 14 hand-maintained host regexes gate every fetch, Apple and TikTok have none, Amazon has a bespoke adapter of the rotting class (zapply silently 585-600/day -> 0 on 08-19). LATENT DEFECT: `content_hash("")` == `content_hash(whitespace)` == `e3b0c442...`, feeding `exact_quad`, the only SUPPRESSING identity kind, and `_verify_quad` agrees because `"" == ""` — live store has 13 such postings and **0 colliding groups**, so latent not firing; fix before any lane lands a row. Lane mechanics: **not a 7th Provider** (set-equality test + fixture rule R13), must emit `partial` (an EMPTY `complete` is legal and marks every open posting missing), must insert `jobs` first (ABORT trigger), must call `_write_posting_identity` (skipping one posting disables dedup CORPUS-WIDE). Corrects D-275 (`ashby.parse_payload` is a METHOD; `smartrecruiters.parse_posting` is called by no test; greenhouse is `?content=true&pay_transparency=true`) and D-272's source list (two repos now redirect to their 2027 successors; jobright is 100% tracker with no JSON; Simplify is 86.8% direct ATS, 1 of 18,823 a tracker). Rejected: a generic careers-page reader, bespoke adapters, browser automation, **trusting hiring.cafe's 90 pre-extracted eligibility fields** (they cite no span from the frozen JD), DuckDuckGo fan-out. Also required with lane 1: per-source stub attribution. Owner-gated: Oracle/iCIMS as PROVIDERS, the cap's number, and whether LinkedIn earns its O(n) cost |
| D-279 | DECISIONS.md | 12716 | **The body-less suppression is FIXED on both paths (#132), and lane 1's client REVERSES D-278's "via JobSpy".** The defect is now DEMONSTRATED, not argued: two body-less postings at one company sharing a normalized title and locations produced `Suppression(posting_id=2, survivor_posting_id=1, kind='exact_quad')` — a real posting suppressed on no evidence. `body_evidence()` mirrors `normalized_locations` (absence => None => no `exact_quad`), AND `_verify_quad` now requires body **PRESENCE** not equality, because that resolver groups STORED identities that nothing rewrites on its own schedule and two absent bodies compare EQUAL — the same belt-and-braces D-083 applied to locations. Its docstring had asserted the defect away ("NOT NULL ... so there is no missing-body branch"); NOT NULL is not non-empty. **Rejected: an IDENTITY_ALGORITHM_VERSION bump** (no normalizer, key tuple or host-class table changed; a bump would take suppression and every `unique` to None across 32,229 postings for nothing — the presence check neutralises the stale rows and `write_identities` DROPS each as an unwanted kind on the next write, so they self-heal). **Rejected: withholding `content_hash_only`** (annotate-only, and keeping it is what keeps `identities_complete()` true — a posting emitting NOTHING would disable suppression corpus-wide, the very failure being fixed). **JobSpy is UNINSTALLABLE here: `python-jobspy` 1.1.82 pins `NUMPY==1.26.3`, newest wheel `cp312`**, against `requires-python >=3.11`, CI on 3.13 and a 3.13.12 dev venv — it would break the published package for 3.13 users, and its own `requests`/`tls-client` stack is unreachable by the per-host politeness lock §4.7 rules must stay boardwatch's. Assumption (offered to Mit, no answer in window, revisable): use the `httpx`/`Fetcher` already shipped; every D-278 ruling survives since the body is still free in the search response and only the CLIENT changes. Second assumption: **the per-run new-company cap is 10**. **Run 68's exit 1 has a ROOT CAUSE and it is process, not code**: the live store was stamped `p_board_coverage` by a run whose tree had the migration, while the file did not reach the primary checkout until **08:17** — 17 min AFTER the 08:00 tick ran `main`'s tree. Repaired (both heads agree; run 69 clean at 11:47). The violated rule is the standing one: **an agent must be FORCED onto a scratch `BOARDWATCH_DATA_DIR`, because the live store is the DEFAULT and "don't touch it" fails open** — and a migration is worse than a write, since it breaks the NEXT scheduled run, not the one that erred. Cost a second Gate P3 day. **Phase 1 planned with NO network code** (`docs/superpowers/plans/2026-08-22-lane-groundwork.md`): the ten-outcome catalog with a counter each and `is_silent_outage`, the `Lane` protocol whose `lane_snapshot()` makes `status="complete"` UNEXPRESSIBLE, per-source stub attribution, the company cap. The Indeed client is deferred to its own plan whose FIRST step pins the GraphQL document and headers against the live endpoint — this session did not verify that contract, and transcribing one from a summary would put fiction into a no-placeholder plan. Owed with the client, not phase 1: §4.5's four quality controls, §4.7's browser-UA `Fetcher`, and a drain for any stub bucket the lane creates |
| D-280 | DECISIONS.md | 12789 | The finish line becomes a provisional pass (3 frozen B1–B7 runs) + a 14-day background confirm; breadth (3 lanes) pulled into scope NOW ahead of acceptance; P6 dedup ruling = `exact_quad` only; sessionized parts 1–6, Sonnet-default/Opus-for-hard. P3 is human-adjudicated off launchd's counter so it compresses; the 14-day acceptance is the true wall (B1 net-new/day can't be faked). No code, no gate moved |
| D-281 | DECISIONS.md | 12831 | **Two D-280 premises measured FALSE.** `built` is permanent, so each run retires its shortlist for good: (1) breadth does NOT protect B1 — `capped_by_top_n` is **3,683**, ~368 days at ≥10/day, and GROWING (+126, +55 after 40 consumed); (2) intraday runs yield FULL net-new — run 69 got **40 of 40 net-new 3h after run 68**, lead sets **100% disjoint**. Mit's rulings: **lane order STANDS** (Parts 2–4 before the freeze; parity/company-reach is the surviving justification, not B1) and **COMPRESS EVERYTHING** (~3h cadence; first 7 clean ticks close P3, 3 of them are the provisional pass; ~1–2 days not ~10). Cost recorded: B1/B5 are per-DAY, so the provisional pass certifies per-RUN behaviour and the 14-day confirm is the control. Caveat on the bar: a 14-day B1 pass does NOT evidence discovery health — it is near-guaranteed for ~92 runs by ledger drain; the real threat is a **ledger reopen** (run 66: 8 leads, 0 net-new; 12 `built` rows reopened). B1 scored on the STRICT reading (engine `eligible`, not "not ineligible") — both readings pass at cap 40, three of four days would fail at cap 8. No 14-day back-test exists (no runs 08-08…08-18) |
| D-282 | DECISIONS.md | 12882 | **The zero-output guard was NOT changed, and B5 has no working instrument.** Two record corrections. (1) **The false alarm is NOT reachable** — firing needs `hidden_handled == 0` (measured **8/48/128** on runs 68/69/71) and an empty shortlist (`capped_by_top_n` 3,603–3,683, so `visible` is full every run), so it was never the Gate P3 blocker D-280 called it. (2) **The docstring's "D-246" attribution was NEVER a ruling** — D-246 is the seniority-gate decision and says nothing about this guard; no entry is superseded. The obvious fix (rejection buckets disarm the fatal, loudly) was built and **REJECTED before merge**: it makes the fatal UNREACHABLE, because `hidden_hard_filter` is corpus-scoped at **18,472–18,932** every run — it fixed the false alarm by deleting the alarm. **Root cause, and it generalises: `candidate_judged_this_run` is RUN-scoped while every `hidden_*` bucket is CORPUS-scoped, and those buckets are an EXHAUSTIVE PARTITION — so "can this run explain the empty day?" is always yes by construction. A complete partition cannot evidence a silent failure.** Ships only the stale-premise docstring correction at BOTH sites (`pipeline/runner.py`, `store/run_funnel_queries.py`); guard left armed. Rejected: the disarm; adding `hidden_ineligible == 0 and hidden_below_cutoff == 0` (both routinely non-zero — 550 and 3,683 on run 69). **Owner-gated: run-scoped rank attribution is the only honest fix, and until it exists B5 is UNSCOREABLE — do not score it on exit status alone.** Also recorded unfixed: excluding `hidden_ineligible` "because `ineligible` is not in `candidate_judged_this_run`" holds only for the deterministic engine; `top_cmd.py` also hides on the LLM final gate's verdict |
| D-283 | DECISIONS.md | 12932 | `exact_quad` (company+title+location+body) ratified as the ONLY duplicate kind — matches `SUPPRESSING_KINDS`; body-hash alone and company+title+location alone both merge distinct jobs (727 body-hash groups span different titles/locations). Ships `boardwatch identities leakage [--days N=7] [--json]`: counted over jobs that **reached leads**, not the corpus; `redundant = identified − distinct groups` at **job** granularity (posting granularity would flag a correct regroup as a leak); window anchored on `first_decided_at`, not `decided_at`. `unidentified` (body-less, withheld an identity by D-279) is its own bucket, never folded, never in the denominator; "not measurable" instead of 0%. **LIVE: 100 surfaced / 100 groups / 0 redundant = 0.00%.** Second path agreed exactly (180/180/0 on a copy) and `--days` proved to filter (150/153/180/180). Measurable and passing but **not yet "over 7 days"** — ledger starts 2026-08-19, and the 7-day `seen` TTL cannot be observed faster than itself; first true window ~2026-08-26, inside Parts 2–4, so off the critical path |
...
## D-229 — P5b's criteria, named by the owner: three clean projected runs over ≥ 30 distinct postings

*2026-08-18. Answers the open question D-225 left explicitly to Mit, and closes the last thing blocking
the projection spec's parent P5 from ever being assessable.*

**Context.** D-225 shipped `boardwatch run --project` as opt-in and stated that flipping `resume.yaml` out
of the unattended default is **P5b, owner-gated**, with its criteria unnamed. An unnamed gate is not a
conservative gate — it is an unmeasurable one, and parent P5 could never be marked met or unmet while it
stayed unnamed. STATE carried it as an open question and three sessions declined to infer it.

**The ruling (Mit's, given at session start).** `resume.yaml` stops being the unattended default after:

| Clause | Bar |
|---|---|
| Clean projected runs | **3** |
| Distinct postings across them | **≥ 30** |
| Run-scoped preflight fatals | **0** |
| Résumé QA-gate failures | **0** |
| Fabrications | **0** |

Deliberately lighter than Gate P3's seven-run shape, and the trade was stated when the choice was made:
it exercises the bundle path as the real default sooner, on a thinner evidence base than either Gate P3
or B4's `n ≥ 100` asks for. Naming a light gate that is actually measured beats an unnamed strict one.

**Evidence gathered the same session, against a COPY of the live store.** Three `--project --no-scan
--top 12` runs (ids 23/24/25), `BOARDWATCH_DATA_DIR` **and** `BOARDWATCH_CONFIG_DIR` both overridden —
`--data-dir` alone is not enough, because the bundle resolves off `config_dir`:

| | run 23 | run 24 | run 25 |
|---|---|---|---|
| `summary.fatal` | None | None | None |
| projection entered → advanced | 12 → 12 | 12 → 12 | 12 → 11 |
| tailor | 12 → 12 | 12 → 12 | 11 → 11 |
| pdf | 12 → 12 | 12 → 12 | 11 → 11 |
| wall clock | 28.8 s | 52.2 s | 28.7 s |

**35 distinct postings, 35 PDFs, 0 drops in the tailor and pdf stages.** Run 25's single projection drop
is `withheld_not_live` — liveness re-fetched the posting and it answered 404/410, so it never entered the
tailor loop. That is the liveness stage working, not a render failure, and it is its own bucket precisely
so it cannot be read as one. `hidden_handled` grew 0 → 12 → 24 across the three runs, which is the
disposition ledger suppressing each run's leads from the next — the mechanism that made the postings
distinct rather than the same twelve three times.

**The QA clause is verified through a second path.** The funnel's `pdf` stage is the emitter's own report,
so it cannot be the evidence that the page-count gate held. `pdfinfo` over the 35 emitted files
independently returns **1 page for all 35**, against `resume_max_pages=1`.

**Four of the five clauses are met by this evidence; the fifth is not, and is not ours.** Fabrications are
B4's independent audit and no run can self-report them. **Nothing here flips the default** — that is
Mit's action, on his reading of the fabrication clause.

**What this evidence is not.** These are `--no-scan` runs against a copy. They prove the mechanism end to
end; whether copy runs count toward a gate about *unattended* operation is Mit's call. Running them
against the live store would have earned 35 real postings a permanent `built` disposition — the exact
defect D-225's fail-closed design exists to prevent — for evidence that a copy produces identically. The
live store was confirmed untouched afterwards: still **0** `job_dispositions` rows and 22 `runs`.

## D-230 — The backfilled `runs` rows close as `ok`, matched by predicate, and `doctor` asserts the invariant

*2026-08-18. Repairs the leak diagnosed 2026-08-17 and left owner-gated on which terminal status the rows
should get.*

**Context.** Four `runs` rows (ids 1-4) read `status='running'` with `finished_at` set. Migration
`p0_run_status` (`541dfdd`) added `status` with `DEFAULT 'running'`, backfilling rows the pre-status
`finish_run` had already closed by stamping `finished_at` and nothing else. No drain can reach them:
`reap_stale_runs` requires `finished_at IS NULL`, and `tests/unit/test_queries.py` pins that exclusion as
intentional. Blast radius is nil — every other `runs` query is scoped by `run_id` or `finished_at IS NULL`
— so this is a correctness repair, not an outage fix.

**Decision 1 — the status is `ok` (Mit's).** `p0_run_status`'s own docstring argued the opposite:
*"inventing `ok` for them would be a fabricated measurement"*, on the premise that the rows are
*"indistinguishable from a killed run"*. **That premise is false, and the store disproves it.** A killed
run never reaches the closer, so it keeps `finished_at` NULL; every affected row carries a `finished_at`,
so every one of them was closed by a writer that ran. The genuinely unrecorded question is narrower than
the docstring assumed — not "did this run survive" but "did the close succeed or fail", which the
pre-status closer did not distinguish either way. `ok` is the owner's ruling on rows whose pipeline
demonstrably reached its closer. Recorded because it contradicts a prior entry's reasoning: the ruling
stands on a **corrected premise**, not on overriding the old argument on its own terms.

**Decision 2 — matched by predicate, never by id.** The migration updates
`WHERE status='running' AND finished_at IS NOT NULL`. That combination is **unreachable through every
current write path** — `record_scan_run(finished=True)`, `finish_run` and `reap_stale_runs` each stamp
`finished_at` and `status` in the *same* UPDATE, so no writer can produce one without the other. It is
therefore a backfill artifact by construction rather than a fact about this repo's database, which is what
makes the repair correct for an operator whose affected ids are not 1-4. Hardcoding `id IN (1,2,3,4)`
would have been correct here and wrong everywhere else — a published mechanism carrying a personal
instance's row numbers (§3b).

**Decision 3 — `downgrade` is a deliberate no-op.** After the upgrade the repaired rows are
byte-identical to runs that genuinely finished `ok`, so no predicate can pick them back out; running the
upgrade's predicate in reverse would relabel every successful run `running` and manufacture the defect the
migration exists to remove. A downgrade that cannot restore the prior state truthfully does nothing.

**Decision 4 — the invariant is asserted, not just repaired.** A one-shot migration fixes the four rows
and says nothing about the fifth. `doctor` now queries the same predicate, names the offending ids, and
**exits 1**. Fatal rather than a warning: the rows are inert today, but the only things that can produce
one now are a bad migration or direct database surgery, and both deserve to stop a run.

**It found a defect on its first execution.** `test_cli_stale_board_is_informational_not_failure` seeded a
`runs` row with `finished_at` set and no `status`, taking the `running` server default — constructing, in
a fixture, exactly the shape no write path can produce. The invariant failed that test immediately. The
fixture was wrong and now states `status="ok"`.

**Generalises.** *A column default that is right for new rows can be wrong for the rows it backfills.*
`DEFAULT 'running'` is correct for a row being created and was silently applied to rows that had already
finished — the migration's `ALTER TABLE ... DEFAULT` is two decisions wearing one syntax, and only the
first one gets thought about. And: *an invariant is worth more than the repair that motivated it*, because
the repair is a claim about four known rows while the invariant is a claim about every future one.

## D-231 — D-225's residuals: four closed, one declined, one not reproducible

*2026-08-18. Clears the residual list D-225 recorded as "recorded, not fixed". None was blocking.*

**Closed.**

1. **The three authoring `as_of` values now read the same clock as the render path.**
   `cli/profile_bundle_cmd.py`'s `promote-candidates`, `edit-fact` and `add-fact` called `date.today()`
   while projection and render were unified on `utcnow().date()`. `as_of` decides *which facts are
   effective*, so a local-date authoring decision either side of UTC midnight could disagree with the
   render's — a fact authored "today" that the renderer does not consider effective yet. Now
   `utcnow().date()` at all three.
2. **The projected-artifact recount no longer spells out the field name.**
   `store/run_funnel_queries.py` hardcoded `json_extract(..., "$.projection_kind")`. It correctly avoided
   copying the emitter's *value* constant but still carried its *field name*, so renaming
   `ResumeSourceLineage.kind` would have left the probe compiling, running, and matching nothing — and
   "no rows carry this lineage" is indistinguishable from "the key I asked for no longer exists". The
   prefix is now a named constant `META_PREFIX` in `core/lineage.py`, and `meta_probe_key()` derives the
   key from `dataclasses.fields(...)` — sound for any field, because `as_meta` writes them all.
3. **`projection_ran` is required.** `collect_run_funnel(..., projection_ran: bool = False)` failed in the
   omission direction: a caller that forgot it silently claimed projection never ran and dropped the whole
   stage from the artifact, which reads as a legitimate authored run rather than a mistake. There is no
   safe default in the other direction either (`True` fabricates an empty stage on every authored run), so
   the honest answer is that the caller states it and the type checker enforces that it did. One
   production caller, which already passed it explicitly.
4. **The run-scoped refusal message is written once.** `_projection_unavailable()` now serves both the
   preflight and the run-scoped arm of the per-lead loop. They resolve through the same closed catalog but
   wrote their sentences separately, and only the preflight's carried the remedy — so a `bundle_unreadable`
   surfacing from the loop told the operator what happened and nothing about how to recover, for a cause
   the preflight explains fully. Which site a run-scoped member surfaces from is an accident of timing and
   may not decide how much the operator is told.

**Declined, deliberately.** `run_preflight`'s per-lead taxonomy load stays as it is. D-225 accepted it with
a stated reason — a mid-run `taxonomy.yaml` edit makes a later lead raise `NO_JD_EXTRACTION`, routed to the
per-lead `extraction_unavailable` outcome, so the lead is skipped and counted rather than rendered under
mixed rules. Refusing the lead is the fail-safe direction. Changing it is a §4.1 freeze question, not
residual cleanup.

**Not reproducible.** D-225 recorded that *"five test comments quote the old fatal text inexactly"*. No
test comment quotes that text: the projection tests assert on **enum members**, and `grep` for the fatal
string, its remedy clause and its paraphrases returns nothing across `tests/`. Recorded as not-found
rather than silently dropped or "fixed" by editing comments chosen to fit the claim. *A residual is a claim
too, and it is not privileged by having been written down by the session that created it.*

**One adjacent site observed and left alone.** `profile_bundle_cmd.py:524` builds a `--completeness`
effective date from `datetime.now().astimezone().date()` — the same local-vs-UTC class as residual 1, but a
validation as-of rather than an authoring one, and outside what D-225 named. Recorded here so the next
session finds it deliberately left rather than missed.

## D-232 — An independent repo audit found a false number on the rendered résumés; P5b's fabrication clause fails, then is repaired

*2026-08-18. The first verification of bundle content by a party that did not write it. Follows D-229,
which named the clause this tests.*

**Context.** P5b's fifth clause is *0 fabrications* and no run can self-report it (D-229). Two routes were
considered. The first — sweeping job-apps' 355 sent résumés, cover letters and application Q&A — was
built (`CLAIMS-TO-VERIFY.md`) and then **demoted to secondary by the owner**, on the correct ground that
job-apps documents were also authored by Mit: agreement proves *consistency*, not truth, and cannot catch
an error the two share. The second, and the one that ran: a **fresh session with no access to boardwatch,
the wiki or job-apps**, checking claims against **source code and git history only**.

**The audit surface is 19 claims, not 42 résumés.** All 42 PDFs from D-229's runs are subsets of one
fixed pool; the wording never varies between them, only which items a JD selects. Verifying the pool once
verifies every document — a property worth knowing before anyone budgets time for this again.
...
| **CONTRADICTED** | **1** | StreakSync — see below |
| Mixed | 1 | Saayam — see below |
| UNVERIFIABLE | 7 | NIO (3, private NSF app), SAKEC (2, private + a team's work), Knowledge Forge (2, **no repo exists**) |

**The defect. `fact.streaksync.contribution.001.r3` claimed "446 XCTest tests across 36 suites"; the repo
holds 396 across 31.** Confirmed by a second, independent count before acting on it: 386 unit + 10 UI
tests, 29 + 2 `XCTestCase` classes, and **0** Swift-Testing `@Test` macros, so no counting convention
reaches 446. Repaired as `.r4` in **revision 22**; the correction is character-neutral (`446`→`396`,
`36`→`31`) so the D-219 page budget cannot shift. Verified rendering through `profile-bundle project`.

**How the number survived eleven refinement sessions — the finding that generalises.** The provenance is
readable in the fact chain:

- **`.r2`** (D-215's refinement) said *"…176 files, ~32k LOC, **446 XCTest tests**)"*.
- **`.r3`** rewrote the sentence completely — different structure, different emphasis — **carried `446`
  forward untouched, and ADDED `36 suites`**, a number that had never appeared before.

So a rewrite by a repo-grounded session both preserved one unmeasured number and introduced a second.
**Rewriting a sentence is not re-measuring its facts, and it feels like it is.** The edit's attention goes
to structure and wording; a numeral that survives the rewrite is never re-read as a claim. Worse, a rewrite
is where *new* numbers get added, because the new structure invites a detail the old one had no slot for —
and that number is born unverified while sitting inside a sentence that has just been "reviewed".

**Consequence for authoring, adopted:** when `edit-fact` changes a value that contains a numeral, the
numeral is a **new claim regardless of whether it changed**, and must be re-derived from the artifact —
never carried. Inheritance is not evidence.

**Saayam — the audit's verdict was narrower than reported, and the distinction was load-bearing.** The
report returned *UNSUPPORTED (authorship)* on `fact.saayam.accomplishment.001.r2`, citing **0 commits by
this person across 46 authors** (verified: the sole "Mit" match is *Mitalakar*). But the bullet reads
*"Volunteer on the AI/GenAI track at a 501(c)(3) non-profit aid platform **whose** … microservice … routes
…"* — the relative clause describes **the platform**, not the author. The sentence never claims he wrote
it, so an authorship finding answers a claim it does not make. What is genuinely off is narrower: **"on
AWS Lambda" is not in the current tree** (`app.run(debug=True)`; Lambda appears only in that repo's
history, by other authors). **Owner ruled: leave it unchanged** (D-191 — wording is his).

**A negative that was retracted before it was reported.** A scan of the other seven `saayam-for-all` repos
for Lambda config returned empty, and was **not** treated as evidence: a control query proved the search
worked, then the API rate-limited mid-sweep, so the empty result described the instrument, not the org.
The claim was scoped to the one repo actually cloned. *An empty result is a claim about your instrument*
— and the discipline has to fire while a finding is being written, not after.

**P5b's standing.** The clause **failed**, on a real false number, on résumés that had already been
rendered. It is repaired at revision 22 — but the 42 audited PDFs were rendered from **revision 21**, so
they no longer correspond to what the pipeline produces. **Whether re-rendering and re-auditing is owed
before the clause counts as met is the owner's call, not a session's.** The default has not flipped.

**What this vindicates.** The audit existed because no run can self-report fabrication, and it found a
defect within one pass that eleven attended refinement sessions and a passing Gate B did not. **Gate B's
...
## D-233 — ATS-parsable résumé PDFs: XeTeX ligatures disabled via fontspec

*2026-08-18. PR #83, merged. The rendered résumé read correctly on screen and wrong to every machine that
parses it.*

**Context.** tectonic (XeTeX) with the default Computer Modern fonts emits the `ff` ligature as the single
codepoint U+FB00 with no ToUnicode mapping, so `pdftotext` — and every ATS that runs one — reads
"e**ffi**ciency" and "tra**ffi**c" as garbled or dropped text. The word looks right in a viewer and is
unsearchable underneath, which is the D-006 silent-degrade shape aimed at the one reader that decides
whether the résumé is seen at all.

**Choice.** Load the font through `fontspec` with `Ligatures=NoCommon` in the bundled `resume_base.tex`, so
the common ligatures never form and each glyph stays one ASCII codepoint. Small caps are preserved by naming
the separate face explicitly — `SmallCapsFont={lmromancaps10-regular.otf}` (Latin Modern, visually identical
to Computer Modern) — because `\scshape` otherwise falls back to normal letters once the default CM face is
displaced. The font is resolved from **tectonic's own bundle**; nothing is shipped. The generalization R7
template-hash pin on `resume_base.tex` was updated to the new bytes. Text now extracts as ASCII; small caps
and the one-page fit are unchanged.

**Alternatives rejected.** `\input{glyphtounicode}` + `\pdfgentounicode=1` is pdfTeX-only and errors under
XeTeX. `\XeTeXgenerateactualtext=1` alone writes ActualText spans but produces NUL bytes with CM fonts,
trading one unparseable form for another. Shipping a font file was unnecessary once the bundle's Latin
Modern resolved.

**Generalises.** *A glyph that renders correctly is not a character that extracts correctly* — the on-screen
form and the copy-paste/ToUnicode form are two separate outputs, and the ATS reads the one no human
proofreads.

## D-234 — Opt-in `fill_to_page` second growth phase for projection Stage 2

*2026-08-18. PR #84, merged. Default off.*

**Context.** Stage-2 `select` enforces a strict `ADMISSION_FLOOR=0` — a candidate that shares no JD-skill
overlap is never admitted — and its `no_match_fallback` is all-or-nothing. On a narrowly-targeted JD that
leaves the page short, neither lever fills it: the floor excludes the only remaining candidates and the
fallback would swap the whole selection rather than top it off. The result is a JD-targeted résumé that
cannot use the page it has.

**Choice.** A per-declaration opt-in `fill_to_page: bool` on `ProjectionDeclaration`. When true, a **second
growth phase** runs after the ranked selection: it tops off from the remaining candidates in **declaration
order**, bypassing `ADMISSION_FLOOR`, until the page budget is reached. The ids it added are recorded on
`SelectionResult.fill_added_ids`, so the fill is auditable and distinguishable from ranked admissions.
Default off leaves the pipeline unchanged.

**Consequence.** Adding the field shifts every `projection_digest`, so existing users must re-approve their
projection once on upgrade (`approve-projection`). This is the intended fail-safe: the digest reopens the
owner's editorial gate whenever the projection's controlling shape changes.

**Alternatives rejected.** Relaxing `ADMISSION_FLOOR` globally would push JD-irrelevant content onto
**every** targeted résumé, not just the ones with room — the floor's whole point. A digest scheme stable
under added fields is a larger change that itself shifts the digest once, buying nothing over the single
re-approval this costs.

## D-235 — Opt-in first-bullet link placement and reverse-chronological project sort

*2026-08-18. PR #85, merged. Two independent opt-in controls, both default off, both carrying D-234's
digest-reopens-gate consequence.*

**Context / choice 1 — `link_in_first_bullet` (per-entry).** A project's link rendered on its heading
collides with the dates on a wide heading line (Crop-RF's DOI was the case). When set, the entry's link
renders as an underlined label appended to its **first bullet** instead of the heading, clearing the
collision. The layout firewall `validate_layout` reconstructs the F1 first-bullet expected substring through
the **shared `_href` helper**, so the escaping round-trip stays byte-exact. A review caught that the naive
first cut computed the expected text a second way: the round-trip diverged, raised `ESCAPING_MISMATCH`, and
silently degraded tailoring to the untailored static fallback — non-shippable. Fixed by routing both the
emitter and the check through `_href`, and tested.

**Choice 2 — `sort_projects_by_date`.** Orders PROJECT entries newest-first by their structured
`year_month` start date, applied **after** selection and fill so it reorders without changing what was
selected. Experience entries are untouched.

**Consequence.** Both add fields to the projection surface, so each shifts `projection_digest` and reopens
the owner's `approve-projection` gate once on upgrade — the same fail-safe as D-234.

**Generalises.** *A layout check that recomputes the emitter's output by a second route will drift from it*
— the expected substring and the rendered one must come through one helper, or the firewall fails safe on a
difference it created itself.

## D-236 — STATE's "next session is the résumé" line was stale; the résumé track is complete and the linear roadmap is frozen or owner-gated

*2026-08-18. A bookkeeping correction under the session-start ritual — "if STATE disagrees with the repo, the
repo wins; fix STATE and record the correction here." Changes no shipped behaviour.*

**Context.** `STATE.md` still carried *"The next session is about the RÉSUMÉ, at Mit's direction — not the
engineering backlog."* The repo contradicts it: PRs #83–#86 merged the résumé-render track (D-233 ATS
ligatures, D-234 `fill_to_page`, D-235 first-bullet link + project sort), and Mit's personal template and
declarations already adopt all four. Nothing about the résumé is queued for a Claude session; what remains on
that track is Mit's alone (whether to send a given document; the two owner-gated prose rewrites of D-220).

**Choice.** Corrected the line and, in the same rewrite, trimmed `STATE.md` back toward its ~170-line target
(it had drifted to ~324 as sessions appended narration that belongs in this log). The new current-standing
statement is the load-bearing one a stale "do the résumé" line was hiding: **the linear roadmap is not
unilaterally advanceable.** P0/P1/P2/P5 gates are MET; P3, P6 and the 14-day clock are **frozen by D-155**
(job-apps delivers Mit's résumés daily, so there is no live gap and the freeze is costless); P4 is built but
its only remaining clause is Mit's blind craft review; P7 is breadth-last and gated on P0 attribution data.
Because P3's gate is unmet, the ordering rule ("do not start a phase whose predecessor's gate is not met")
bars starting P4 — and P3/P6 are frozen by owner decision, not by a missing build. **The next roadmap move is
therefore an owner decision** (unfreeze P3/P6, run the P4 blind review, or open an owner-gated item — STATE
item 4's `_merge_categories`/D-184-finding-2/Education-Slice-C, or P2 item 8's onboarding gatherer), not code
a fresh session should manufacture.

**Alternatives rejected.** *Start P4 or an owner-gated item unilaterally to "make roadmap progress"* — that
overrides a deliberate owner freeze (D-155) and the "do not start" markers on those items; the honest move is
to surface the block. *Leave STATE as-is and only patch the one line* — the whole file had drifted long and
the stale framing was symptomatic, so the trim and the correction are one change.

**Consequence.** A fresh session now reads, from STATE alone, that the buildable roadmap is exhausted pending
an owner call, instead of being pointed at a résumé task that no longer exists. No decision here is reversed;
D-155's freeze stands until Mit lifts it.

## D-237 — `_merge_categories` refuses a label that slug-collides with a SEEDED catalog category; the catalog owns the display_name

*2026-08-18. Mit's ruling on the last open slug-collision site, the one D-210 deferred. Closes the
D-202…D-210 lossy-slug track.*

**Context.** `_merge_categories` guarded only `if category_id not in known`, so a promoted skill's
group label whose `_slug` hit a `category_id` the **seeded** catalog already defined under a *different*
`display_name` (author writes group "Technique"; the catalog defines `technique` → "Techniques") was
silently skipped: the skill filed under the catalog's label, the author's dropped, nothing warned. This
is the same lossy-slug class the four sibling guards refuse (D-202…D-206, D-210), one step later — and
STATE carried it as owner-gated. D-210 deferred it because, unlike the siblings (two *author* labels
colliding, unambiguously refuse), this collides with the *pre-existing* catalog, where "normalize the
author's variant to the canonical term" is a defensible behaviour — so who owns the `display_name` was
genuinely open.

**Choice (Mit's).** The catalog owns the `display_name`; **refuse.** On `category_id in known` with a
differing `display_name`, raise `PromotionError` naming the author's label, the catalog's `display_name`
and the shared id, telling the author to align the group label to the catalog or change the catalog's
`display_name`. Consistent with every sibling guard and `CLAUDE.md`'s closed-catalog / no-silent-merge
doctrine. **All five slug-collision sites now refuse** — the track is closed.

**Alternatives rejected.**
- *Catalog wins, made explicit* — keep silently normalising the author's variant to the seeded label,
  but on purpose and tested. Rejected: a silently-replaced label is the same silent merge the other
  guards refuse; a loud "align your label" is safer and actionable. The catalog's `aliases` field is the
  designed path for genuine variants, but promotion slugs the label directly and **does not consult
  aliases**, so an alias would not currently resolve a colliding label (a possible future enhancement,
  not this change).
- *Author wins — override the seeded `display_name`.* Rejected: lets one bundle's promotion rewrite the
  shipped seed vocabulary.

**Consequence.** Closes the last owner-gated slug-collision site. `known` becomes a
`category_id → display_name` map (the guard needs the value, not just the key). Reachable for any
operator shipping a seed catalog (the comprehensive example seeds `technique` → "Techniques"); Mit's own
bundle seeds no categories, so it does not fire for him today. Pinned by
`test_promotion_refuses_when_a_label_collides_with_a_seeded_category`, driven through the full
`promote_candidates` path like its four siblings.

**Generalises.** *A guard that keys only on "is this id new?" cannot see a collision with an EXISTING
member under a different display* — enumerate the existing values, not just their keys.

## D-238 — A partial extraction sets the record aside whole; the drain fires even when the record produced candidates

*2026-08-18. Mit's ruling on the open owner-gated item "D-184 finding 2". Closes the silent partial-emission
leak.*

**Context.** `run_extraction` recorded a drain reason only for a record that produced **zero** candidates
(`if not produced`). A record that grounded some slots but hit a slot-level failure on another — an
untypeable value (a garbled `dates`), an ungrounded span, or an unsupported entry kind — kept its good
candidates and recorded **no** reason. `_disposition_for` (`imports.py`) derives disposition from the
candidate package, so such a record dispositioned `imported`, and the dropped field went unaccounted.
`validate_extraction_report` enforces exactly one reason per `review_required` record and none for
`imported`, so the failed slot had nowhere to be recorded — a silent partial emission.
`test_a_garbled_dates_string_drains_value_not_typeable` explicitly encoded the old behaviour ("heading
still lands project.name, so the record is NOT review_required"), which is what confirmed the leak was
intentional-looking rather than incidental.

**Choice (Mit's).** Set the whole record aside for review when any slot fails. `run_extraction` now
withholds a record's candidates and drains its reason whenever a slot reported one — imported **iff** it
produced candidates **and** raised no reason. The disposition follows automatically: withholding the
candidates makes the record `review_required` with one drain reason, and the candidates/failures
disjointness invariant is preserved. Minimal surgical change to one function; latent, not live (D-184
confirmed every live résumé entry parses, so nothing regresses today).

**Alternatives rejected — the four designs the finding named.** (A) *Refuse the whole extraction* — needs
a required-vs-optional slot vocabulary the catalog lacks and would refuse legitimately-open ranges. (C) *A
per-slot partial-loss entry on the still-imported record* — breaks the report's 1:1 mirror of
`review_required` and the denominator arithmetic, and needs a schema bump. (D) *A new `imported_partial`
disposition* — breaks the closed three-branch disposition catalog. **B (this)** is the minimum change,
reuses the drain designed with the quarantine, and is the keystone analogue: a record that cannot ground
every part it reached abstains rather than importing a half-record.

**Consequence.** `test_a_garbled_dates_string_drains_value_not_typeable` was rewritten to its still-true
no-candidate case; `test_a_record_with_a_failing_slot_is_set_aside_whole_not_imported_partial` pins the new
behaviour. The legitimate-absence path is untouched — a `None` from `_emit` (an absent field, an open range
end) sets no reason, so heading-only and open-ended records still import.

**Generalises.** *A drain that fires only when a record produced NOTHING cannot see a partial emission* —
key the drain on "did this record fully succeed?", not on "did it produce anything?".

## D-239 — Education Slice C ruled: exclude `header/1` (the professional name) like `header/2`; the name is owner-supplied at onboarding

*2026-08-18. Mit's ruling on owner-gated item 1. Closes the last owner-gated code item on the bundle track.*

**Context.** `build_promotion` (`profile_bundle/candidate_promotion.py:182`) partitions a source's candidates
by locator shape; `header/*` falls through and is never promoted, "person facts need `facts/identity.yaml`".
`header/2` (the email) is excluded because no predicate models it (`no_predicate_exists`).
`header/1` (`person.professional_name`) was the one undecided locator: promote it into the owner-authored
`facts/identity.yaml` — or exclude it like `header/2`. Promotion's write contract is **whole-doc, one-shot,
no-clobber** (D-182), and `facts/identity.yaml` is owner-authored and already carries the display name,
gathered at onboarding (D-054). Extraction still lands a `header/1` candidate; only promotion declines it —
the current fall-through already excludes it, and three suites pin that (`test_extraction_mapping.py`,
`test_candidate_promotion.py`, `test_profile_bundle_source_enumeration.py`).

**Choice (Mit's).** Exclude `header/1`, like `header/2`. The name is owner-supplied at setup, so re-deriving
it from the résumé and reconciling a duplicate is net-negative. **No code change** — the ruling confirms the
existing, tested behaviour; it converts a comment into a settled decision.

**Alternatives rejected.** *Promote-and-merge the `professional_name` fact* — would teach promotion to reach
into an owner-authored file, reconcile against a probably-present `owner_confirmed` name, and break the
no-clobber whole-doc write contract (D-182), all to import a fact the owner already supplies. Its one
argument — a résumé-upload-only user who never typed a name — is void because onboarding always gathers
identity up front (D-054); a genuinely missing name is a gap in the onboarding gatherer (owner-gated item 2),
not a job for promotion.

**Consequence.** Education Slice C closes as "nothing to build." STATE drops it from the owner-gated list.
Behaviour and tests unchanged.

**Generalises.** *A fact the owner authors at onboarding is not re-derived from an import source.* Exclude it
from promotion rather than teaching promotion to clobber the owner's file.

## D-240 — Mit lifts D-155's freeze on P3 / P6 / the 14-day clock; the linear roadmap resumes and its remaining gates are operational, not build

*2026-08-18. Mit's owner decision, the one STATE (D-236) reserved for him. Amends D-155 (the freeze only) and
supersedes D-236's "buildable roadmap exhausted" conclusion.*

**Context.** D-155 (2026-08-13) did two things: reoriented the program onto the bundle-to-résumé path, and
**froze** P3, P6 and the 14-day acceptance clock — job-apps delivered Mit's résumés daily, so there was no
live gap and the freeze was costless. STATE's standing (D-236) was that, with P3's gate unmet and P3/P6
frozen by owner decision, **no** linear-roadmap phase was unilaterally startable and the next move was an
owner decision. Mit made it: unfreeze.

**Choice (Mit's).** Lift D-155's freeze; the linear roadmap resumes. **Only the freeze is lifted — D-155's
reorientation onto the bundle path stands; `resume.yaml` remains an import source, never hand-fixed.** The
material fact, surfaced before the decision and recorded here so it is not re-litigated: **unfreezing
surfaces mostly OPERATIONAL work, not new build.** The builds for P3/P4/P6 are essentially complete; their
gates close by *running* boardwatch, not by coding:
- **Gate P3** = 7 consecutive unattended runs (0 silent empty days, 0 false-success, 0 stale feeds) **plus**
  the one remaining P3 build item — item 8, the cross-OS two-writer corruption test (the only frozen item
  with no `DONE` marker).
- **Gate P4** = the owner's blind craft review of 10 résumés (never run; the ordering rule bars starting it
  until P3's gate is met).
- **Gate P6** = duplicate leakage ≤ 5% measured over a real 7-day window, and 0 dead postings reaching the
  lead list — both need real runs whose leads are liveness-probed.
- **14-day acceptance** measures a frozen system and starts *after* P6. **P7 breadth stays last.**

Two workstreams open now, both Mit-chosen: **(a)** build P3 item 8; **(b)** stand up the daily
unattended-run mechanism — the owner starts it against his live store (we build/document the mechanism only,
never run it against the live store unilaterally).

**Alternatives rejected.** *Keep the freeze* (Mit's call to lift). *Start P4 first* — barred by the ordering
principle while P3's gate is unmet. *Jump to P7 breadth* — breadth is last; it multiplies whatever is
downstream, and conversion is not yet proven by a run. *Manufacture roadmap code to look busy* — there is
almost none to write; the honest next step is operation.

**Consequence.** STATE drops "frozen" from P3/P6/the 14-day clock; the "buildable roadmap exhausted" headline
(D-236) is superseded. The critical-path build item is **P3 item 8**; everything downstream is operation or
owner-review. The D-230 `runs_status_backfill_repair` migration, owed to the live store, becomes relevant the
moment daily runs start.

**Generalises.** *A freeze justified by "no live gap" is ended by an owner's decision to operate, not by new
code.* Unfreezing a built-but-never-run system surfaces operation, not build — say so plainly rather than
inventing phases to fill the gap.

## D-241 — P3 item 8's two-writer test/guard: same-OS test + a runtime refusal of WAL-unsafe filesystems, because the cross-OS config cannot run in CI

*2026-08-18. Building the one remaining P3 build item after Mit unfroze the roadmap (D-240). Mit chose the
recommended path: test the testable, guard the untestable.*

**Context.** P3 item 8 asks for single-writer discipline on the SQLite file, a documented WAL stance, and a
**two-writer** test — one that "must cover the cross-OS case", the Linux-container-plus-macOS-host bind-mount
that corrupted job-apps' primary key, because "a same-OS test will pass and prove nothing about the failure
at risk". Two of the three were already done: the WAL PRAGMAs (`store/db.py:49-51`), the single-writer
discipline (the scan lock + the on-conflict-do-nothing unique indexes, D-020/D-041), and `WAL_DISCIPLINE.md`.
The gap was the two-writer test — and the cross-OS half is unbuildable as a green CI check: **GitHub's macOS
runners cannot run Docker**, so the exact corrupting config can never be reproduced in CI.

**Choice (Mit's, the recommended option).** Split the risk by testability. **(1) Same-OS — a real test.**
`tests/pipeline/test_two_writer_concurrency.py` spawns two subprocesses that each append 200 run rows to one
`boardwatch.db` concurrently, then asserts `PRAGMA integrity_check == "ok"` and that all 400 writes landed —
genuine kernel-level concurrency, the regression guard for WAL + busy_timeout under contention. **(2)
Cross-OS — prevention, not a test.** A new `store/fs_safety.py::unsafe_wal_filesystem` reads
`/proc/self/mountinfo`, and `get_engine` **refuses** (`WalUnsafeFilesystemError`) when the store sits on a
WAL-unsafe filesystem. Detection is precise: a host bind-mount reads inside the container as
`virtiofs`/`fuse.grpcfuse`/`9p`/etc. (a closed catalog + the `fuse` prefix), while a named Docker volume
reads as the container's own `ext4`/`overlay` and is cleared. Detection is Linux-only — non-Linux hosts have
no `/proc/self/mountinfo`, the host side of a bind-mount is normal local disk, so the guard is a no-op and
never refuses a legitimate local run (including every `make check` on macOS and every CI job whose tmp is
ext4/tmpfs/overlay). The Gate P3 clause is amended to match: same-OS test green + cross-OS refused at runtime.

**Alternatives rejected.** *Same-OS test only, keep the doc note* — leaves the real hazard an unenforced
paragraph nobody is forced to read. *A Docker integration test* — real, but it runs only locally or on a
self-hosted macOS runner, never green in GitHub CI, so it cannot gate `make check`; a gate clause that no CI
job runs is not a gate. *Warn instead of refuse* — a warning is not an enforced safeguard, and the fail-safe
direction for a corruption gate is to fail CLOSED: refusal on a network share is a recoverable inconvenience
(move to local disk), silent corruption is unrecoverable data loss.

**Consequence.** Item 8's test/guard half is DONE; the 7-consecutive-runs half is operational and begins when
Mit stands up the daily unattended run. `WAL_DISCIPLINE.md` and PROGRAM.md §3.P3 are updated. The guard lives
at the true DB-open site (`get_engine`), so every entrypoint inherits it. Latent for Mit today — his store is
on local macOS disk, where the guard is inert.

**Generalises.** *When the exact failure config cannot run in CI, split the risk by testability:* test the
part you can reproduce, and for the part you cannot, prevent the config at runtime rather than pretending a
same-OS test covers it. A guard belongs at the resource-open site, not at one caller.

## D-242 — The unattended daily entrypoint is `boardwatch run`, scheduled by the operator; the daily driver runs LIVE, not against a copy

*2026-08-18. The second workstream Mit opened by unfreezing the roadmap (D-240): stand up daily unattended
runs. Documents the mechanism and corrects a stale belief the investigation caught.*

**Context.** P3's gate needs 7 consecutive unattended runs, but the repo documented scheduling only for
`boardwatch scan` (README "Schedule scans") — a partial job that fetches boards without evaluating, ranking,
tailoring, or emitting the morning artifact. Two beliefs about this were **wrong** and are corrected here:
(a) D-123 is **not** about copy-only unattended pipeline runs — it is an old Claude-Code dev-prompt
LaunchAgent that re-fired a shipped one-shot task; (b) there is **no** scheduling code in `src/` and was no
committed scheduler for the full pipeline. The "copy-only" discipline is real but is a **measurement**
practice (gate/benchmark runs use `--no-scan` against a byte-verified copy so the live store is never
mutated); it was never meant to be the daily driver. That is precisely why the headline number is 0: every
run to date has been against a copy.

**Choice.** Document scheduling the **full pipeline** — `boardwatch run` (scan → eligibility → rank → tailor
→ morning artifact, one `runs` row) — as the unattended daily driver, mirroring the existing `scan`
launchd/cron/systemd examples. The scheduler itself is **operator-installed OS config, not product code** —
there is nothing to build in `src/`; launchd/cron/systemd are the mechanism, and the operator owns installing
and starting it against the **live** store. Recorded facts a scheduled run relies on, verified at the call
site: it exits 0 unless a fatal condition (an unreachable board / un-tailorable lead is a normal partial day;
a zero-output day is caught by the zero-output guard, not reported as success); it makes **zero network LLM
calls** by default (`pipeline/runner.py` passes no client to `run_tailor`); its first invocation migrates the
store to head — **not** via `build_context(ensure=True)` (`run_cmd.py` opens with `ensure=False`) but via
`ensure_schema` inside the scan lock (or directly on `--no-scan`), so the owed D-230 backfill self-applies;
and liveness checking is **on by default** (`--no-check-liveness` opts out).

**Alternatives rejected.** *A committed `.plist`/unit-file template* — a new `deploy/` tree departs from the
repo's established inline-README pattern for scheduling; copy-paste from the README is the reused mechanism.
*Building an in-`src` scheduler/daemon* — the OS scheduler is the right tool; a bespoke daemon is code to
maintain for no gain and contradicts reuse-first. *Running the daily driver against a copy* — copy-only is a
measurement guard against mutating the store during a benchmark; the whole point of operating is to run live.

**Consequence.** README gains a "Run the full daily pipeline unattended" section. Standing up the daily run is
now operational and Mit's to start. But see D-243: on Mit's specific instance, both résumé sources are
currently blocked, so the run cannot yet produce a clean day.

**Generalises.** *A "copy-only" safety practice for measurement is not an operating mode.* When a system has
never run in anger, the missing piece is usually the operator wiring, not more code.

## D-243 — Verifying the daily-run handoff on a live copy: D-230 is already applied, `doctor`'s exit is not a valid check, and BOTH résumé sources block a clean run

*2026-08-18. Findings from verifying the "stand up daily runs" handoff (D-242) against a byte copy of Mit's
live store and one live `--project` probe. Recorded because they correct STATE and gate the window's start.*

**Context.** Before handing Mit "schedule `boardwatch run`", the exact commands and the pipeline itself were
verified against a byte-identical copy of the live store (never mutating it), plus one live `--project` probe.
Four things came back different from what STATE and the session handoff assumed.

**Findings.** (1) **D-230 is already applied to the live store** — 0 rows match `status='running' AND
finished_at IS NOT NULL`, alembic is at `runs_status_backfill_repair` head, and `stats` migrates nothing.
STATE's "not yet run against Mit's live store / four stuck rows" was stale. (2) **`doctor`'s exit code is not
a D-230 check** — on Mit's store `doctor` exits 1 because of ~12 dead/error Workday boards (`report.actionable`),
with `integrity: ok`/`schema: ok` and no stuck rows; the earlier "`doctor` exit 0 = fixed" instruction can
never fire here. The honest read-only check is `sqlite3 "file:<db>?immutable=1" "SELECT COUNT(*) FROM runs
WHERE status='running' AND finished_at IS NOT NULL"`. (3) **`doctor` is not read-only** — it calls
`reap_stale_runs` and `probe_health`, both writes; `ensure=False` only skips migration. A "baseline" `doctor`
on live wrote board-health metadata (no data change). (4) **Both résumé sources block a clean run.** A dry-run
of the default path (authored `resume.yaml`) exited 1 fatal, 0 résumés: bullet `nio-1` is 241 chars (ceiling
220) and the full résumé overflows one page — `resume.yaml` is stale, the canonical bullets live in the
bundle (rev 22–24). The `--project` path also refuses: the current projection digest
(`sha256:87513b4d…`) has no approval (`missing_projection_approval`).

**Consequence / choice.** Nothing to do for D-230. Do not use `doctor`'s exit as an invariant check. **The P3
window cannot start until Mit takes one TTY action** (there is no `--yes`): either `profile-bundle
approve-projection` to clear the projection path, or re-sync `resume.yaml` from the bundle for the default
path — the résumé must not be hand-edited (D-155). STATE's D-230 row is corrected and a window-start blocker
row is added. `resume.yaml` is deliberately **not** hand-fixed.

**Generalises.** *A component's `ensure=False`/"read" label is not proof it does not write, and an exit code
that ORs several conditions is not a check for any one of them.* Verify the handed-over command's real
behaviour before shipping the instruction — the failure was silent-daily, exactly the class this program
exists to prevent.

## D-244 — The daily driver goes live: projection approved, launchd agent installed with an explicit homebrew PATH, and run 61 is the first clean unattended run

**Context.** D-243 left the P3 window blocked on one TTY action, and STATE's headline **0** mapped onto
"built but never run in anger." On 2026-08-19 Mit approved the projection on a controlling terminal
(`profile-bundle approve-projection` — there is no `--yes`, and `!` gives no controlling terminal, so this
gate is his alone), clearing the `--project` path. The approval was verified before anything was scheduled:
`run --project --no-scan --top 3` exited 0 with 3 one-page PDFs, each confirmed via `pdfinfo` (612×792pt),
not the run's own "N with PDF" line.

**Choice.** Schedule `boardwatch run --project` at 8:00 AM daily via a user LaunchAgent
(`~/Library/LaunchAgents/com.boardwatch.run.plist`, label `com.boardwatch.run`), logging to
`~/Library/Logs/boardwatch-run.log`. The plist carries an explicit
`EnvironmentVariables → PATH = /opt/homebrew/bin:/usr/bin:/bin:/usr/sbin:/sbin`. The scheduler and the
projection approval both live outside the repo (the config dir and `~/Library/LaunchAgents`), so this record
is the only committed artefact.

**Why the PATH is not optional.** launchd runs jobs with a minimal PATH that omits homebrew;
`reports/tailor.py` resolves both `tectonic` and `pdfinfo` via `shutil.which` / bare `subprocess.run`, and
both binaries live in `/opt/homebrew/bin`. Without the override the render step fails and the run produces an
empty day — the exact silent-daily failure D-204 named ("a machine with tectonic but no poppler got an empty
run every morning and the cause was never named"). `launchctl print` was used to confirm the job's
environment carries the override.

**Run 61 — the first clean unattended run.** Triggered through the installed agent (`launchctl kickstart`) so
it ran in launchd's real environment, not a shell. Exit 0; ~30 min (a one-time taxonomy re-extraction of
5,733 postings plus ~12 dead/auth-gated Workday boards timing out — 401/403/422/invalid-payload, all normal
partial-day noise; the run still exited 0). 15,366 seen · 26,997 open · 5,740 evaluated · 8 shortlisted ·
**8 tailored · 8 one-page PDFs** (each confirmed via `pdfinfo`), written to `~/boardwatch-applications/
2026-08-19/`. Liveness: 8 checked, 0 gone.

**Alternatives rejected.** D-243's other option — re-syncing `resume.yaml` — was rejected: the README is
explicit that there is no bundle→résumé bridge, so it would mean hand-editing, which D-155 forbids; the
projection *is* the bundle→résumé path. Leaving the plist on the README's PATH-less example was rejected for
the D-204 reason above.

**Open — Mit's.** (1) Whether a kickstarted run 61 counts as day 1 of Gate P3's 7 consecutive clean runs, or
the streak begins with the next scheduled 8:00 AM run. (2) Observed, not acted on: "Airbnb — Disaster
Response Coordinator" was shortlisted as SWE-eligible (a role-relevance false positive) and Snap "Level 5"
reads senior against Mit's new-grad target — both P4/craft + relevance territory, flagged not fixed.

**Generalises.** *A scheduled job's environment is not your interactive shell's.* An unattended runner that
resolves tools via PATH must be handed the PATH explicitly, and the only proof is running it once through the
real scheduler — asserting from a config dump is the same "component's self-report is not verification" trap
in a new place.

## D-245 — Run 61 ruled day 1 of Gate P3; the "run 61" number explained; nothing to capitalize from prior runs; and the root cause of two off-target leads

**Context.** After D-244, Mit asked three things: why the counter says "run 61" (were there 60 lost days?);
whether anything from the prior 60 runs is worth capitalizing; and why run 61 shortlisted two off-target
leads. Two read-only investigations answered them (live store `boardwatch.db`, queried `immutable=1`; no
`doctor`).

**Ruling.** **Run 61 is day 1 of Gate P3's 7 consecutive clean runs** (Mit's call). It ran through the real
launchd agent, on the live store, with no human in the loop — a kickstart trigger does not disqualify it. 6
to go; the next arrives with tomorrow's scheduled 8:00 AM fire.

**Finding 1 — "run 61" ≠ 61 days.** `runs.id` is a global counter incremented once per `boardwatch run`
invocation of any kind. Only ~7 of the 61 rows were real board scans (`boards_attempted > 0`): run 1 (08-04,
first inventory), a few on 08-05/06/07, and run 61. The other ~54 scanned zero boards — tailor-only tests,
the reaper, verify probes, the D-230 stuck/reaped failures, and a 36-run burst on 08-18 (the 3-lens
coverage-résumé dev work). The number reflects development, not missed workdays.

**Finding 2 — nothing to capitalize from the prior 60.** `applications` and `application_events` are both
genuinely 0 rows; nothing was ever applied. Prior output dirs (`~/boardwatch-applications/2026-08-05`, `-07`)
hold 8 PDFs, all stale on independent grounds: they are **Typst** (`.typ`), superseded by the current
**tectonic/LaTeX** render path; they carry pre-refinement bullets (bundle now rev 24), including a number
later found false (D-232); several were non-relevant roles (Deal Strategist, Asset Tracking Technician); and
the only good still-open leads they covered — Affirm 2012 (Collections) and Stripe 10947 — were already
re-tailored in runs 60/61 with the current bundle. **Verdict: start fresh from run 61's 8 leads.**

**Finding 3 — why the two off-target leads leaked (root cause, verified).** Role and seniority are filtered
**only at rank time**, never by eligibility (`rules.yaml` gates only work_auth / experience_years / clearance
/ degree / contract / internship — no role or seniority family, by design). Two rank-time gates:
`role_verdict` (`rank/role_gate.py:270`) → `swe`/`not_swe`/`uncertain`, and a case-folded substring veto
against the profile's `exclude_titles` (`rank/heuristic.py:144`), the *only* seniority mechanism anywhere.
- **Airbnb "Disaster Response Coordinator"** → `role_verdict` returns **`uncertain`** ("no role signal";
  "coordinator" is denied only when prefixed — office/facilities/operations), and the ranker
  (`cli/top_cmd.py:256-259`) drops only `not_swe`, **passing `uncertain` through** (fail-open by design). So
  it is not in the "4,898 non-SWE" drop bucket. Verified: `role_verdict("Disaster Response Coordinator")` →
  `('uncertain', 'no role signal in title')`.
- **Snap "Software Engineer, Specs, Level 5"** → `role_verdict` correctly returns **`swe`**; the live
  `exclude_titles` is `["Senior","Sr","Staff","Principal","Lead","Manager","Director","II","III",…]` — it
  enumerates seniority *words* but no **numeric leveling**, so "Level 5" (and every Level-5 iOS/Android/
  Backend variant) passes. Verified against the live profile row. The same blunt list is why `Sr` falsely
  vetoes `SRE` — it over-vetoes one scheme and under-vetoes the other.

**The fix fork — OPEN, owner's.** Presented to Mit; he was away, so it is unresolved, not decided. Per
multi-tenancy doctrine (what counts as senior/off-target is profile/field DATA; the classifier is code):
(a) **Data, per-user, zero code:** add `Coordinator` and `Level 4/5/6` (etc.) to `exclude_titles` via
`boardwatch profile edit` — immediate and reversible, but a blunt substring (collision risk, silent veto;
levels are company-relative — Google L3 / Meta E3 are *entry*, so never exclude those). (b) **Code, the real
fix:** a guarded bare-`coordinator` deny in `role_gate` (`uncertain`→`not_swe`) plus a dedicated **seniority
gate** parallel to `role_gate` that parses level tokens and compares to the user's target band, abstaining
when no level is stated — mechanism in code, "what's senior" as field data, "my band" as profile data,
measured against the module's eval corpus. This is P4 craft work, not a one-liner.

**Alternatives rejected.** Fixing role/seniority in `eligibility/rules.yaml` — rejected: eligibility
deliberately gates only hard bars; role and seniority belong at rank time. Acting on the stale Aug-5/7 PDFs —
rejected per Finding 2.

**Generalises.** *A fail-open `uncertain` bucket is invisible in a "non-X dropped" count* — the funnel's
"non-SWE" number hides everything the classifier was merely unsure about. *And a substring veto encodes
exactly one naming scheme:* it silently misses every sibling scheme (numeric leveling) while over-matching
unrelated tokens (`Sr`⊂`SRE`). A closed, typed catalog with an explicit abstain beats a substring list.

## D-246 — A rank-time seniority gate with a versioned, company-free leveling catalog; the owner's "company map in the registry" was not buildable and was replaced by user-config bindings

**Context.** D-245 left the fix fork open. Run 61 shortlisted two off-target leads, and the root cause was
structural, not a missing pattern: role and seniority are gated **only at rank time**; `role_verdict`'s
`uncertain` verdict passes through fail-open by design; and the *entire* seniority mechanism anywhere in the
program was a case-folded substring veto against the profile's `exclude_titles` list
(`rank/heuristic.py:144`). That list enumerates seniority *words* and no numeric leveling at all, which is
why "Software Engineer, Specs, Level 5" passed and why `Sr` falsely vetoes `SRE`.

**Choice.** Build D-245's option (b), the "real fix", in four parts:

1. **A guarded bare-`coordinator` deny** appended to `role_gate`'s `_DENY_BUSINESS_SOFT`, anchored by the
   same `_NOENG` lookahead the bare-`sales` pattern uses.
2. **`rank/seniority_gate.py`** — a new rank-time gate mirroring `role_gate` in shape and in discipline:
   ordered rules, pure functions, and every non-pass verdict carrying the text that decided it.
3. **`rank/leveling.yaml`** — a versioned catalog, split universal (level grammars, named rung ladders) /
   field-dependent (word→band and roman→band maps, keyed by career field), per the multi-tenancy doctrine:
   the taxonomy ships as data, never as code.
4. **`profile.target_seniority_band`** — an explicit closed-vocabulary profile field
   (`entry | mid | senior | any`) defaulting to `any`, on which the gate short-circuits. The gate therefore
   ships **inert**: nothing about ranking changes on upgrade until the owner names a band.

The fail direction is fixed by the keystone invariant. Only a confident word, roman numeral, or
bound-scheme hit may drop, into the new `hidden_over_seniority` bucket drained by
`top --include-over-seniority`. An unbound level token, a level outside its scheme's range, and every
ambiguous bare letter+digit token abstain — counted and reported as `uncertain_band`, never folded into
either neighbour.

**The owner's chosen option was not buildable — this is the substantive departure.** Presented with where
the company→level-scheme map should live, Mit chose "put it in the board registry". It cannot go there.
`src/boardwatch/registry/companies.yaml` is a **37-entry public seed catalog** whose model is
`extra="forbid"` (`registry/validate.py:19`), so a `leveling:` key cannot be added without changing a
validated public schema — and, decisively, **Snap, Twilio and Google are not in it**. They are boards *this
operator added*, which is precisely the point: the registry is a starter set, not a census, and growing it
to hold the companies one user watches so their ladders can be recorded is the "personal target list" R8
exists to refuse. Replaced by: the shipped catalog carries **named, company-free rung ladders**
(`ic_1_to_7`, `ic_1_to_6`) and **zero company names**, and the company→scheme binding lives in user config
at `{config_dir}/leveling-bindings.yaml`, keyed on `(provider, slug)` — the pair the store and the registry
already agree on. With no bindings file every level token abstains, which is the honest default.

**Alternatives rejected.**
- **A blind numeric floor** ("veto any Level/L/E/IC ≥ N") — rejected on measurement: it would have vetoed
  Cisco's "L2 - Routing", where the token is OSI layer 2. Most bare letter+digit tokens in the live corpus
  are not levels at all but OSI layers, support tiers and facility codes, so they are matched **only** so
  the gate can abstain loudly rather than ignore them silently.
- **Classify but never drop** (flag every over-band posting, hide nothing) — rejected: it leaves the
  shortlist exactly as crowded as before and moves the filtering work onto the operator, which is the job
  the program exists to do.
- **Deriving the band from `total_years_experience`** — rejected: it makes an editorial choice ("what am I
  targeting?") a silent function of a biographical fact, and it is wrong for anyone deliberately targeting
  below or above their years. The band is asked for explicitly.
- **Fixing this in `eligibility/rules.yaml`** — rejected again, restating D-245: eligibility gates hard
  bars only; role and seniority belong at rank time.

**Measured facts (2026-08-19, 26,997 live open postings).**
...
separate, larger work with its own measurement.

**Consequence.** `policy_version` re-keys once: the band and the catalog's digest enter `profile_row_hash`,
staling 11 live ledger rows, released with `ledger reopen --stale`. A second fix rode along: draining the
role-gate quarantine with `--include-non-swe` used to record the revealed rows `seen`, so looking into the
bucket suppressed them from later runs — the drain closed behind you. Drained rows are now excluded from the
`seen` write, and `--include-over-seniority` is built that way from the start.
...
## D-247 — The four open review findings on the seniority gate closed; the inert-gate probe now answers for the gate that would actually run

**Context.** D-246 shipped with `make check` never having passed end to end on `seniority-gate-spec`, and
four review findings open. Three were live defects, the fourth a dead field. Three of the four share one
shape: a second implementation of the gate's logic, written in order to *describe* the gate, that quietly
disagreed with it. In a branch whose whole discipline is reconciliation, that is the defect class to hunt.

**Choice.**

1. **`reports/stats.py` now counts in the ranker's ORDER.** `cli/top_cmd.py` gates sequentially — the role
   gate `continue`s before the seniority gate is ever reached — so a posting that is both non-software and
   over-band is `hidden_non_swe` and nothing else. `compute_stats` evaluated the two independently, so such
   a posting landed in *both* buckets and `stats.over_seniority` read higher than the funnel's
   `hidden_over_seniority` for the same corpus. `over_seniority` is now derived only for postings the role
   gate passed, which also spares a `seniority_verdict` call per non-software row. The new test counts the
   same corpus through `compute_stats` **and** `rank_open_postings` and asserts they agree — a component's
   self-report is not verification.
2. **`build_token_probe` returns a predicate, and answers for the ARMED gate.** Two divergences, one named
   by review and one not:
   - **Case sensitivity is now per branch.** `_PATTERNS`' bare-letter grammars and `_ROMAN` are
     deliberately case-SENSITIVE — lowercase `l2` is a network layer, `iv` a word fragment — and
     recompiling their *source* under one global `re.IGNORECASE` made all of them fire on lowercase. Each
     branch is now wrapped in a scoped `(?i:...)` only if its own compiled pattern carries the flag.
   - **The title is masked first**, exactly as step 1 of `parse_seniority` masks it. This was **not** in the
     finding, and it is the half that was actually firing. Measured over the same 26,997 open postings: the
     lowercase-token case has **0** live hits, while "Member of Technical Staff" accounts for **90** — every
     one of them `swe`-classified. The probe drives a notice telling the operator to *set a target band*, so
     every title it counts that an armed gate would ignore is a nag toward a setting that changes nothing.
     Agreement with `seniority_verdict` went from 90 disagreements to **0 of 26,997**.
3. **`_why_cell` accumulates every applicable drain** instead of returning on the first match. A row can be
   drained twice — `above_band` reaches `visible` only via `--include-over-seniority`, and a
   duplicate/applied/handled row via its own `--include-*` — and the band reason shadowed the other, so a
   row the operator had already applied to read as merely over-band. The latter three remain mutually
   exclusive by construction, so at most two annotations ever appear.
4. **`LevelingCatalog.version` is deleted.** It could only ever hold `LEVELING_VERSION`, because
   `load_leveling` refuses any document disagreeing with the constant; the version that genuinely travels is
   already inside `digest`, which is what the run identity consumes. Enforcement is unchanged and still
   covered by `test_version_mismatch_raises`.

**Alternatives rejected.**

- **Subtracting `non_swe` inside `summarize`** rather than ordering the evaluation in `compute_stats` —
  rejected: it leaves `PostingStat` able to express a self-contradictory row, and puts the ordering rule in
  a different function from the ordering itself.
- **Keeping the probe a `re.Pattern` and masking at the call site** — rejected: it would export
  `_mask_non_seniority_phrases` and let the next caller forget it. The predicate cannot be used wrong.
- **Fixing only the `IGNORECASE` case as the finding described it** — rejected once measured. It has zero
  live hits; stopping there would have closed the latent half and left the firing half in place, and the
  branch would have carried a "fixed" finding that changed no number.
- **Giving `LevelingCatalog.version` a reader** to justify keeping it — rejected: inventing a consumer for a
  dead field is exactly how a check that cannot fire survives review.
- **Widening `leveling.yaml`'s `fields.software.words`** while in this file — not attempted. Any addition
  requires enumerating and *reading* the `swe`-classified titles it would drop (D-246), which is a
  measurement session, not a cleanup.

**Generalises.** *A reported number must be produced by the same ordering, and the same flags, as the thing
it reports.* Findings 1 and 2 are one error at two altitudes: a reporting path that re-derives a decision
path's logic instead of re-using its shape. And the review that named finding 2 named the half with no live
hits — *a finding is a hypothesis about a defect, not a boundary around it*; walking the category around it
(D-246's own lesson about `staff` inside "Member of Technical Staff", now recurring in the counter rather
than in the gate) is what found the 90.

---

## D-248 — The launchd calendar trigger has never fired: run 61 was a kickstart, Gate P3 is 0 of 7 unattended, and nothing outside a run can see a missed one

**Context.** D-244 recorded run 61 as "the first clean unattended run" and D-245 ruled it day 1 of Gate P3's
seven. A sister-project review challenged the claim that the validated path and the scheduled path are the
same. Checking it falsified the premise underneath both entries.

**The measurement.** System TZ is CDT, and `runs` timestamps are naive UTC.

| Event | Local | Source |
|---|---|---|
| 08:00 calendar window | 08:00:00 | plist `StartCalendarInterval` |
| **plist file created** | **08:39:07** | `stat` |
| run 61 started | 08:40:17 | `runs.started_at` = `13:40:17` UTC |
| run 61 finished | 09:11:10 | `runs.finished_at` = `14:11:10` UTC |

The plist did not exist until 39 minutes after the window, and the run began 70 seconds after the plist was
written. `launchctl print gui/501/com.boardwatch.run` reports `runs = 1`, and the calendar event channel
reads `active = 0`, `watching = 1`. **The trigger has fired zero times.**

**Choice.** Record that the agent is *installed but never scheduled*. Gate P3 is **1 of 7 by Mit's ruling
(D-245), 0 of 7 unattended.** D-245's ruling is not reversed — a kickstarted run in launchd's real
environment is still a real run — but it may no longer be described as evidence that the schedule works.

**Alternatives rejected.** *Silently keep "1 of 7"* — the number is defensible, the words around it were
not. *Claim the code paths differ* — they do not. `launchctl kickstart` spawns the job from the same plist;
`launchctl print` confirms identical argv, PATH, domain, audit session and spawn type. The sister project's
framing was wrong and its conclusion right, for a different reason.

**Consequence.** Two gaps are now named. **(1)** The trigger is unproven; one real scheduled firing is the
next P3 action. **(2)** Nothing outside a run can detect a missed one. The funnel artifact already carries
the heartbeat payload (`started_at`, `finished_at`, lead counts), but `folders_reconcile` is called only from
`pipeline/runner.py:43` and `doctor` is manual — both require a run to happen. `runs` carries no
pid/heartbeat column (`core/settings.py:81`, `store/queries.py:156`). None of the other 24 LaunchAgents
watches boardwatch. An external staleness alarm is owed. Also corrected: `StartCalendarInterval` *does* fire
on wake, unlike cron, so "a missed window is lost forever" is false; what is true is no retry on failure,
one catch-up rather than one per missed window, and no alarm in either case.

---

## D-249 — Eligibility never decided because one profile field was unset, not because the design was wrong; setting it produced the first 1,650 ineligible verdicts, all span-backed

**Context.** Across 120,330 evaluations the engine had produced **zero** `ineligible` verdicts. Run 61 carved
26,997 open postings to 8 leads entirely with two title regexes and a cap; `hidden_ineligible` was 0. A
sister project read this as the precision machinery being decorative. It is — and the cause was one field.

**The mechanism.** Four facts compose:

1. `work_auth` is the only family whose default policy is `blocker` (`rules.yaml:81`), and only
   blocker + required + unmet yields `ineligible` (`engine.py:265-272`).
2. Mit's fact was `{"status": "ead_or_similar", "jurisdiction": "us"}`. The orthogonal third field
   `needs_sponsorship` (`facts.py:35`, `rules.yaml:93`) was **absent**.
3. `resolve.py:170-178` — when the bit is set the rule DECIDES: `UNMET` if true, `MET` if false.
4. `resolve.py:188-191` — absent, with `ead_or_similar`, returns `UNKNOWN`: "authorization is conditional; a
   sponsorship need cannot be ruled out."

So `work_auth:no_sponsorship_offered` fired on 1,696 postings and abstained on 1,696 — a 100% abstain rate
on the only rule able to stop anything. Meanwhile 424 `unmet` rows sat across the five `preference`
families, none able to reach `ineligible`.

**Choice.** Set `work_authorization.needs_sponsorship = true` (Mit's answer, 2026-08-19) and re-evaluate the
corpus. Result, with exact accounting:
...
`1,739 − 89 = 1,650`, and `eligible` did not move. **Verification through a second path:** all 1,692 unmet
rows carry a JD locator, 0 lack one, 0 rejections have zero unmet rows — and extracting the raw `body_text`
at every byte span shows **all 1,692 contain "sponsor"** (`'will not provide immigration sponsorship'`,
`'will not sponsor applicants for work visas'`). Zero span violations under 1,650 new rejections.

**Net effect is 401, not 1,650.** Of the rejections, 1,249 (75.7%) were already dropped by the title/role
gates; 401 were reaching the ranked pool, and only 43 of those were role-gate `swe`. This bought
correctness, not yield.

**Alternatives rejected.** *Make more families `blocker`* — `rules.yaml:869-876` measured contract precision
at 86% and internship at 100% against the providers' own structured field, so a blocker default would hide
about one real posting in seven. *Treat `uncertain` as a drop* — forbidden; abstain is never folded into a
neighbour. *Guess the value* — it is a fact about the owner, not the code.

**Consequence.** The keystone invariant is **vindicated and indicted at once.** It did exactly its job: it
surfaced a missing profile field as a 100% abstain rate in the per-rule table of every funnel artifact, every
run. Nobody read the table. The instrument worked and the dial went unwatched, which is a monitoring failure
of the operator, not the design. Still open: **17 of 44 rules sit at 100% abstain**, including
`experience_years:scoped_years_minimum` on all 16,007 observations; the artifact reports abstain **per rule**
but never aggregates **per family**; and `engine.py:265-279` still reaches `eligible` *by silence* — zero
detections means `any()` is False twice — so **41.3% of run-61 `eligible` verdicts carried zero requirement
rows**, against CLAUDE.md's own "'No flags' ≠ cleared". A zero-row `eligible` should abstain regardless of
why the chain is empty; that fix is owed.

## D-250 — A zero-evidence `eligible` now abstains to `uncertain`; the fix is label-only and rebaselines a third of the frozen corpus

**Context.** D-249 left this owed. The roll-up reaches `eligible` through two `any()` tests that are both
False when `rows` is empty (`engine.py:265-279`), so a body that fires no family in any of the six cleared
with an empty evidence chain. Measured on the current store (engine `1+2f3754409711`, post-sponsorship
profile): **11,158 of 26,997 postings (41.3%)** are `eligible` with zero requirement rows. `ineligible`
(1,650) and `uncertain` (89) carry zero such rows — only `eligible` is reached by silence.

**Three findings changed the decision before it was made.**

1. **Label-only, not lead-loss.** The ranker hides only `ineligible` (`top_cmd.py`, D-P2-10); `eligible` and
   `uncertain` are both candidate leads. Flipping a zero-row `eligible` to `uncertain` changes the label and
   the reason, not the shortlist.
2. **Not short boilerplate.** Of the 11,158, **98.0% (10,935) are truly thin** — no eligibility token
   matches any family anywhere — at a **median body length of 5,922 characters** (5,332 are ≥ 6,000). A
   JD-length guard would repair ≈1% of cases, so it was rejected; the empty evidence chain itself is gated.
3. **The frozen oracle certifies the old behavior.** `test_eligibility_corpus.py` — 987 cases reconciled
   against the reviewed prototype `proto.evaluate` — asserts **321 zero-row cases as golden `eligible`**
   (near-miss, negation, and known-extraction-gap bodies). `scratchpad/gen_corpus.py` was never committed,
   so the corpus is unregenerable via committed tooling.

**Choice (Mit's, 2026-08-19b/c).** Presented the three findings and the conflict; Mit chose **flip and
rebaseline the corpus** over keep-and-monitor. `eligible` now requires a requirement row **OR** a family the
profile's own policy (`ignore`) or field scope (`skip`) excluded that would have detected one
(`engine._no_evaluable_requirement`). So a user who opted out of the only applicable requirement keeps
`eligible` (multi-tenancy; the corpus's one `policy IGNORE` case, `p0002`, is preserved), while a genuinely
un-evaluable posting abstains to `uncertain`. For a default or all-`blocker` profile nothing is excluded, so
the guard short-circuits without a second `detect()` — zero cost on Mit's corpus.

**Companion change (required, not optional).** `count_eligible_judged_this_run` →
`count_candidate_judged_this_run`, now counting `eligible` **+** `uncertain`. The silent-empty-day guard's
predicate is "new candidate work produced 0 leads"; since `uncertain` is a candidate lead, and since the
*normal* thin posting now abstains rather than clears, counting `eligible` alone would silently shrink the
guard's coverage to nothing on a thin-only day. This preserves it.

**Alternatives rejected.** (a) A JD-length guard — 98% of cases are full-length. (b) Synthetic per-family
`unknown` rows so the report shows "six families abstained" — this corrupts the abstain report's
`never_fired` (rate `None`) vs `fully_abstaining` (rate 1.0) distinction, which is the keystone's own
monitoring signal; zero rows is the honest record and the new keystone test asserts the empty chain, not
fabricated rows. (c) Keep `eligible`, add a monitored bucket — the posting still reads `eligible`, so
"'No flags' ≠ cleared" stays unmet.

**Rebaseline.** 320 corpus cases flip `eligible`→`uncertain` (rows unchanged); `p0002` stays. Suppressor
tests in `test_eligibility_new_families.py` assert `uncertain` (or `!= "ineligible"` where one param keeps a
row); the `ELIGIBLE_NO_RULES_APPLIED` presentation test repoints to an `ignore`-based fixture. Verified by a
2,217-test pipeline+eligibility sweep, green.

**Still open (unchanged by this).** The per-family abstain aggregate, the 17 rules at 100% abstain, and
`experience_years:scoped_years_minimum` at 100% (D-249) are untouched.

## D-251 — The hard location gate becomes a US-only classifier; a real US detector replaces profile-substring matching, keep-on-doubt

**Context.** Mit ruled (2026-08-19) that the location filter is a **HARD, US-only** gate: any posting
outside the USA is invalid, and remote counts only if US-based. It is a visa requirement, not a preference.
The existing `location_filter_mode="hard"` could not do this. `passes_hard_filters` called
`location_fit`, which matched the posting's location as a bidirectional **substring** of the profile's
`locations` list, so it (a) dropped real US jobs whose string named no profile location — "Boston, MA"
does not contain "United States" — and (b) kept non-US remote, which scored a passing `0.5`. And the
default was `soft`, so no location veto had ever fired (the `hidden_hard_filter` bucket's second clause was
measured at 0 on 2026-08-06).

**Measurement (26,997 open postings; `locations_json`).** Only **7** have an empty location. A positive US
classifier (state abbrevs + names, "United States"/"USA", ZIP, a curated US-city list, US-scoped remote)
plus a non-US classifier (country names, a curated non-US-city list, macro-regions) labels: **US 61%
(keep), non-US 34% (drop), unknown 4.7% (keep)**. `remote_policy` is 73% `unknown`, so it is useless as a
location proxy — the location STRING is decisive.
...
**Fail direction — Mit's, measured before asked.** The ~4.7% unknown (bare "Remote", "Americas"/"North
America", rare foreign cities) is **kept** (fail-open), per "never silently delete a real job": a US-remote
role labeled only "Remote" still reaches him. Rejected: dropping unknown (strict) would lose ~1,100
postings including genuine US-remote.

**Alternatives rejected.** Flipping the flag to `hard` as-is (drops real US jobs, keeps non-US remote); a
non-US denylist (cannot be a hard gate); `remote_policy` as the US signal (73% unknown).

**Deferred (noted, not built).** A per-run count of location drops SEPARATE from title vetoes, and a
per-lead "location unverified" flag on kept-unknown postings — both want a report-only counter with several
mirror sites (the `uncertain_band` pattern), so they are a follow-up. The essential half of Mit's ruling —
KEEP the unclassifiable rather than drop it — ships now.

**Multi-tenancy limitation.** Hard mode is US-centric in v1; a non-US target would need its country's tokens
(the lists are data, so that is an extension, not a rewrite). Default stays `soft`, so no other user is
affected. **Activation:** `boardwatch config set location_filter_mode hard`, run only AFTER this merges —
setting it earlier runs the old substring gate and drops real US jobs.

## D-252 — The role gate's pre-sales / support / business-development consistency gaps closed

**Context.** An overnight analysis of the role gate's fail-open `uncertain` lane (≈67% of the scored pool)
found ≈half of it is non-software BUSINESS roles leaking in. The deny lists were narrowed aggressively over
time, and the proven `_NOENG`-anchored guard was applied to only two nouns (`sales`, `coordinator`). Three
inconsistencies remained: the gate denied "Solutions **Engineer**" but not "Solutions **Architect**",
"field support engineer" but not plain/prefixed "Support Engineer", and bare sales but not non-sales
"Development Representative/Manager".

**Choice.** Three soft-lane denies in `_DENY_BUSINESS_SOFT` (`rank/role_gate.py`) — `(solutions|enterprise|
customer|pre-sales|sales) architect`, `(business|partner|account|channel|corporate|market|revenue)
development (representative|manager|…)`, `(customer|technical|it|desktop|help-desk) support engineer` — plus
`SW Engineer` added to the software signal. The soft lane runs only when no software signal is present, so a
rescued "Software Architect" or any signalled IC title is never reached; "Software Solutions Architect" is
rescued software-first. **Measured over 26,997 open postings: swe 5,438 → 5,453 (+15 from SW Engineer, zero
software titles lost), ≈491 pre-sales/support/BD roles moved uncertain→not_swe; bare "Support Engineer"
stays `uncertain` (kept).** Structurally, a soft deny can never reach a `swe` title, so no false drop is
possible.

**Alternatives rejected / deferred to the owner.** P1 — `_NOENG`-guarding ALL business head nouns
(manager/director/specialist/…, ≈4,200 flips) — is a scope call (it hard-excludes every non-engineering
manager) and is left for Mit. Data Scientist / Data Analyst are left in scope (his Data lens). A body-text
role gate was measured and rejected historically (density can't separate the populations).

## D-253 — Structurally-undecidable rules reported apart from the fixable 100%-abstains

**Context.** 16-17 rules sit at 100% abstain. An overnight analysis split them into four kinds: fixable by
one profile fact (≈6 clearance rules, once `security_clearance` is set), fixable by a small code change (2),
structurally undecidable (the schema has no datum for them, for ANY profile), and correct conditional
abstains (4 work_auth rules). The report blended all four into one alarming number, masking the fixable ones.

**Choice.** A versioned `STRUCTURALLY_UNDECIDABLE` allowlist in `reports/abstain.py` naming the two
resolvers that abstain UNCONDITIONALLY — `experience_years:scoped_years_minimum` (no per-skill durations in
the schema) and `clearance:clearable_required` (no obtain-after-hire fact). Adds a `structurally_undecidable`
flag on `RuleAbstain` and `AbstainReport.structurally_undecidable` / `.fully_abstaining_fixable` properties;
the `eligibility abstain` summary now reads "N fire but never decide · M structurally undecidable", and the
per-rule table dims the schema-gap rows. **Additive:** `fully_abstaining` keeps its meaning and every
consumer, so nothing downstream changes.

**Alternatives rejected.** Deleting the two detections (they fire on purpose to keep the requirement row
visible and prevent an `eligible`-by-silence — D-250); changing `fully_abstaining`'s semantics (ripples to
the funnel JSON, the funnel display, and the CLI). The degree in-field rules are NOT on the list: they
decide UNMET for a lower-degree profile, so they are profile-dependent, not structurally undecidable.

## D-254 — The 08:00 scheduled trigger fired for the first time; Gate P3 accrual begins

**Context.** D-248 recorded the launchd trigger had fired zero times: the Aug-19 plist was written 39 min
after that day's 08:00 window and run 61 was a kickstart 70 s later (`runs = 1`). Nothing had proven the
schedule could fire on its own, and Gate P3 was 0 of 7 unattended.

**Outcome.** On 2026-08-20 at 08:00:22 CDT the trigger fired unattended — `launchctl` `runs` went 1→2 and
run 63 completed clean (exit 0, ~50 min, funnel RECONCILES, 8 leads / 8 one-page PDFs, all US-located under
the armed hard location gate). This is the first genuine scheduled run in the program's history and it
resolves D-248: the trigger was never *broken*, it had simply had no opportunity. **Gate P3 is now 1 of 7
UNATTENDED** — it needs 7 consecutive clean scheduled runs. The run also performed the owed post-#102 store
re-eval automatically (engine digest had moved).

**Still open.** No external staleness alarm for a MISSED window — a run must happen to notice one did not.
CI reliability is a separate carried gap (see D-259's note and the CI-flakiness memory).

## D-255 — Role gate hard-excludes non-engineering managers/directors and Data Scientist/Analyst (#106)

**Context.** The role gate's fail-open `uncertain` lane was large; owner review of run 63's ranked pool found
non-software management and data-science titles reaching the visible top. Mit ruled precision-first (never
tune `DEFAULT_TOP_N` — shrink the eligible set instead).

**Choice.** Two `_NOENG`-guarded SOFT-lane denies in `rank/role_gate.py`: `\b(manager|director)\b` and
`\bdata\s+(scientist|analyst)\b`. Both land in the audited `not_swe` lane; the `_NOENG` anchor spares every
engineering title (Engineering Manager, Director of Engineering; "Software Development Manager" is rescued
software-first), and the literal scientist/analyst token cannot touch "Data Engineer"/"Analytics Engineer".
TDD; `make check` green (6888).

**Note.** The manager/director half is **redundant for Mit** — his `exclude_titles` already
word-boundary-vetoes Manager/Director upstream of the role gate — but it is the correct GENERIC (multi-tenant)
form, and the Data-role half is not in exclude_titles and does help Mit.

## D-256 — Two eligibility rules that could never resolve MET, fixed (#107)

**Context.** Run 63's abstain table showed `degree:any_degree_required` (97 rows) and
`work_auth:sponsorship_available` (4 rows) at 100% abstain — structural defects, not conservatism.

**Choice.** In `resolve.py`: `any_degree_required`'s unleveled requirement (`bar is None`) routed every
degree-holder to UNKNOWN because the only MET line needs a numeric bar — a completed degree now clears an
any-level requirement (scoped to `implies == "degree_required"`, the sole unleveled degree pattern).
`sponsorship_available` read only `status` and ignored the P2a `needs_sponsorship` bit — it now mirrors the
unavailable branch's bit read. Both preserve the keystone invariant (absent field still abstains). One oracle
row (`m0105`) had encoded bug 1 in the answer key; corrected to met/eligible (matches m0088's shape) and the
corpus content pin re-recorded (987 rows unchanged). TDD; `make check` green (6878).

## D-257 — Clearance armed as a blocker: clearance-required jobs are removed, not demoted (Mit)

**Context.** ~138 clearance-required postings abstained in run 63 because `security_clearance` was unset. Mit
confirmed he holds no clearance and ruled that clearance-required jobs should be REMOVED, not merely demoted
— which resolves STATE open Q2 (a family other than `work_auth` becomes a blocker).

**Choice.** Set on the live store: `security_clearance = {state: none, level: none}` (both required —
`_incoherent_clearance` rejects state=none with a real level) via `eligibility facts set`, and
`eligibility policy set clearance blocker`. Verified in the store: fact `{level:none,state:none}`, policy
`{work_auth:blocker, clearance:blocker}`. The 11 required-clearance patterns now resolve UNMET → ineligible →
dropped. Severity stays user-owned (D-P2-1); another user keeps clearance at `preference`.

## D-258 — Seniority band = entry, and internships excluded via exclude_titles (Mit); armed by his TTY

**Context.** Mit targets new-grad/entry (0–2 yrs); open Q4 asked which band and he chose `entry`. He also
asked to exclude internships, warning of the job-apps trap: a JD BODY that says "internship experience counts"
must never reject a real full-time role.

**Choice.** Band → `entry` (activates the already-merged inert gate; ambiguous level tokens like "Level 3"
still ABSTAIN and pass through — company ladders are not guessed). Internships → title-based `exclude_titles`
additions (`Intern`, `Internship`, `Co-op`), NOT the eligibility engine: consulting job-apps confirmed intern
detection must be BY TITLE, and boardwatch's engine reads only `posting_versions.body_text` (no title column),
so its `internship_role_declared` is deliberately 100%-precision/~27%-recall and already suppresses the
"internships count" trap. `exclude_titles` (word-boundary, upstream of the role gate) catches the title-only
interns the body-only engine misses. Both `target_seniority_band` and `exclude_titles` enter
`profile_row_hash`, so the first run after arming re-keys `policy_version` (~11 ledger rows →
`ledger reopen --stale`). **Arming is Mit's one TTY act** (`profile edit`; no non-interactive path) — pending
at session end.

## D-259 — Role gate vetoes business/ops/admin/pricing titles that leaked into the ranked pool (#108)

**Context.** Run 63's ranked top 40 (the first clean scheduled run) carried ~28% non-software titles that
cleared every gate — Strategy & Ops (#2 overall), Business Operations/Partner, Stock Plan Administrator,
Trucking Pricing Associate.

**Choice.** Four SOFT-lane denies in `rank/role_gate.py`: `\bstrategy\s*(?:&|and|/)\s*(?:ops|operations)\b`,
`\bbusiness\s+(?:operations|partner)\b(?!.*\bengineer\b)` (engineer-guarded so a real "Business Operations
Engineer" is spared), `\bstock\s+plan\b`, and `\bpricing\s+(?:analyst|associate|manager|strategist|lead|
specialist)\b`. All SOFT-lane, so a rescued or signalled software title never reaches them. TDD; `make check`
green (6906).

**Deferred to owner (borderline, not in this PR).** Team Leader, Data Center Engineer, bare Administrator.
NOT excluding "User Researcher" — it overlaps real ML/Research *Engineer* roles, so a broad researcher deny
...
## D-262 — A manual verification run exposed a `Lead` role-gate hole the `exclude_titles` refine reopened (#114)

**Context.** With #111/#112 merged, a **manual** verification run — `run --project` on the editable venv,
identical argv to the launchd job but launched by hand, so **NOT a P3 unattended tick** (the launchd `runs`
counter is untouched; Gate P3 stays 1 of 7) — was run to watch every tightened gate (#106/#108/#111/#112 +
clearance/band) fire together on live data before the next scheduled run. It was clean (reconciles, exit 0,
~37 min, 8 leads / 8 PDFs, 0 fabrications) and confirmed the #108 win (Business Operations Associate gone). But
precision **regressed**: five of eight leads were non-software "Lead" business/ops roles — Affirm "Lead,
Technical Account Management" ×2, Airbnb "Programs Operations Lead", Instacart "Insights Lead" ×2 — which
crowded real software roles out under the top-N cap (2 US SWE leads vs run 63's 5). The two Affirm / two
Instacart entries were DISTINCT reqs, not a dedup miss.

**Root cause.** D-261's `exclude_titles` refine removed bare `Lead` (it over-vetoed product nouns). The
parallel `Manager`/`Director` removal was compensated by the role gate's `_NOENG + \b(manager|director)\b` deny
(D-255), but `Lead` was left with **no gate** — not in `exclude_titles`, not denied by the role gate — so a
business/ops "Lead" title passed both stages to the shortlist. In run 63 `exclude_titles` still vetoed them.

**Choice.** Add `lead` to the anchored `_NOENG` management deny: `_NOENG + \b(manager|director|lead)\b`, the
exact mirror of the manager/director rule. `_NOENG` spares any title carrying an engineering noun (Lead
Engineer, Engineering Lead, Lead Software Engineer), and the SOFT lane keeps rescued/signalled software titles
out of reach. New `TestBareLeadDeny` (RED-confirmed before the fix); `make check` green (6943 passed); merged
#114 after one tectonic-flake rerun on the 3.11 CI job (3.12/3.13 passed on the same commit — flake, re-run not
admin-bypass). **Rejected:** re-adding bare `Lead` to `exclude_titles` — it reintroduces the product-noun
over-veto #112 removed; the `_NOENG` guard is the correct generic (multi-tenant) form.

**Deferred (Mit's, next session).** GE HealthCare "Ingénieur logiciel … (Full Stack)" in Buc, **France**
reached run 65's shortlist: `Full Stack` rescues it as software, and the US-only location gate keeps it because
`Buc` is not in the positive US allowlist and fail-open (the visa ruling) never drops the unclassifiable.
Fixing it needs a non-US signal (city denylist or non-US-language cue) that does not fight fail-open.

## D-263 — The location catalog's alternation was never word-bounded, and the bug was intermittent

**Context.** Measuring the deferred Buc/France leak (D-262) meant classifying every open posting's
location. A new US-protection test — `Waukesha` must not be dropped — failed, and the cause was not the
change under test.

`_alternation` built `(?<![a-z])a|b|c(?![a-z])`. Alternation binds looser than concatenation, so the
compiled form is `((?<![a-z])a) | (b) | (c(?![a-z]))`: the lookbehind guards only the FIRST token, the
lookahead only the LAST, and every token between them matches as a bare substring. The module docstring
asserted the opposite ("Matching is word-bounded, so region token `uk` does not fire inside `Milwaukee`") —
it was documenting an intent the code never implemented.

**Measured damage.** Region token `uk` fired inside `Waukesha` and `West Milwaukee`, so the US-only hard
gate dropped **41 real GE HealthCare Wisconsin postings** as non-US — `Software Engineer`, `Senior Software
Engineer – Interoperability & Cybersecurity`, `Lead Application Engineer – MR Pulse Sequence` among them.
A false US drop is the worst error this gate can make, and these were Mit's target roles.

**Why it survived review: it was intermittent, not constant.** Which token lands last follows `frozenset`
iteration order, which varies with per-process hash randomisation. Over `PYTHONHASHSEED` 0–7, seeds 0–3 put
`eu` last (so `uk` was unbounded → `Waukesha` = non_us, dropped) and seeds 4–7 put `uk` last (→ `unknown`,
kept). **43 postings' drop decision differed between seed 0 and seed 4** — the same store and the same code
producing different funnel numbers run to run. Any test written against it passed or failed by luck.

**Choice.** Group the body: `(?<![a-z])(?:{body})(?![a-z])`. `Milano`, `Deutschland` and `Russian
Federation` were being dropped only by the same substring accident (`milan`, `eu`, `russia` inside them), so
each gains a real token — otherwise the fix would trade a false drop for a false keep.

`test_no_token_matches_inside_a_longer_word` pins the invariant across all seven catalogs and is
seed-independent: the ungrouped form leaves every token but two unbounded, so whichever ordering a process
picks, some token in each catalog fails. RED-confirmed by mutating a COPY of `src` (not the worktree) —
14 failures across 6 of 7 catalogs. Verdicts verified identical across seeds 0/2/4/6/9 after the fix.

**Found alongside, fixed separately in D-266:** `engine_version()` digested `ast.dump` output, which is
interpreter-dependent, so a Python upgrade silently re-keyed the whole eligibility ledger.

## D-264 — Three independent non-US signals close the Buc/France leak without touching fail-open

**Context.** D-262 deferred this to its own session. Run 65 shortlisted GE HealthCare "Ingénieur(e) logiciel
… (Full Stack)" in **Buc, France** at rank 16. `Full Stack` rescues it as software, and the US-only gate
keeps it because `Buc` is in no positive US allowlist and fail-open never drops the unclassifiable. The
fail-open is Mit's visa ruling and was not weakened; the leak needed a non-US signal instead.

**Measured first, on a 1.1 GB store snapshot (28,287 open postings, 3,484 ranked survivors).** 444 survivors
(12.7%) classify `unknown`. That bucket is mixed, and the mix is the whole difficulty: `Buc` (France) sits at
rank 16, but `Princeton University` and `Armonk` (both US) sit at 41 and 42, and `Dublin` is Ireland today
while Dublin OH is Cardinal Health's headquarters. Dropping `unknown` outright would remove 444 survivors at
a ~25% false-drop rate — priced and rejected, not proposed.

**Choice — three signals, each measured to fire on 0 US-classified postings:**

| | signal | drops | best rank |
|---|---|---|---|
| A | 57 unambiguous foreign city names + `Saxony`/`Thuringia`; `LOCATION_DATA_VERSION` 1→2 | 145 | **16** |
| B | structural ISO alpha-3 code — `BGR-Varna`, `VNM06-01-…`, `Remote (IND)` | 11 | 516 |
| C | `foreign_ad_gate`: DACH `(m/w/d)`, French `(H/F)`, `Ingénieur`, read off the TITLE | 30 | **16** |

Signal C is the only one that reaches three GE HealthCare postings whose `locations_json` is exactly
`["Remote"]` — no place named at all, so no catalog can ever help. It is a separate module, not folded into
`classify_location`, so that function's contract stays location-only; and it is gated on the location not
having confirmed US, so a US-located posting with a German title is still kept.

**Net: 299 corpus-wide drops (162 ranked survivors), 36 US false drops recovered (D-263), 280 of 444
`unknown` survivors still passing — fail-open intact.** No new drop bucket: both fold into the existing
`hidden_hard_filter` the funnel already reports (a new bucket has ≥27 mirror sites).

**Rejected.** (a) Dropping `unknown` — Mit's ruling, and 25% false. (b) Demoting `unknown` below `us` in the
ranker instead of dropping: with 3,040 US survivors and a cap of 8 this is a drop by another name, and worse,
a silent one — `hidden_hard_filter` reports a drop, a demotion reports nothing. (c) 2-letter country codes —
they collide with 51 US state abbreviations (`IN` is Indiana as often as India) and with department/compass
prefixes (`IT -`, `SE -`), for a yield of 2 postings ranked 3158 and 3396. (d) A German role-noun list —
measured; every token either never fired or was already caught by the gender marker, and a bare `koch` would
have fired on "Koch Industries". (e) A bare-umlaut rule — 0 US hits observed but too weak a mechanism.

**Deliberately left leaking, per Mit's ruling on the ambiguous names:** `Dublin` (23 Irish survivors, one at
rank 48), `Limerick`, `Birmingham`, `Uxbridge`, `Abingdon`, `Cambridge`, `Warren`, `Ontario`, `Valencia`,
`Moscow`, `Zwolle`. Each has a US namesake a posting could plausibly carry, and a silent US drop is worse
than a foreign keep. The rejected list and the "name the US employer it would cost you" test now live in the
`location_data` docstring so a later pass does not "complete" it.

**Recorded exposure, not widened.** US EEO strings are also slash-separated. `(M/F/D/V)` and `EOE M/F/D/V` do
not match, but a bare `(M/F/D)` or `(M/F)` in a title would; the corpus holds zero such US titles (all 28
`M/F`-shaped titles are German postings) and the `!= "us"` guard bounds it further. Pinned by
`test_the_us_eeo_string_does_not_fire`.

## D-265 — The funnel's `hidden_hard_filter` note denied the number printed beside it

**Context.** The drop note read "excluded title; ALSO a rejected location when `location_filter_mode` is
`hard`, which is not the default and **has never been measured firing**". Hard mode has been set since
D-251, run 63 confirmed it firing, and in run 66 the bucket carried **17,189 drops**. STATE had already
flagged the sentence as a stale template string; it survived because nothing tests report prose.

**Choice.** Say what is true, and cover the second axis D-264 adds: "excluded title; ALSO a non-US location,
or a non-US job-ad convention on the title, when `location_filter_mode` is `hard` (not the default)". The
"(not the default)" clause is kept because the funnel is multi-tenant and soft IS the default for other
users; what is dropped is the claim about measurement. A reader reconciling 17,189 is no longer sent looking
for a location that was never the reason.

**Not done.** The note still does not name the mode that was actually active on the run. That would need the
mode threaded into `Drop`, which is a wider change than a false sentence warrants.

## D-266 — `engine_version` was interpreter-dependent, so a Python upgrade re-keyed every verdict

**Context.** Found while investigating an apparent rules-hash shift during D-264's measurement — which
turned out to be my own artifact (two venvs on different Pythons), but the underlying cause was real.

`digest_of_sources` hashed `ast.dump` output. Python 3.13 changed `ast.dump` to omit fields holding their
default, so 3.12 emits `args=[], keywords=[], type_params=[]` where 3.13 emits nothing, and `type_params`
did not exist before 3.12. Byte-identical source therefore digested to **three different values**:
`6f9feb84bfee` (3.11), `a1d0be72a338` (3.12), `7e88ed2b193d` (3.13).

`engine_version` keys the ledger and the deterministic anti-join only reuses a verdict at a matching
version, so a Python upgrade silently re-evaluated all ~29,000 postings, and the three CI interpreters each
computed a different engine version for one commit. Fail-safe in direction — it re-evaluates rather than
serving a stale verdict — but the digest is meant to track semantics, and the interpreter is not semantics.

**Choice.** `canonical_dump` walks `node._fields` directly, so `ast.dump`'s defaults stop mattering. Empty
lists are skipped, which is what absorbs a grammar field a later version adds but this code does not use
(`type_params` is the live case). `None` is KEPT rather than skipped, so `Constant(None)` — the literal
`None` — stays distinguishable from a node with an absent optional. Comments and formatting stay invisible,
which is the property D-P2-22 wanted from `ast.dump` in the first place. Verified: the new sources digest to
`63c6f8fd5a3e` on 3.11, 3.12 and 3.13 alike, while the old path still gives three different values.

**Rejected.** (a) Hashing raw bytes — re-keys the corpus on every comment edit, which is exactly what
D-P2-22 chose `ast.dump` to avoid. (b) `ast.unparse` — normalised source, but its own output drifts between
versions (f-string handling changed in 3.12). (c) Recording it and moving on — Mit's call was to fix it.

**One-time cost, flagged.** The digest necessarily changes, so the next run re-evaluates the corpus once and
a `ledger reopen --stale` may be owed afterwards, exactly as D-258's profile re-key was. Paying it once
replaces paying it on every future Python upgrade.

Three new tests pin the mechanism rather than the constant: no `=[]` survives in a dump, a node carrying an
extra empty-list field serialises identically to one without the field, and `x = None` stays distinct from
`x = 0`. The four existing digest properties (comment-insensitive, formatting-insensitive,
semantically-sensitive per module, SyntaxError not swallowed) all still hold and were re-run.

## D-267 — A "ranked pool" measurement was the word "bucket", and the funnel cannot evidence a lead's location

**Context.** D-264's session recorded that run 66's ranked pool "still had `Buc` 4×", read off
`grep -ic buc funnel-66.json`, and passed that to the next session as the check to repeat on run 67.

The count does not depend on `Buc`. It is the substring inside "bucket"/"buckets" in the funnel's own
stage prose. Measured across every funnel ever written: runs 61, 63, 65 and 66 all return **4**, and the
older `artifact_version` files return **2**. Run **65** returns 4 while its shortlist genuinely carried the
GE HealthCare role in **Buc, France** — the number is identical whether the city is present or absent.
Run 67 returned **6**, up from 4, with no `Buc` anywhere: the metric tracks how much explanatory prose the
artifact contains. A case-sensitive `\bBuc\b` is **0** in every funnel including run 65's.

Two structural reasons, and they are the point of this entry:

1. The funnel records **no ranked pool**. It carries `leads` plus aggregate stage counters. Nothing between
   "considered" and "shortlisted" is enumerated, so no grep of the artifact can describe the pool.
2. A `leads` record carries **no location**. The fields are `posting_id, title, company, provider,
   board_slug, company_source, out_dir, pdf_built`. The run-65 France lead is identifiable in the artifact
   only by its French *title*.

**Choice.** Retract the claim at both sites in `METRICS.md` and drop the derived instruction from
`STATE.md`. The pool question is answered by running the production predicate
`rank/heuristic.passes_hard_filters` against the store — the route D-264's own measurement used, and which
reproduces independently: **0 of 62** open postings naming Buc or Ingenieur survive the hard gate,
including the run-65 leak (posting 31365).

**Rejected.** Grepping for `Ing` instead. It works today — 1 in run 65, 0 in 61/63/66 — but it is the same
...
alone**; location has no equivalent. Every "all leads US-located" claim in `METRICS.md` — run 63's at line
5009, run 67's here — came from a by-hand store read inside a session and cannot be reproduced from the
artifact afterwards. Closing it means adding `locations` and the `classify_location` verdict to `Lead`, at
the cost of an `artifact_version` bump. **Not taken unilaterally: it is a shipped-schema change.**

**Not a defect in the gate's tests.** Mutation-tested this session against a `src` copy, since three
overlapping non-US signals is exactly the shape where each clause goes individually untested. It does not
happen here. Removing only the `buc` token fails exactly one named test
(`test_unambiguous_foreign_cities_are_non_us`); emptying `NON_US_CITIES` fails 37; emptying `NON_US_ISO3`
fails 11; forcing `has_non_us_ad_marker` to `False` fails 12. Each clause is load-bearing and guarded.

## D-268 — Run 68's six reopened leaks are blocked before the tick, and D-267's own denominator was unpinned

**Context.** The 2026-08-21b session reopened 16 permanent decisions (`ledger reopen --stale`, D-266's
fingerprint re-key), so 16 postings re-enter the next run's shortlist. Six of them are the exact postings
that previously defeated the gates: GE HealthCare's `Ingénieur(e) logiciel en imagerie médicale (Full Stack)`
in Buc, France (posting 31365, D-264) and run 65's five non-SWE `Lead` roles (D-262). The scheduled 08:00
tick that becomes run 68 was ~19 h away, so the verification could not be *observed* — but it could be
**predicted**, against the same corpus run 68 starts from (30,243 open postings, unchanged since run 67).

**Choice: pre-verify through the production gate chain rather than wait for the artifact.** Each of the 16
reopened postings was run through `passes_hard_filters` → `role_verdict` → `seniority_verdict` with the real
profile (`profile_view_from_row`), the real catalog (`load_leveling` / `resolve_schemes`) and the real
`location_filter_mode=hard`. **All six leaks are blocked; the other ten surface.** Five die in the role gate
(`Lead` / `Programs Operations Lead` / `Insights Lead`, D-262's `_NOENG+lead`) and posting 31365 dies in the
hard filter (`classify_location` → `non_us`, D-264). The ten survivors are exactly the legitimate SWE set
Mit accepted when reopening (Stripe ×4, Reddit, Ramp, Roblox ×2, Snap, T-Mobile), including Roblox
`Data Center Engineer` and Snap `Level 5`, both present by owner decision rather than by defect. This makes
tomorrow's check a confirmation (`runs` 3→4, and the six absent from `funnel-68.json`'s leads) instead of an
investigation.

**Corrects D-267's citation: "0 of 62 Buc/Ingenieur open postings survive".** The **0 is robust** — it holds
under every match rule and corpus scope tried: word-boundary `buc|ingénieur` over open postings (27 matched),
the same over all statuses (27), and both widened with the D-264 ad-gate title signals `(m/w/d)`/`(H/F)`
(69 open, 70 all). **The 62 is not reproducible**, because the match rule and the corpus snapshot were never
recorded beside it; D-264 measured against 28,287 open postings and the corpus is now 30,243, and the
likely rule (Buc + Ingénieur + ad signals) yields 69 today. The number was probably right when taken. The
defect is that a ratio was recorded without the two things that make it reproducible.

**And the match rule is load-bearing, not a detail.** Under a bare substring the answer is not even 0:
`buc|ingenieur` unbounded matches **103** open postings of which **39 survive the hard gate** — every one a
US `Starbucks Barista`/`Starbucks Team Leader` posting, plus `Bucharest`, `Roebuck`, `Buckhead`, `Bucerias`
and `Betriebsingenieur` in the matched set. D-267 retracted a metric for counting the word "bucket"; the same
family of error was still sitting inside its replacement's denominator.

**Rule adopted:** a ratio in `METRICS.md` records its **match rule** and its **corpus size** beside it, or
only the numerator is quotable later.

**The reader route matters mid-run, and the obvious one fails.** `sqlite3` CLI with `?mode=ro` cannot open
the store at all when it is cleanly checkpointed — no `-shm` exists, a read-only connection may not create
one, and the result is `SQLITE_CANTOPEN (14)`, which reads exactly like a missing file or a sandbox denial.
It is neither, and it is not the Bash sandbox: it fails with sandboxing disabled too. `?immutable=1` works
but skips the WAL, so it is **stale against a live writer** and must never be the mid-run probe. **Python's
`sqlite3` with `?mode=ro` works in both states** and is therefore the only route that answers
`SELECT COUNT(*) FROM eligibility_evaluations WHERE run_id=68` while run 68 is writing. It creates a
zero-length `-wal` and a `-shm` beside the store, which is a transient artifact, not a content change.

**The raw-SQL/tool discrepancy is now explained, not just warned about.** `job_dispositions.reopened_at`
holds 16 rows dated 2026-08-21 and 6 dated 2026-08-20; `reopen_jobs` sets the timestamp rather than deleting
the row, so a raw `COUNT` over non-null `reopened_at` returns **22** — the 16 of this drain plus 6 of run
65's still-visible history. `ledger show --stale` correctly reports 16, and now reports `no stale decisions`,
so the drain owed after D-266 is closed.

**Rejected.** Reading the run log to decide whether the schedule fired (only `launchctl`'s `runs` counter is
authoritative — runs 65 and 67 were manual and moved nothing). Building `locations` onto `Lead` to close
D-267's artifact gap unilaterally: it is a shipped-schema change with an `artifact_version` bump, it stays
Mit's, and it was re-raised this session. Deleting either store backup or `spike/p0-5-resume` to reclaim
disk: 12 GiB free is enough for one run and both deletions are irreversible.

## D-269 — The nightly's three root causes, and the WAL one was a real defect hiding behind a flaky test

**Context.** The nightly was carried as "two intermittent flakes" (tectonic network, two-writer
concurrency). It had failed **7 of its last 8 scheduled runs**. Ubuntu passes every time, so every cause
lives in the jobs only the schedule runs — which is also why `make check` stayed green locally throughout.
Three distinct causes, found by reading the failed job logs rather than the carried description.

**1. WAL was never established when a store was created — a production defect, not a test artifact.**
`get_engine` is lazy, so it sets no pragmas; `ensure_schema` runs alembic, and `_alembic_config` hands
alembic a URL from which it builds its **own** engine, so the `connect` listener that sets
`journal_mode=WAL` never fires. A freshly created store is therefore in **`delete`** mode, and the switch to
WAL is deferred to whichever connection happens to run first. That deferred switch is a journal-mode
**conversion**, and no lock held by any other connection permits one. Measured directly against a
`delete`-mode database:

| Competing lock | `PRAGMA journal_mode=WAL` |
|---|---|
...
So two processes opening a fresh store race, and the loser cannot open it at all. Fix: open one connection
through the instrumented engine before alembic runs. Then every later `PRAGMA journal_mode=WAL` is the
cheap no-op it was always assumed to be — verified both ways (`delete` + competing writer ⇒ raises; `wal` +
same writer ⇒ returns `'wal'`). **Mit's live store already reads `wal`, so no existing store needs
migrating**; the defect was in creation only.

**A falsified hypothesis worth recording, because it was the obvious one.** The first theory was that
`_set_pragmas` runs `busy_timeout` *after* `journal_mode=WAL`, leaving that statement on SQLite's default
timeout of 0. The test written to prove it **passed against the unfixed code**: pysqlite's
`sqlite3.connect` already arms `timeout=5.0`, so a 5000 ms busy timeout is in force before any pragma
runs. Writing the test first is the only reason a plausible, wrong fix was not shipped. Residual wart, NOT
fixed here: `get_engine(busy_timeout_ms=N)` cannot lower or raise the timeout for statements inside
`_set_pragmas` itself, since pysqlite set 5000 at connect; threading it through `connect_args` would be
more honest and is now harmless either way.

**2. Five Windows `fs_safety` failures, deterministic on all three Windows jobs, every night.**
`_fstype_for_path` resolves its argument with `os.path.realpath` before comparing against mountinfo. On
Windows that rewrites `/data` to `\data` (proved locally through `ntpath`, the real Windows path module),
which can never match a POSIX mount point, so every case collapses onto the root mount: the ones expecting
a fstype fail, and the ones expecting `None` **pass for the wrong reason** — they would pass against an
empty catalog. Production is unaffected: Windows has no `/proc/self/mountinfo`, so `unsafe_wal_filesystem`
returns before comparing any path. Fix: skip the eight POSIX-addressed cases on `win32`, keeping the three
platform-independent ones (the `get_engine` wiring and the no-mountinfo case) running everywhere. Verified
as **3 passed / 8 skipped** with the condition forced. Rejected: a whole-module skip (loses the three that
work, including the guard's own wiring); rewriting `_fstype_for_path` to POSIX-pure semantics (changes
production to suit a test, and on Linux `os.path` *is* `posixpath`, so it buys nothing).

**3. The tectonic cache could never grow past the warmup, so every run needed the network.** The warmup
compiled `\documentclass{article}` + `x`. The résumé template pulls `fontspec`, the Latin Modern OTF faces,
`titlesec`, `tabularx`, `marvosym`, `hyperref`, `babel` and more, none of which that document fetches — and
`actions/cache` **only saves on a miss**, so the partial bundle written by the very first run was restored
unchanged forever after. Every run therefore reached the network for the same missing files, and one CDN
hiccup took **~52 render tests** with it. Fix: derive the warmup from the template's own preamble
(everything above `\begin{document}`, which is placeholder-free), assert `fontspec` survived the
extraction so a stub bundle fails loudly, and bump the cache key so the fuller bundle is saved once. The
step was extracted from the YAML and run verbatim before committing. Cannot drift from what the tests
compile, which is the same rule already applied to fixtures (D-228).

**Verification.** `make check` green on the merge: **7061 passed** (7060 + the new WAL regression test), 4
xfailed. **Windows and macOS evidence can only come from CI** — Windows never runs on a pull request
(D-151/D-212) — so this is confirmed by a `workflow_dispatch` of `ci.yml` and then by the scheduled
nightly, not by the PR's own checks.

**Rejected.** Treating any of the three as "a flake to re-run": a 7-of-8 failure rate is not an
intermittency, and `gh run rerun --failed` on a deterministic Windows failure can never go green.
Admin-bypassing the merge (Mit's standing ruling).

---

## D-270 — The daily new-posting rate measures our own fetch budget, and 15,535 listed postings were never materialized

**Context.** Mit asked where boardwatch stands on job discovery against job-apps. The comparison was
answered from live data on both sides, and the answer carried a caveat: that boardwatch's 2,300–4,300
"postings first seen per day" was probably inflated by the same job re-keyed under a new provider id,
because `jobs` and `postings` both stood at 34,367 and grouping was therefore collapsing nothing. Mit
asked for that caveat to be investigated rather than carried. **It was wrong about the cause.**

**Falsified: re-keying is a small fraction, measured with its match rule and corpus size (D-268's rule).**
Corpus 34,367 postings / 30,243 open. Of the 2,323 rows first seen 2026-08-21, the number sharing a
`posting_identities` key with a *strictly earlier* posting was **176** on `company_title_location` (7.6%),
**182** on `content_hash_only` (7.8%) and **37** on `exact_quad` (1.6%). On 2026-08-20: 281 / 315 / 51 of
4,285. So ~92% of each day's rows are new to the store under every available key. The caveat is retracted.

**Root cause: `detail_fetch_budget`, default 50, is a per-board-per-run cap on materialising unseen
postings** (`core/settings.py`, threaded through `scan/coordinator.py` into each provider's
`BoardRequest.detail_budget`). The daily count is therefore a statement about boardwatch's throughput, not
about the market. The arithmetic is exact and is what makes this conclusive rather than plausible: **19
Workday boards hold exactly 600 rows each**; each gained **exactly 100** on 2026-08-21 (2 runs: 66, 67) and
**exactly 150** on 2026-08-20 (3 runs: 63, 64, 65) — 50 × runs, the identical figure across companies with
nothing in common. Run 67's carve reconciles to the funnel's `postings_seen` of 7,762: 44 `complete` boards
listed 6,762, 20 `partial` listed exactly 1,000 (20 × 50), 59 `unchanged` and 12 `failed` listed 0.

**The finding this exposed: a backlog of 15,535 listed-but-unmaterialised postings**, across those 20
Workday boards (Citi 1,614, Target 1,572, T-Mobile 1,552, NVIDIA 1,511, Wells Fargo 1,479, Capital One
1,368, Salesforce 1,082, then a tail to Fidelity 104). It is recorded **only as English inside
`board_scans.error`** — "detail budget of 50 exceeded (N unseen postings); raise detail_fetch_budget or
rescan" — so reading it needs a regex over a prose column, and the funnel reports `postings_seen` while
never reporting the deferred remainder. This is the same shape as D-267: a quantity that decides how much
of the market we can see, leaving no first-class trace in the artifact that reports the run. Standing
beside it: **12 boards fail outright**, all Workday, all HTTP 401/403/422 (Snowflake, Veeva, Riot Games,
Rice, Broad Institute, Texas Instruments, Walmart Global Tech, Pure Storage, Splunk, Informatica, DocuSign,
Roku), and **17 registered companies have never produced a single posting**.

**`jobs` == `postings` is a POLICY choice, not a broken grouper.** Only `exact_quad` merges — 96 events, all
method `exact_quad`, all `algorithm_version` p6.2 (89 on 08-19, 5 on 08-20, 2 on 08-21). The looser keys are
computed and stored and never merge. Postings per job: 34,194 jobs hold 1, and 64/8/4/1 jobs hold 2/3/4/5;
no posting has a null `job_id`; the 96 remaining rows in `jobs` are the emptied `from_job_id` of each merge.
`posting_identities` holds 168,943 rows over five kinds, and the keys shared by more than one posting whose
postings sit in **different** jobs are 1,118 (`content_hash_only`, 3,523 postings), 1,062
(`company_title_location` and `cross_host`, 2,546) and 285 (`exact_quad`, 619). **Gate P6's duplicate-leakage
clause is therefore answerable with a query against a table that already exists** — it needs a definition of
which kinds count as a duplicate, not new code.

**Verified NOT a defect: budget-skipped postings are not falsely closed.** Every subset-detail provider
keeps the full live listing in `snapshot.listed_ids` (`core/models.py`) computed *before* the detail phase,
and `scan/apply.py` calls `_process_missing` only when `snapshot.status == "complete"`. Confirmed through a
second path, as a component's self-report is not verification: **0** postings are closed on any of the 20
partial boards.

**Rejected.** Raising `detail_fetch_budget` in this session — it adds input, which `CLAUDE.md`'s
breadth-is-last rule puts behind proven conversion, and it lengthens a run that already takes 42m41s
against a 30,243-posting corpus; it is Mit's call, not a tidy-up. Making the grouper merge the looser keys —
that changes which posting is canonical and therefore which lead carries a suppression, and it is exactly
the "new bucket at a call site" the catalog rules forbid. Repairing the 12 failing boards as part of a
documentation session. Rewriting the funnel's dedup-stage note in `reports/run_funnel.py`: it says the
*stage* counts nothing, which is still true — the measurement above was taken out of band.

**Owner-gated, all three (input-side or shipped-artifact changes).** Whether to raise the budget and how
far; whether the deferred backlog becomes a first-class funnel counter beside `postings_seen`; whether the
12 dead Workday boards are repaired, re-slugged or dropped from the registry.

---

## D-271 — boardwatch cannot see 92% of what job-apps surfaces, and Workday's own total is censored

**2026-08-22 · a measurement session. No code changed.** Mit: *"I need to know with confidence and clarity
that it can replace job apps and i dont have any insecurity that it does not miss any jd's job apps wont."*

**This reopens D-008**, which retired the pre-registered parity comparison and made the bar absolute;
`PROGRAM.md` §4 still lists "Statistical parity testing vs job-apps" under *Deliberately NOT doing*. The
measurement below is that comparison. Recorded as a reopening, not slipped in — the ruling is Mit's.

### The answer

**41 of 530 (7.7%)** of job-apps' eligible records over 2026-08-12…08-21 are at a company boardwatch
watches. The set spans **352 distinct companies**; boardwatch watches **24**. Match rule: company name
lowercased, punctuation stripped, corporate suffixes removed, whitespace removed, matched against both
`companies.name` and the leading label of `companies.slug` over all 135 `watched=1` rows (159 keys). Corpus
size stated per D-268. Largest missing: Amazon 25, TikTok 20, AWS 8, Apple 7, ByteDance 7, SpaceX 6, IBM 5,
Vanguard 5 — **none of them uses any of the six supported ATS platforms**, so adding a slug cannot reach
them; each needs a first-party adapter.

**No 135-board hand-curated list covers 352 companies.** The insecurity is correct, and the cause is the
architecture, not a defect.

### Lane value measured by loss-if-removed, not by frequency

| lane | eligible records | lost if removed |
|---|---:|---:|
| commercial aggregators (LinkedIn, Indeed, hiring.cafe) | 446 | 421 |
| GitHub new-grad lists | 103 | **73** |
| direct ATS (the six providers) | 14 | 5 |

Cross-lane overlap is **5.8%** — the lanes are non-redundant, so no lane substitutes for another. The
GitHub lists (SimplifyJobs, jobright, vanshb03, speedyapply) carry **19.1% of the entire eligible yield for
about five HTTP GETs against public repositories**. The v2 decision excluded "aggregators" on ToS and
anti-bot-treadmill grounds — an argument strong for LinkedIn and Indeed that does not obviously reach a
public GitHub JSON file. They were swept up by category.

**Counterweight, recorded so the gap is not overstated:** roughly a fifth of job-apps' yield is staffing
firms (Infosys 5, TCS 5, BeaconFire 5, Capgemini 3, UST 2) and artifacts (`Jobright.ai`, `RemoteHunter`,
`Sapphire Partners`, `Stealth Startup`, a literal `↳`) that boardwatch's gates would reject.

### Where boardwatch is better, and it is not close

Greenhouse/Lever/Ashby: both systems fetch a whole board un-paginated, but job-apps runs
`_is_relevant_career_page_title` **inside the fetch loop** and discards non-SWE before storing, so
boardwatch holds strictly more. Workday: job-apps does not enumerate, it *searches* —
`WORKDAY_SEARCH_QUERIES` (3 fixed strings) × `WORKDAY_MAX_PAGES_PER_QUERY` 2 ×
`WORKDAY_MAX_DETAILS_PER_COMPANY` 12, producing **77 roles over 39 boards on 2026-08-21 with 16 boards
returning zero**. boardwatch enumerates the board. The gap is *which companies are reachable*, never fetch
depth on a shared board.

### New defect — Workday's `total` is censored at 2,000; the facet counts are not

Probed live, one POST per board with the body `_search_body(0)` already builds:

| board | `total` | facet sum | held | coverage |
|---|---:|---:|---:|---:|
| Citi | 2,000 | **4,589** | 600 | **13.1%** |
| NVIDIA | 2,000 | **2,656** | 600 | 22.6% |
| Adobe | 740 | 740 | 600 | 81.1% |
| Intel | 645 | 645 | 600 | 93.0% |
| Regeneron | 592 | 592 | 600 | 101.4% |
| Fidelity | 565 | 565 | 600 | 106.2% |

**The known-positive control passed** — on every uncensored board the facet sum equals `total` exactly, so
this metric can agree when it should and is not the unfailable kind that produced D-267 and killed
`SourceTotal` (D-028). Consequence: boardwatch's pager *also* wraps at ~2,000, so after the backlog drains
Citi stays at ~2,214 of 4,589 — **a permanent, invisible hole that is not the detail budget.** Mirror
defect: coverage above 100% (Regeneron, Fidelity) means we hold postings the board no longer lists, because
a permanently `partial` board never runs `_process_missing`.

### D-270 confirmed against a contrary claim

A code audit this session claimed the budget is not a queue: Workday serves newest-first, `unseen[:50]`
re-takes the same prefix, tails are never read — inferred from Citi's 2-day max capture age.
**Falsified.** Every board's backlog falls monotonically 26–49 per scan with no exceptions and 15 boards
have drained (35 → 20). `posted_at` shows tails *are* being read: Databricks reaches 2019-11, Cisco
2025-12, Adobe 2026-03. Citi is an outlier, not the pattern. **ETA: 48 more runs (~7 weeks at 1/day).**

### Other coverage findings

- **5 boards report green and return zero, ever** — Snyk, Vercel, HubSpot, Plaid, Qualcomm
  (`last_health='empty'`, `failed_scans=0`, current `last_ok_at`, `max_listed=0` across 12 scans). The
  dangerous class: a board that fails loudly gets fixed; one that succeeds at nothing does not.
- **7 of the 12 dead boards return HTTP 422**, which Workday sends for a malformed request body, not for
  auth — probably wrong tenant/site slugs, i.e. recoverable coverage. Never diagnosed. No backoff, no
  auto-disable, no quarantine: `get_watched_companies` filters on `watched` alone.
- **`unchanged` is unaudited.** 59 of 135 boards listed nothing in run 67 on a payload hash. No test exists
  for a hash misreporting a changed board. A false `unchanged` is silent, permanent, undetectable.
- **`hidden_hard_filter` (17,891 = 59% of the corpus) is the only bucket with no `--include-` drain**,
  against `CLAUDE.md`'s "every quarantine needs a drain".
- **The 8-vs-42 shortfall is a separate problem from coverage.** `capped_by_top_n` = 3,502; job-apps' 42 is
  a natural yield with no top-N constant anywhere. Raising the cap matches the *volume* and does not close
  the *parity* gap — the shortlists would overlap ~8%. The cap also makes P7's own gate un-runnable, since
  per-source yield is `8/26,997` with the numerator fixed by construction.

### The instrument is a persistence problem, not a fetching problem

Every provider already enumerates its whole board and two already parse a server total (`total`,
`totalFound`); Greenhouse returns `meta.total` — **confirmed live on the cheap `_health_url` shape
`probe_health` already fetches** (stripe 576, databricks 818). None of it reaches the database:
`board_scans.postings_listed` stores `len(snapshot.postings)`, which on a capped board is the budget.
Persisting three nullable columns plus the facet sum costs **zero new HTTP requests**. Coverage must be
published as a five-bucket partition — `measured` / `enumerated_only` / `censored` / `dark` / `stale` —
never folded, mirroring the ABSTAIN invariant, because a ratio over our own array length cannot fail. Full
design, including eight ways the metric could lie: `docs/superpowers/specs/2026-08-22-coverage-assurance-design.md`.

**Rejected.** Implementing anything this session — the owner went AFK mid-decision and Track 5 reverses a
shipped decision. LinkedIn/Indeed adapters — the genuine ToS and anti-bot treadmill the v2 decision was
written about, and it has killed two projects in this space. Naming the funnel section `coverage` — that
key already means résumé keyword coverage (`tailor/coverage.py`) and the collision would mislead.

**Owner-gated.** Reopening D-008; Track 5 scope (GitHub lists / + first-party adapters / everything /
instrument-first); `DEFAULT_TOP_N`; the funnel `artifact_version` bump for a `board_coverage` section; and
whether breadth-is-last still binds, since Tracks 3–5 all add input and conversion is still unproven.

---

## D-272 — Mit's ruling: three discovery lanes in, bespoke adapters out, cap to 40

**2026-08-22 · owner ruling on D-271's findings. Reverses two shipped decisions.** Recorded before any
code so it is never re-litigated after a context reset.

### What was ruled

1. **Add three discovery lanes**, in this order: **(a) GitHub new-grad lists** (SimplifyJobs, jobright,
   vanshb03, speedyapply); **(b) hiring.cafe**; **(c) LinkedIn + Indeed via JobSpy**.
2. **Do NOT build bespoke first-party adapters** (Amazon, Apple, TikTok/ByteDance).
3. **`DEFAULT_TOP_N` 8 → 40.**

### What this reverses, and on what evidence

`boardwatch-vs-jobapps-decision-v2.md` §4.1/§6 excluded scraper/aggregator sources from the default
distribution as a "ToS + maintenance trap" that "has already killed two projects in this exact space
(JobFunnel, AIHawk)". Mit's challenge: job-apps has run those sources without being banned, so the risk is
empirical, not theoretical. **Measured over job-apps' full ledger history:**

| source | observations | days active | last 14 days |
|---|---:|---:|---:|
| linkedin | 22,016 | 22 of 23 | **14 / 14** |
| indeed | 15,189 | 22 of 23 | **14 / 14** |
| hiringcafe | 4,318 | 22 of 23 | **14 / 14** |
| jobright | 2,865 | 22 of 23 | **14 / 14** |
| simplify | 273 | 21 of 23 | 13 / 14 |

**Correction to the premise, recorded because it bounds the confidence: the history is 23 days, not
months.** The ledger's first intake is 2026-07-30. Within that window there are no gaps and no
degradation.

**The maintenance treadmill is real, and it killed the opposite class.** Dead in the same store:
`hn` (1 day), `smartrecruiters_api` (2), `amazon_api` (2), `jobicy` (1), `himalayas` (1),
`oracle_hcm_api` (1), `workable_api` (1). Two never worked at all — **Handshake returns HTTP 403 and has
zero rows, ever**, and HigherEdJobs was killed by Incapsula (`HIGHEREDJOBS_FEEDS` commented out). Every
casualty is a **bespoke first-party adapter or a small niche API**; not one is an aggregator. This is why
(2) was ruled: `amazon_api` produced 19 observations over 2 days and was disabled, while Amazon roles still
reach job-apps' eligible set 33 times via LinkedIn and jobright.

Lane quality by job-apps' own `materialization_state` (its downstream-progress field), which independently
reproduces the recommended ordering: simplify 75.8% · speedyapply 63.2% · hiringcafe 57.6% · vanshb03
57.1% · jobright 36.6% · **linkedin 28.3% (8,623 pending)** · **indeed 19.8% (10,374 pending)**. The
curated lists convert; LinkedIn and Indeed are volume, not quality.

### The blocker attached to this ruling — a lane without a body produces zero leads

**The eligibility engine is body-only.** `eligibility/preflight.py` selects
`posting_versions.body_text` and passes it alone to `evaluate`; a stub is a whitespace-only body
(`store/run_funnel_queries.py:count_stub_postings`), and under **D-250** a zero-evidence verdict abstains
to `uncertain`. Aggregator postings arrive as title + URL. **Shipping any of the three lanes without a
JD-body acquisition step adds corpus and produces no leads at all** — every row would sit in `uncertain`.

This is exactly the condition `PROGRAM.md` §4 set when it deferred the JD acquisition chain "to P7 where a
non-API source might first appear". Three have now appeared, so the deferral is met and the row is struck.

**boardwatch does not need job-apps' 2,200 lines.** P7 already *requires* a dereferencing step for any
aggregator lane ("a hard assertion that no aggregator URL survives into the final apply surface"). That
same step is the body fix: an aggregator link mostly resolves to a Greenhouse/Lever/Ashby/Workday posting
whose parser already exists. Dereference, then reuse the provider. A link that resolves to nothing
parseable stays a stub and is reported as one — it must never be quietly dropped, and the stub rate
(currently 17 of 30,243 = 0.056%) becomes the metric that governs whether a lane is pulling its weight.

### Ordering

Track 1 (the coverage instrument) ships **first**, before any lane. Not caution — necessity: a banned or
broken lane currently dies silently, and per-lane coverage reporting is what turns that into a visible
`dark` bucket the next morning. It is also the only way to judge a new lane, since P7's rule is "judge
every new source by leads produced over ≥3 runs".

**Rejected.** Bespoke first-party adapters for Amazon/Apple/TikTok — measured to be the fragile class.
Shipping a lane before the instrument. Shipping a lane without dereferencing.

**Still owner-gated.** The funnel `artifact_version` bump for a `board_coverage` section; whether the
15,535-posting backlog is drained faster by raising `detail_fetch_budget`; and the 17 silent boards.

---

## D-273 — The coverage instrument shipped, and `main` is deliberately ahead of the checkout that runs

**2026-08-22, overnight, owner asleep under a standing instruction to finish.** Implements
D-271/D-272. Two PRs merged: **#124** (the measurement and the lane ruling) and **#125** (the code).
`make check` exit 0 — 7,153 passed, 4 xfailed — and full CI green on all six checks.

### What shipped

`boardwatch coverage`, reporting every watched board as a **seven-way partition** that never folds a
bucket into a neighbour: `measured` · `enumerated_only` · `censored` · `dark` · `stale` ·
`unscanned` · `unreadable`. Four nullable `board_scans` columns carry it
(`board_reported_total`, `board_enumerated`, `detail_deferred`, `board_total_censored`), populated
from values the six providers already computed — **zero additional HTTP requests**. Plus Workday's
uncapped facet sum, and `DEFAULT_TOP_N` 8 → 40.

Two buckets beyond the five the spec designed. `unscanned` (watched, no scan row this run) was added
because an `INNER JOIN` was silently dropping such boards — the exact "a board must not vanish from
the denominator" lie the feature exists to prevent, and reachable routinely, since `scan --company X`
mints a run containing only that company. `unreadable` was added so one unclassifiable row degrades
**one board** instead of the whole report. Folding either into `dark` would have conflated "we tried
and it broke" with "we never tried".

### Measured, on a copy of the live store after a real 135-board scan

`measured 90 · enumerated_only 11 · censored 4 · dark 12 · stale 18 · unscanned 0 · unreadable 0`
— exactly 135. **Global measured coverage 82.7%, 26,183 held of 31,643 stated.**

**Target's real board is 12,097 postings against 649 held — 5.4%, the largest hole in the corpus.**
...
Ten build tasks, each implemented by a fresh subagent and reviewed by another; six needed a fix
round. The whole-branch review then found seven more that no task review could see, because they
live in the seams:

1. **`board_enumerated` carried three incompatible meanings across six providers** — distinct ids /
   raw rows including id-less / post-parse-failure. It defeated the instrument's own id-less-row
   detector: SmartRecruiters *detected* the Mastercard `collected 1128 of 1129` case using
   `listed_ids`, then persisted 1129, so the shortfall read 0. **A persisted column cannot be
   corrected retroactively**, which is why this was fixed before any scheduled run wrote rows. All
   six now call one shared `count_listed_ids`; no provider needed an exception.
2. **The known-positive control test returned before reaching the code it controlled** — it would
   have passed with the entire facet block deleted. The facet arithmetic's only assertion was a
   fixture agreeing with its own constant.
3. `_uncapped_total` raised on six malformed payload shapes, on exactly the large censored tenants
   the instrument targets, degrading a board from `partial` to `failed`.
4. One malformed board total took down the whole report rather than one board.
5. A censor value was persisted as if it were a recovered total; now an unrecovered censor returns
   no total at all, so a floor is never mistaken for a measurement.
6. `censored` withheld `shortfall` as well as `ratio`, keeping Citi's 3,989-posting gap out of every
   summary line. Shortfall is now published; the **ratio stays `None`** — publishing one from a
   second aggregation path is a bigger claim than the spec authorised, and that question is open.
7. The schema guard caught an absent schema but not a stale one, which was the raw traceback it
   existed to prevent.

Every fix was mutation-verified: the covering test was shown to go red when the fix was reverted.

### The limitation that bounds the deliverable

**Nothing reads the new columns unattended.** `run_pipeline`, the morning digest and `notify` are
untouched; `coverage` is a manual command. A scheduled run **persists** coverage and **reports**
nothing. Closing that needs a `board_coverage` section in the funnel artifact and therefore an
`artifact_version` bump — the same class as D-267's `locations` on `Lead`, and **Mit's**.

**Owner-gated, carried forward.** The `artifact_version` bump; arming the checkout; whether
`censored` boards should publish a ratio; a `CheckConstraint` on `board_total_censored` (considered
and deliberately not added — a CHECK cannot protect a value written by direct SQL, so the typed
`UnknownCensorFlag` and the degrade-one-board path are the load-bearing half); `detail_fetch_budget`;
the 17 silent boards; and sub-projects 3 and 4 of the coverage program (JD acquisition, then the
three approved discovery lanes), neither started.

---

## D-274 — The coverage instrument reports itself; both artifacts gain a section and both versions bump

**2026-08-22 · the mute-instrument gap D-273 left open.** D-273 shipped a correct seven-bucket
coverage partition and closed with the limitation stated plainly: *nothing reads the columns
unattended.* A scheduled run persisted four `board_scans` columns and reported nothing, so the only
way to see coverage was to type `boardwatch coverage`. Mit's verbatim ask is "confidence and
clarity, **every day**", which a manual command cannot satisfy.

**Choice: render it into both artifacts a scheduled run already writes, from ONE load.**

- `reports/board_coverage.py` gains `board_coverage_to_dict`, `board_coverage_headline` and
  `board_coverage_table` — pure, no I/O, no change to any shipped invariant.
- Funnel: a `board_coverage` key immediately after `scan` (the section it supplies the missing
  denominator for) and a `## Board coverage` markdown section. **`ARTIFACT_VERSION` 5 → 6.**
- Morning: a `## Discovery reach` block. **`ARTIFACT_VERSION` 1 → 2.**
- `cli/run_cmd.py` prints one `board coverage → …` line, which is what lands in
  `~/Library/Logs/boardwatch-run.log` on the 08:00 tick.
- `boardwatch coverage --json` was refactored to call the SAME serializer, so the command and the
  artifacts cannot describe one number two ways. That removes a drift class rather than adding a
  test asserting two dict literals agree.

**The load happens ONCE, in `runner.py`'s `finally`, and the same object goes to both emitters.**
This is load-bearing, not stylistic: `held` is counted straight out of `postings` with **no run
dimension** (`store/coverage_queries.py`), so two loads seconds apart can legitimately differ, and
one run's two artifacts disagreeing about its own coverage is worse than neither reporting it. Both
artifacts therefore carry `note` in JSON and a prose caveat: stated totals are this run's, `held` is
as of write time, and the section is **not** a frozen historical record.

**Scoped to `run_id`, explicitly.** `load_board_coverage` defaults to the newest run holding any
`board_scans` rows; an artifact stamped `funnel-N` must describe run N, so the runner passes
`run_id=summary.run_id`. Runs predating the columns (67 and earlier) therefore report honestly
rather than borrowing run 68's numbers — verified: run 67's four columns are 135 rows of NULL.

**A coverage failure costs the SECTION, never the artifact.** `_emit_funnel` and `_emit_morning` are
each wrapped in one `except Exception` that prints a warning and writes **nothing**, so an exception
raised while assembling coverage would have traded the whole funnel for a nice-to-have number.
`load_board_coverage` already degrades a single bad ROW to `unreadable` (D-273), but a whole-SELECT
failure — a store predating the four columns raises `OperationalError` — is a different fault and
got its own guard, `_load_board_coverage`. **The seam test caught a real defect while being
written:** `console` is not module-level in `runner.py`, so the guard's own warning raised
`NameError` inside the handler. A guard that crashes is not a guard.

**Deliberately NOT done, because it is Mit's and remains open:** publishing a ratio for `censored`
boards. `BoardCoverage.__post_init__` actively **enforces** `ratio is None` there — that guard is
the mechanism preventing unfailable ratios, and editing it to add a facet-derived ratio while the
owner is unavailable would pre-empt the decision, not implement it. Instead the already-computed
absolute `shortfall` is published prominently: the four censored boards are short **18,927**
postings in total, which makes the largest holes in the corpus visible without a ratio.

**Also corrected: `notify` was never a candidate surface.** D-273's limitation named
`run_pipeline`, the morning digest and `notify`. Measured this session: `notify` is a standalone CLI
command, `runner.py` imports nothing from it, and the launchd plist runs only
`boardwatch run --project` — there is no scheduled `notify` job. Making it read the columns could
not have made coverage visible unattended.

**Measured through the shipped read path**, on a scratch store seeded from live values (the live
store was never opened read-write; a 1.2 GB copy was rejected at 97% disk capacity): run 68 reports
**82.4%, 26,075 held of 31,629 board-stated**, buckets 90 / 11 / 4 / 12 / 18 / 0 / 0 summing to
135. Worst measured: Capital One 34.7%, Wells Fargo 36.3%, Salesforce 42.5%. **One measured board
states a total of 0** and is excluded from the roll-up's numerator and denominator rather than
folded in.

**What that measurement does and does not establish**, stated precisely because the looser claim
was written first and is wrong. It shows the RENDER path produces the same figures the CLI path
does, from the same four columns. It is **not** an independent corroboration of the coverage number
itself: the scratch store was seeded from those very columns, so a wrong column value would be
reproduced faithfully. Nor does it "reproduce D-273" — that entry recorded 82.7% / 26,183 of 31,643
from a store copy taken at a different moment, which is a different number, and the 82.4% triple
was already on record as run 68's live reading.

**Rejected:** a separate `coverage-N.json` artifact (no version bump needed, but a third file to
open every morning and the number sits away from the run it describes); funnel-only (leaves the
file Mit actually opens mute); morning-only (nowhere for the 135-row table, and it cuts against
`morning.py`'s stated contract of never restating the funnel); `ruff format` on the touched files
(the repo is broadly not format-clean and the gate does not check it, so it would have buried a
surgical diff).

**Naming.** The section is `board_coverage`, never `coverage` — that key has meant résumé keyword
coverage since P4 item 6. The morning block is titled **Discovery reach** because every lead in
that file already prints a `coverage:` line: in JSON the two are distinct keys, but on the page a
human would read one word meaning two things.

**Owner-gated, carried forward:** whether `censored` publishes a ratio; **merging and arming this**
(the launchd venv is an editable install resolving to the primary tree's `src/`, so `git pull` in
that checkout is what arms it — merging alone arms nothing); `detail_fetch_budget`; the 17 silent
boards; and sub-projects 3 and 4 (JD acquisition, then the three approved lanes), neither started.

---

## D-275 — "Dereference, then reuse the provider parser" is the wrong shape for four of six providers

**2026-08-22 · read-only scoping of the JD-acquisition prerequisite (sub-project 3). Corrects the
plan recorded in D-272 and in the design doc's Track 5 blocker. Nothing built.**

D-272 attached a blocker to all three approved discovery lanes: the engine is body-only, so an
aggregator lane without JD-body acquisition adds corpus and produces zero leads. The recorded fix
was "an aggregator link mostly resolves to a Greenhouse/Lever/Ashby/Workday posting whose parser
already exists — dereference, then reuse the provider."

**Half of that premise is confirmed and half is false.**

**Confirmed: the parsers are separable.** Every provider's body parsing lives in a module-level pure
function outside the fetch loop, and the tests already call them directly — `greenhouse.parse_job`,
`lever.parse_posting`, `ashby.parse_job`, `workable.parse_job`,
`smartrecruiters.parse_posting`, `workday.parse_posting`. `ashby.parse_payload` is the one existing
bytes → `BoardSnapshot` seam and is the pattern to copy.

**False: that there is anything to fetch.** The `Provider` protocol declares only `board_url`,
`fetch_board` and `healthcheck` — there is **no `fetch_posting`**, and a per-posting URL builder
(`_detail_url`) exists for **SmartRecruiters and Workday only**. The reason is that the other four
board endpoints already inline every body, verified by reading the URLs:
`greenhouse ?content=true`, `lever ?mode=json`, `ashby ?includeCompensation=true`,
`workable ?details=true`.

**So for four of six providers the lane's body problem is COMPANY DISCOVERY, not posting
dereferencing.** `parse_board_target` → `upsert_watch` → `run_scan` already turns a pasted board URL
into the company's **entire board for zero new code**. Dereferencing one of those links instead
means writing a URL builder against an API this repo has never called, with no fixture — and every
new fixture owes a live-verified, drift-fingerprinted recording. If most aggregator links resolve to
these four, the dereferencing design solves a problem the codebase does not have.

**The single biggest obstacle, and it sits exactly where the plan cannot route around it: nothing
extracts a posting identifier from a URL.** `parse_board_target` returns `(provider, board_slug)`
and discards the rest, so `https://boards.greenhouse.io/acme/jobs/123` yields
`("greenhouse", "acme")` and the `123` is gone. SmartRecruiters needs a `posting_id` for its
`_detail_url`; **Workday needs an `externalPath` path-string, not an id**, and mapping the public
`en-US/{site}/job/...` form to the CXS path is plausible but verified nowhere in this repo. Those two
are precisely the providers where dereferencing is the only option.

**Secondary costs, in order.** (1) `FetchResult` carries only `status_code`, `content`,
`not_modified`, `observed_validators` — **no final URL and no redirect history** — so although the
client sets `follow_redirects=True`, the fetcher cannot today report where a link landed. That is a
small, concrete, unavoidable change. (2) `count_stub_postings` is **corpus-wide** with no per-source
split, and `SourceOutcome` has no stubs field, so a new lane's stub rate silently moves the global
number instead of riding existing instrumentation — new plumbing, and it matters because P7 judges a
source by leads over ≥3 runs. (3) Per-host serialization is 1.0s and dereferencing 40 leads is fine
while a 12,096-posting corpus is not — the pressure `detail_fetch_budget` already exists to bound.

**A sharp edge in the store path.** `apply_board` with `status='complete'` runs `_process_missing`,
which closes every open posting of that `company_id` absent from the snapshot — so a lane emitting
`complete` for a company that also has a real board would close the board's inventory. It must emit
`partial` or own separate company rows. Also: `postings.job_id` is nullable but a trigger ABORTs on
NULL, so any new insert path must mint a `jobs` row in the same transaction, and skipping
`_write_posting_identity` turns off duplicate suppression corpus-wide.

**An argument FOR dereferencing anyway, where it is possible.** If a link resolves to the real
`(provider, slug, provider_posting_id)`, `UNIQUE(company_id, provider_posting_id)` makes the
aggregator-discovered posting **converge** with the eventual board scan rather than duplicate it.
That is a real point in favour of dereferencing over synthetic company rows.

**Prior art not to rebuild:** `core/board_urls.py:parse_board_target`,
`core/host_class.py:classify_host` (already lists linkedin/indeed/jobright as aggregators),
`core/normalize.py:normalize_url`, and `pipeline/liveness.py:build_prober` — the only existing
"GET one posting URL" path and the right template for the fetch half.

**No decision is taken here and nothing is built.** Sub-project 3 still needs its own brainstorm;
this entry exists so the false half of the premise is not re-derived, and so the design starts from
company discovery for four providers and true dereferencing for two.

---

## D-276 — Gate P3 reset to 0 of 7 when run 68's tick failed; it was recorded as "stays at 2"

**2026-08-22 · a counting correction, found while confirming the armed state. Corrects D-273 and
every STATE line that carried "2 of 7" after 2026-08-22.**

`PROGRAM.md`'s Gate P3 reads **"7 *consecutive* unattended runs"**. Run 68's 08:00 scheduled tick
exited 1 (`CommandError: Can't locate revision identified by 'p_board_coverage'` — a subagent had
migrated the live store while the checkout was pinned to older code). A failed unattended run breaks
consecutiveness, so the streak went to **zero**. D-273 and STATE both recorded "Gate P3 stays at 2 of
7", which reads the counter as a monotonic tally of clean runs rather than as a consecutive streak.

**Evidence, from the counter that is the only authority:** `launchctl print
gui/$(id -u)/com.boardwatch.run` reports `runs = 4` and `last exit code = 1`, and
`~/Library/Logs/boardwatch-run.log` carries the traceback. Invocations 2 and 3 were the clean ticks
(runs 63 on 2026-08-20 and 66 on 2026-08-21); invocation 4 died.

**Consequence, and it is the reason this is worth an entry.** The next clean scheduled tick is **1 of
7, not 3 of 7** — seven more clean days, not five. Gate P4 (the owner's blind craft review) is barred
until P3 is met, so the P4 date moves with it. Reading a broken streak as a preserved tally would
have had the program expecting a gate two runs earlier than it can arrive, which is the kind of error
that only surfaces when the gate fails to close on time.

**Not changed:** the gate's wording. "Consecutive" is doing real work — it is what makes the gate a
statement about *operational stability* rather than about an accumulated count of good days, and a
run that crashes is exactly the instability it exists to detect. Weakening it to a tally so the
earlier number stays true was considered and rejected; the metric says what it says (`CLAUDE.md`: a
gate is met when its metric says so, not when the work feels done).

**Also not changed:** whether a *manual* `run --project` can contribute. It cannot, and that is
unchanged and correct — runs 65, 67, 68-rerun and 69 are all manual and move nothing.

---

## D-277 — The hard-filter bucket gets a drain, and its bool was hiding four clauses behind one counter

**2026-08-22 · closes the `hidden_hard_filter` row STATE has carried as `open`.** PR #129, green
under `make check` (7,196 passed, 4 xfailed, coverage 95.76%).

`hidden_hard_filter` cut **17,891 postings on run 67 — 59% of the corpus**, the single largest cut
in the pipeline, and the only drop bucket with no `--include-` flag. Against the standing rule that
every quarantine needs a drain.

**The blocker was not the missing flag.** `rank.heuristic.passes_hard_filters` returned a bare
`bool`, so which clause fired and which token matched were discarded at the `return` — no consumer
*could* have reported it. Fixed by adding `hard_filter_verdict`, returning a frozen `HardFilterVeto`
(a closed-catalog `clause` plus the `detail` that decided it), with `passes_hard_filters` reduced to
a thin wrapper over it. One source of truth, so the bool and the reason cannot disagree about the
same title. **Rejected: changing `passes_hard_filters`' own signature** — 28 assertions in
`tests/unit/test_ranking.py` call it as a predicate, and churning all of them buys nothing;
`notify`, `stats` and `count_filter_matches` keep the bool untouched.

**FALSIFIED, and it had been cited for 16 days: `hidden_hard_filter` is NOT "all `exclude_titles`".**
It is **four** independent clauses — the `exclude_titles` word-bounded veto (always), plus
remote-only, the hard US location gate (D-251) and the foreign-ad title marker (D-264) in `hard`
mode. The "100% exclude_titles / 0% location" split is a 2026-08-06 measurement taken when
`location_filter_mode` was `soft` and two of the four clauses **did not yet exist**. D-265 is
literally a correction of the same error in the funnel note — which printed "hard mode has never
been measured firing" beside 17,189 drops from that clause in run 66. The identical stale sentence
was still in `cli/top_cmd.py`'s `RankedResults.hidden_hard_filter` field comment ("Two clauses, but
only one has ever been observed firing … because `location_filter_mode` defaults to `soft`"), wrong
on three counts; D-265 fixed the funnel twin and left this one. Now corrected.

**Owner rulings, both taken this session.** (1) **The drain is UNBOUNDED by the rank cutoff.** A
drain bounded by `limit` reaches only the vetoed rows that would also have ranked, which cannot
audit the 59% that never gets near it; precedent exists both ways
(`--include-duplicates` is bounded, the applied-state drain has a test asserting it is not).
(2) **`_shortlist_line` now names the bucket**, and `hidden_hard_filter` LEAVES
`SUMMARY_LINE_EXEMPT` — the exemption was defensible only while the bucket was un-inspectable.

**No new drop bucket, so no mirror site moves** — the clause rides on the ROW (`RankedPosting`
twin fields `hard_filter` / `hard_filter_reason`, the same shape as `band`/`band_reason`),
consistent with D-251 and D-264 folding their drops in here precisely because a new bucket has
≥27 hand-maintained mirror sites. The drained row `continue`s **before** `surfaced_job_ids`, so
inspecting the quarantine does not record `seen` and close the drain behind you (D-110). `show <id>`
gains a `Hard filter:` line — it already explained the role and seniority gates and was silent on
the largest one. The daily pipeline never passes the flag, so `run --project` is unchanged.

**Method note worth keeping:** the reconciliation-identity test initially asserted **eight** terms
and failed only because a prior iteration's `seen` writes surfaced as `hidden_handled`. The real
identity has **ten**. An eight-term version passes vacuously whenever handled/applied happen to be
0 in the fixture — which is exactly the state `tests/unit/test_top_accounting.py::_accounted` is
in today.

---

## D-278 — JD-body acquisition: the lane order reverses, and the prior art's browser tier is worth 0%

**2026-08-22 · owner rulings on the D-272/D-275 blocker. Design recorded, nothing built.**
Design doc: `docs/superpowers/specs/2026-08-22-jd-acquisition-design.md`.

**RULING 1 — purpose.** The lanes exist to reach companies **no existing route can reach**, not to
multiply yield inside the six ATS families. This decides every trade-off downstream.

**RULING 2 — the lane order REVERSES D-272.** Indeed (via JobSpy) first, then hiring.cafe, then the
GitHub lists. Measured basis: **Indeed's body arrives free inside the search response** —
`description { html }` is a field of the GraphQL search query itself, zero extra requests.
hiring.cafe needs one unauthenticated `GET /api/job-description?id={objectID}` (verified 200, 3,463
chars). **The GitHub lists carry no body at all** — Simplify's 14-key schema has no
description-shaped field across 18,823 records — and **53.3% of their active entries duplicate the
six boards already scanned**, against 34.7% that serve Ruling 1. D-272 ordered them on yield and
materialization quality, which was measured on real data but not on this question.

**RULING 3 — adding a whole board IS breadth**, so the "resolve the link to its company and watch
the board" route (free for 4 of 6 providers, whose board endpoints already inline every body) is
permitted only under an explicit per-run cap on new companies, with every addition reported.
Unbounded, one Simplify pull would add **5,695 companies**.

**RULING 4 — UA scope.** Honest UA stays on the six ATS providers; a browser UA applies only to the
new aggregator fetches. Raised, reconsidered and re-ruled once the premise was corrected: the first
framing asked whether to "match job-apps", which omitted that boardwatch **already ships**
`boardwatch/{version} (+https://github.com/mit112/boardwatch)` and is a published package others
run. Functional basis for the exception: a live Pinterest posting answers 403 to an unfamiliar UA.
robots.txt stays unparsed, unchanged from today.

**THE PREMISE CHECK, run because the chosen lane was ranked on the wrong axis.** 37,442 rows / 9
lists, classified by the netloc of each job link (rule stated, sample size stated, per D-268):
4 of the 5 named companies are present — TikTok 804 postings, ByteDance 312, Amazon 227, Apple 171
— but **SpaceX is on Greenhouse** and needs no lane at all, so D-271's list of five is a list of
four. **Half the non-six tail is three more ATS families, not bespoke company sites**: Oracle Cloud
HCM 31.3%, iCIMS 13.6%, eightfold. Largest single non-six company is **Sainsbury's at 1,639
postings**, a UK grocer the US-only gate discards anyway.

**THE PRIOR ART: the browser tier costs 13% historically and 0% currently.** Measured across
job-apps' 33 scheduled runs / 9,811 stubs: LinkedIn plain HTTP 60.6% (**97.7% hit rate**, one
`requests.get` + one regex), corpus reuse 18.8%, browser 13.0%, unresolved 7.5%. **The browser tier
died 2026-08-11 and ran 11 consecutive runs at exactly zero without anyone noticing** — no chromium
bundle in `~/Library/Caches/ms-playwright/`, and `fetch_rendered_jd` wraps everything in
`except Exception: return ""`, so a missing dependency, a timeout and an empty page are one empty
string; the step exited 0 and logged OK. Corroborated three ways: unchanged candidate mix (185
attempts → 146 OK on 08-11; 53 → **0** on 08-12, same Workday hosts) and wall clock 13m58s → 1m40s
→ 28s. job-apps' own `HANDOVER.md` already documents the no-browser replacement (Workday CXS,
LinkedIn guest, Ashby JSON; "parallel HTTP, no browser"; ~786 bodies; "higher-yield than the generic
browser path").

**AND WHAT IT DOES NOT PROVE — there is no generic careers-page extraction in it.**
`_is_fetchable_url()` gates every fetch on **14 hand-maintained host regexes**, the browser tier on
a separate 8. An arbitrary careers page is never fetched. **Apple and TikTok have no handling at
all**; Amazon has a bespoke adapter — the class D-272 ruled out and the class that visibly rots
(Glassdoor/ZipRecruiter removed at 100% error rate, the Google scrape at 0 JDs in every logged run,
**zapply silently 585–600/day → 0 on 08-19**). `_JDTextExtractor` is a whole-page visible-text dump
with a 300-char floor — no main-content detection, `readability`/`trafilatura` not installed. So the
honest route to Amazon/Apple/TikTok is an aggregator that carries the body, which is Ruling 2.

**A LATENT DEFECT FOUND, to be fixed before any lane lands a row.** `content_hash("")` and
`content_hash("   \n\t ")` are both `e3b0c442…`, the SHA-256 of the empty string (verified by
calling it). That hash feeds `exact_quad`, the **only** suppressing identity kind, and
`dedup._verify_quad` re-verifies with `normalize_body(a) == normalize_body(b)` where `"" == ""`
**passes**. So two genuinely different body-less postings at one company sharing a normalized title
and locations suppress each other, with the verifier agreeing. Live store, read-only: 32,229 open
postings, **13** whitespace-only bodies, all 13 on that hash, on 2 companies, and **0 colliding
(company, title, locations) groups** — latent, not firing. Design: a body-less posting never
receives an `exact_quad` identity, on D-250's reasoning that absence of evidence is not evidence.

**Lane mechanics settled (verified in `scan/apply.py`).** A lane is **not** a seventh `Provider`:
`test_build_providers_one_instance_per_class_keyed_by_name` asserts set *equality* on the six names,
and fixture rule R13 demands a pinned fixture dir per registered provider, both directions. A lane
gets its own protocol returning the same `BoardSnapshot`, so `apply_board` keeps every invariant.
It **must** emit `status="partial"`: `_process_missing` runs on `complete` only, and an **empty
`complete` is legal** — it sets `effective = frozenset()` and marks every open posting of that
company missing (2 consecutive ⇒ closed). It must insert `jobs` first (`postings.job_id` is a
nullable column with an ABORT trigger), and it must call `_write_posting_identity` — skipping it for
one open posting makes `identities_complete()` False, which disables duplicate suppression
**corpus-wide** and degrades every `SourceOutcome.unique` to None.

**Corrections to D-275** (found by reading the code it cites): `ashby.parse_payload` is a **method
on `AshbyProvider`**, not a module-level function, so "the pattern to copy" needs an instance;
`smartrecruiters.parse_posting` is **not called directly by any test**, only through `fetch_board`
behind respx; and greenhouse's board URL is `?content=true&pay_transparency=true`, whose full string
and parameter order are the cache key. **Corrections to D-272's source list:**
`vanshb03/Summer2026-Internships` and `speedyapply/2026-SWE-College-Jobs` now redirect to their
**2027** successors, so a hardcoded name silently ingests a different season; `jobright-ai` ships no
JSON and is 100% tracker links (recoverable only via an embedded schema.org `JobPosting`, verified
5,298 chars, but its `url`/`directApply` are null); Simplify's links are **86.8% direct ATS** with
exactly **1 of 18,823** a `simplify.jobs` tracker, so the v2 tracker objection does not apply to it.

**Rejected:** a generic careers-page reader (the prior art has none; what it has is a host allowlist,
whose honest form is a provider); bespoke first-party adapters (D-272 stands, with more confirming
evidence); any browser automation (out of scope, and 0% current value); **trusting hiring.cafe's
`v5_processed_job_data`** — 90 pre-extracted fields including `visa_sponsorship` and
`security_clearance`, i.e. somebody else's eligibility extractions, which would clear postings by a
route citing no span from the frozen JD; and DuckDuckGo fan-out (disabled in every one of job-apps'
scheduled paths as "slow/flaky unattended", and still shipping).

**Also required with the first lane: per-source stub attribution.** `count_stub_postings` is
corpus-wide with no source filter and `SourceOutcome` has no stub field, so a lane's stub rate moves
the global number with nothing naming the lane — and P7 judges a source by leads over ≥3 runs.

**Owner-gated, still open:** whether Oracle Cloud HCM and iCIMS should become **providers** instead
of or before any lane (~45% of the non-six tail, fits the existing architecture, and job-apps'
`oracle_hcm_api` dying is weak counter-evidence since its `smartrecruiters_api` and `workable_api`
died too while boardwatch's own versions of both work — but it does **not** reach
Amazon/Apple/TikTok); the per-run new-company cap's number; and whether the LinkedIn lane earns its
O(n) request cost when Indeed's body is free.

## D-279 — The body-less suppression is fixed on both paths, and lane 1 gets a client boardwatch can actually install

**Context.** D-278 recorded the JD-acquisition design and named a latent defect that had to be
closed before any lane lands a row. Mit's instruction for this session was to begin the lane work.
Two facts found while planning it changed what "begin" means.

**The defect is real and now demonstrated, not merely argued.** Before the fix, two body-less
postings at one company sharing a normalized title and locations produced
`Suppression(posting_id=2, survivor_posting_id=1, kind='exact_quad')` — a real posting suppressed on
no evidence whatsoever. `content_hash("")` and `content_hash("   \n\t ")` are both the SHA-256 of
the empty string, that hash is a component of `exact_quad` (the only suppressing kind), and
`_verify_quad` re-verified with `normalize_body(a) == normalize_body(b)` where `"" == ""` passes.

**Fixed on BOTH paths, because they fail differently.** `body_evidence()` mirrors
`normalized_locations`: absence returns `None`, so `compute_identities` emits no `exact_quad`. That
closes the computed path. The stored path needed its own change — `_verify_quad` now requires body
**PRESENCE**, not merely equality — because the resolver groups **stored** identities and nothing in
the scan path rewrites them on its own schedule, so rows written before the check existed still
reach it and two absent bodies compare EQUAL. This is precisely the belt-and-braces the same
function already applies to locations (D-083). The `_verify_quad` docstring asserted the defect away
in so many words — *"body_text and content_hash are NOT NULL in the schema, so there is no
missing-body branch"* — and NOT NULL is not non-empty; that sentence is corrected.

**Rejected: an `IDENTITY_ALGORITHM_VERSION` bump.** The module rule is to bump when a normalizer,
key tuple or host-class table changes; none did — only the emission condition. A bump would degrade
suppression to off and every `SourceOutcome.unique` to `None` corpus-wide until a re-backfill of
32,229 open postings, which buys nothing here: the presence check already neutralises the stale rows,
and `write_identities` DROPS each one as an unwanted kind the next time that posting is written, so
they self-heal. **Rejected: withholding `content_hash_only` too.** It is annotate-only and exists so
the Gate P6 audit can see near-misses; leaving it is what keeps `identities_complete()` true, which
counts distinct posting_ids carrying any current-version row — a posting emitting nothing at all
would have turned suppression off corpus-wide, the exact failure the fix is meant to prevent.
Evidence: 13 whitespace-only bodies among 32,229 open postings, 2 companies, **0** colliding
(company, title, locations) groups — latent, not firing. `make check` green: 7210 passed / 4 xfailed,
which is 7196 + the 14 tests added, so the gate demonstrably ran the change (#132).

**RULING REVERSED BY MEASUREMENT: lane 1 does not use JobSpy.** D-278 wrote "Indeed via JobSpy".
`python-jobspy` 1.1.82 pins **`NUMPY==1.26.3`** — an exact pin whose newest wheel is `cp312` — while
boardwatch is `requires-python = ">=3.11"`, CI tests **3.13**, and the dev venv is 3.13.12. The
library therefore cannot install on a supported interpreter, and adding it would break the published
package for every 3.13 user. It also fetches through its own `requests`/`tls-client` stack, which
boardwatch's per-host politeness lock cannot reach, and §4.7 of the design rules that politeness
stays boardwatch's; the spec itself notes JobSpy's per-posting description fetches sit outside even
its own throttle. **Assumption recorded, revisable by Mit:** call Indeed with the `httpx` client and
`Fetcher` boardwatch already ships. Every ruling D-278 rests on survives — the body is still free
inside the search response, and the lane order is unchanged. What changes is only the client, which
D-278 was not deciding. Offered to Mit as a choice with the measurement stated; no answer inside the
window, so the recommended option proceeds. **Second assumption, same status: the per-run
new-company cap is 10** (~300/month; roughly doubles the 37 watched boards in a week, and slow
enough that each addition is readable in the daily report before it compounds).

**Run 68's exit 1 has a root cause, and it is a process failure, not a code defect.** D-276 recorded
the symptom (`CommandError: Can't locate revision identified by 'p_board_coverage'`). The cause: the
live store was stamped to that revision by a run whose tree had the migration, while the migration
file did not reach the primary checkout until **08:17** — seventeen minutes AFTER the 08:00 tick ran
`main`'s tree, which had no such revision. Store head and code head now agree (`p_board_coverage`
both sides) and run 69 succeeded on that code at 11:47, so the condition is repaired. **The
generalizable rule is the one already in memory and it was violated: an agent must be forced onto a
scratch `BOARDWATCH_DATA_DIR` on every invocation, because the live store is the DEFAULT and "don't
touch it" fails open.** A migration is worse than a write: it moves the store's head, so it breaks
the *next* scheduled run rather than the one that erred, and it cost a second Gate P3 day.

**Phase 1 is planned and deliberately contains no network code**
(`docs/superpowers/plans/2026-08-22-lane-groundwork.md`): the acquisition-outcome catalog (ten typed
outcomes, a counter each, `is_silent_outage` so a tier that attempted work and resolved nothing is
reportable), the `Lane` protocol with `lane_snapshot()` as the only constructor so `status="complete"`
is unexpressible, per-source stub attribution, and the company cap. The Indeed client is deferred to
its own plan whose first step pins the GraphQL document, headers and one fixture against the live
endpoint — this session did not verify that request contract itself, and transcribing one from a
summary would put fiction into a plan that forbids placeholders. Also flagged as owed with the
client, not phase 1: §4.5's four quality controls, §4.7's browser-UA `Fetcher`, and a drain for any
stub bucket the lane creates.

## D-280 — The finish line is redefined to a provisional pass; breadth is pulled into scope now; the dedup ruling is fixed

**Context.** Mit: "get development finished ASAP ... I can't wait around for the 7 or 14 days," under an
unlimited-usage window that will not last. A planning session (no code) mapped every remaining item against
what is genuinely calendar-bound. Verified by reading the plist + settings + three explore agents: nothing
in code counts a run streak or aggregates the acceptance window — **Gate P3's "7 consecutive unattended
ticks" is a human judgment read off launchd's own `runs` counter, and cadence lives only in
`~/Library/LaunchAgents/com.boardwatch.run.plist` (`StartCalendarInterval`)** — so P3 compresses to ~1–2
days by firing more often. The 14-day acceptance does NOT compress: it is frozen-system by definition AND
B1 (≥10 NET-NEW deduped leads/day) is bound to real posting inflow — extra intraday runs create no new
postings, so the 7-day `seen_ttl_days` suppresses repeats (→ ~0 net-new → B1 fails) or leaks duplicates
(→ P6 fails).

**Choice.** (1) **Finish line = provisional pass, confirm later** — "done" is 3 clean FROZEN daily runs
meeting all seven bar metrics (B1–B7); the full 14-day acceptance then runs PASSIVELY in the background to
confirm and no longer blocks the "done" call. This reinterprets PROGRAM §1's acceptance gate; the absolute
bar itself is unchanged. (2) **Breadth is pulled into scope NOW** — the three discovery lanes (Indeed →
hiring.cafe → GitHub new-grad lists) are built in this effort, BEFORE the provisional pass, reordering the
roadmap's "P7 breadth after the 14-day run." Justification: parity is the real replacement gap (boardwatch
sees ~8% of job-apps' eligible set) and breadth de-risks sustaining ≥10 net-new/day through the background
confirm once the fetch backlog drains. The "breadth is last" risk (multiplying a leaky funnel) is bounded
because dedup/liveness/eligibility are already MET and lane postings pass every existing gate. (3) **P6
duplicate-kind ruling: only `exact_quad` (company+title+location+body) counts as a duplicate** — Mit
confirmed. (4) **Execution model: sessionized parts 1–6** (order 1→2→3→4→6, part 5 anytime), each its own
session that resumes from the docs and ends in a handoff; **Sonnet-default workers, Opus only for hard
tasks** (seams, subtle correctness, endpoint reverse-engineering, big reviews), no gratuitous fan-out.

**Alternatives rejected.** Blocking on the full 14-day wait (Mit can't, and it is background-able);
compressing the acceptance cadence to fake the clock (the net-new physics above make it self-defeating);
the broader dedup ruling that also counts body-hash or company+title+location (needs a resolver, an
`IDENTITY_ALGORITHM_VERSION` bump + re-backfill, and collapses 727 genuinely-different jobs — the design's
own counterexample).

**Consequence.** The detailed plan lives OUTSIDE the repo at
`~/.claude/plans/lets-use-this-session-staged-wren.md` and in project memory (loaded every session). Next
action: **Part 1** — measure net-new/day, bank P6-b (liveness `dead==0`), write the leakage query, fix the
