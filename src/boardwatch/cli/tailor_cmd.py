...
def run_cmd(
    ...
    if result.rewrites is not None:
        ...
        if any(r["drop_reason"] == "lane_dead" for r in result.rewrites):
            # The typed reason and the fatal flag are both read off the result, not
            # recomputed here: `run_tailor` closes the durable `runs` row before returning,
            # so recomputing the flag is what let an exit-1 run leave a row saying `ok`
            # (D-147 R1). One computation, both consumers.
            console.print(
                f"Tier B stopped: the LLM credential is unusable "
                f"({result.lane_death_reason}). "
