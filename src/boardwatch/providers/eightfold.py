...
class EightfoldProvider:
    ...
    def fetch_board(self, fetcher: Fetcher, request: BoardRequest) -> BoardSnapshot:
        try:
            ...
            return _failed(request.url, f"invalid eightfold slug: {exc}")
        try:
            boot = fetcher.get(request.url, validators=request.validators)
        ...
        try:
            ...
            return _failed(request.url, f"career page bootstrap: {exc}")

        listed, total, errors = self._page_listing(fetcher, host, domain)
        if listed is None:
            return _failed(request.url, errors[0])
        ...
        for row in unseen:
            ...
            try:
                detail_res = fetcher.get(self._detail_url(host, domain, posting_id))
            except FetchFailure as exc:
    ...
    def _page_listing(
        self, fetcher: Fetcher, host: str, domain: str
        ...
        for page in range(_MAX_PAGES):
            ...
            try:
                result = fetcher.get(self._search_url(host, domain, start))
            except FetchFailure as exc:
                if page == 0:
    ...
    def healthcheck(self, fetcher: Fetcher, slug: str) -> BoardHealth:
        ...
        try:
            boot = fetcher.get(self.board_url(slug))
        except FetchFailure as exc:
        ...
        try:
            ...
            return BoardHealth.ERROR
        try:
            result = fetcher.get(self._search_url(host, domain, 0))
        except FetchFailure as exc:
            return health_from_failure(exc)
        data = _envelope_data(result.content)
        if data is None or not isinstance(data.get("positions"), list):
