...
def _apply_lane(
    ...
    try:
        seed_refusal = _persist_seed_work(
            engine, lane, result, run_id, applied_cleanly=applied_cleanly
        )
...
def _persist_seed_work(
    ...
    **D-418 -- the replacement for the old blanket charge is NAMED, not a silent leak.** Not
    ...
    # absent-versus-zero confusion the acquisition tally exists to prevent. It is NOT fatal: the
    # seeds beside it were written.
    ...
    with engine.connect() as conn:
        ...
        company_of = {
            ...
            for row in conn.execute(
                select(postings.c.id, companies.c.provider, companies.c.slug)
    ...
    try:
        ...
        try:
            ranked = rank_open_postings(
                ...
                # T164: the refresh above already ran; this stops `run_preflight` from running
                # it a second time and keeps its "taxonomy changed" line truthful either way.
