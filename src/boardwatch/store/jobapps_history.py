...
a `job_description.txt` with a `Company:`/`Role:` header) is job-apps' own convention,
...
# Case-insensitive: observed files spell both labels capitalised, but nothing else requires it.
_HEADER_LINE = re.compile(r"^\s*(company|role)\s*:\s*(.*)$", re.IGNORECASE)
...
def _read_header(job_description: Path) -> tuple[str | None, str | None]:
    """(company, title) from a job-apps `job_description.txt` header.
    ...
    matched or at a `====` divider — both mark the header's end in every sample seen.
    """
...
def read_jobapps_dir(path: Path) -> tuple[list[HistoryRow], list[MalformedRow]]:
    ...
    for line_no, folder in enumerate(folders, start=1):
        company, title = _read_header(folder / "job_description.txt")
        url = _read_apply_url(folder)
